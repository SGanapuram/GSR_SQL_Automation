...
CREATE view [dbo].[v_MET_trade_detail]                         
(                               
   trader,                               
   contr_date,                               
   trade_num,                               
...
select                              
   ...
   case when bunk.del_date is not null 
           ...
           isnull(tiwp.del_date_from, isnull(tis.storage_start_date,tit.load_date_from) ) 
   end,                              
   case when bunk.del_date is not null 
           then bunk.del_date 
        else isnull(tiwp.del_date_to, isnull(tis.storage_end_date,tit.load_date_to) ) 
   end,                              
   case when bunk.pricing_exp_date is not null 
           then bunk.pricing_exp_date 
        else ti.accum_start_date 
   end,                                
   case when bunk.pricing_exp_date is not null 
           then bunk.pricing_exp_date 
        else ti.accum_end_date 
   end,                              
   ...
   case when bunk.credit_term_code is not null 
        ...
        else isnull(tiwp.credit_term_code, ticp.credit_term_code) 
   end,                                 
   isnull(tiwp.pay_days, ticp.pay_days),                              
   isnull(tiwp.pay_term_code, ticp.pay_term_code),                              
   isnull(tiwp.del_term_code, bunk.del_term_code),                               
   tiwp.mot_code,                              
   isnull(tiwp.del_loc_code, bunk.storage_loc_code),    
   tiwp.transportation,                              
   ticp.cfd_swap_ind,                              
   ticp.efs_ind,                              
   all_quotes_reqd_ind,                              
...
FROM dbo.v_MET_TS_trade trd                                     
        ...
        LEFT OUTER JOIN dbo.v_MET_TS_trade_item_exch_opt tieo                                     
           on ti.trade_num = tieo.trade_num AND                                         
              ti.order_num = tieo.order_num AND                                         
              ti.item_num = tieo.item_num                                     
        LEFT OUTER JOIN dbo.v_MET_TS_trade_item_fut tif                                     
        ...
        LEFT OUTER JOIN v_MET_TS_portfolio_booking_company te                                     
           on te.port_num = tid.real_port_num                                    
        LEFT OUTER JOIN dbo.v_MET_TS_assign_trade at                                      
           on at.trade_num = ti.trade_num and                                         
              at.order_num = ti.order_num and                                         
              at.item_num = ti.item_num                                  
        LEFT OUTER JOIN dbo.v_MET_TS_assign_trade1 agn                                     
           on agn.trade_num = ti.trade_num and                                        
              agn.order_num = ti.order_num and                                        
              agn.item_num = ti.item_num                                  
        LEFT OUTER JOIN dbo.v_MET_TS_lc_allocation la                                      
           on la.lc_num = at.ct_doc_num                                  
        LEFT OUTER JOIN dbo.v_MET_TS_trade_item_wet_phy tiwp                                      
        ...
        LEFT OUTER JOIN dbo.v_MET_TS_fix_price fixprice                                      
           on fixprice.formula_num = tf1.formula_num                                  
        LEFT OUTER JOIN dbo.v_MET_TS_float_price fltprice                                      
...
GO
GRANT SELECT ON  [dbo].[v_MET_trade_detail] TO [next_usr]
GO
