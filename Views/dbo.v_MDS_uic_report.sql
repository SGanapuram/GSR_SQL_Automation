...
CREATE VIEW [dbo].[v_MDS_uic_report]      
(      
tran_date ,       
entity_name ,      
oid ,      
...
(      
      ...
      join allocation_item ai on  key1=ai.alloc_num and key2 =ai.alloc_item_num      
      ...
      and entity_name in('AllocationItem', 'Allocation')      
...
---Cost      
  ...
  urm.resp_trans_id,      
    ...
    join uic_rpt_values_config urvc on urvc.oid=urh.values_config_id      
    join icts_entity_name ien on urvc.entity_id=ien.oid      
    join cost c on key1 = c.cost_num      
    join account a on a.acct_num = c.acct_num      
    where entity_name = 'Cost'      
    and description = 'Cost Amount'      
 union         
---Trade      
      select      
    tran_date 'ModDate',      
    urm.resp_trans_id,      
      ...
      join uic_rpt_values_config urvc on urvc.oid=urh.values_config_id      
      join icts_entity_name ien on urvc.entity_id=ien.oid      
      join trade_item ti on key1 = ti.trade_num and key2 = ti.order_num and key3 = ti.item_num      
      join trade t on t.trade_num = ti.trade_num      
      join trade_order tro on tro.trade_num = ti.trade_num and tro.order_num = ti.order_num      
      join account a on a.acct_num = t.acct_num      
      where entity_name in ('TradeItem', 'TradeItemWetPhy')      
      and operation_type in ('U','D')      
      and description not in ('EstimateInd','Includes Excise Tax','Includes Fuel Tax','Relative Declare Date Ind','Relative Declare Date Type')        
) uc      
left outer join portfolio_tag pt on pt.port_num = uc.port_num and pt.tag_name = 'PRFTCNTR'      
left outer join portfolio p on p.port_num = uc.port_num      
left outer join account te on te.acct_num = p.trading_entity_num       
left outer join icts_user iu on iu.user_init = uc.user_init      
WHERE 1=1    
    

GO
GRANT SELECT ON  [dbo].[v_MDS_uic_report] TO [next_usr]
GO
