...
CREATE view [dbo].[v_get_conc_trade_detail]                
(                
   TradeKey,                
   TradeNum,                
   OrderNum,                
...
--Assay                
SELECT 
   CONVERT(varchar, tis.trade_num) + '/' + CONVERT(varchar, tis.order_num) + '/' 
               + CONVERT(varchar, tis.item_num) 'TradeKey',
   tis.trade_num,
   tis.order_num,
   tis.item_num, 
   ship.oid,
...
from trade_item_spec tis         
        ...
        LEFT OUTER JOIN (select trade_num, 
                         ...
                         from allocation_item ai    
                                 LEFT OUTER JOIN ai_est_actual_spec aies 
								    ON ai.alloc_num = aies.alloc_num and 
									   ai.alloc_item_num = aies.alloc_item_num     
                                 LEFT OUTER JOIN allocation_item_spec ais       
...
--Payable                
SELECT 
   CONVERT(varchar, c.cost_owner_key6) + '/' + CONVERT(varchar, c.cost_owner_key7) + '/'
...
from cost c       
        LEFT OUTER JOIN shipment ship 
		   ON ship.alloc_num = c.cost_owner_key1    
        LEFT OUTER JOIN cost_price_detail cp 
		   ON c.cost_num = cp.cost_num                
        LEFT OUTER JOIN fb_modular_info fc 
...
SELECT 
   CONVERT(varchar, ti.trade_num) + '/' + CONVERT(varchar, ti.order_num) + '/'
...
SELECT distinct 
   ...
   pr.avg_closed_price * (case when isnull(price_pcnt_value, 1) = 0 then 1 
					      ...
					      end / 100) * case when basis_cmdty_code in ('GOLD', 'SILVER') then (3.11035) 
						                    else 1 
									   end             
from trade_item ti                  
        ...
        INNER JOIN v_BI_cmdty_mkt_detail cmdty 
		   On cmdty.cmdty_code = fc.basis_cmdty_code and 
		      cmdty.mkt_code = fc.risk_mkt_code            
        LEFT OUTER JOIN (select commkt_key, 
		                        price_source_code, 
								trading_prd, 
								max(avg_closed_price) avg_closed_price            
                         from price             
                         where price_source_code = 'INTERNAL'            
                         group by commkt_key, price_source_code, trading_prd) pr 
...
GO
GRANT SELECT ON  [dbo].[v_get_conc_trade_detail] TO [next_usr]
GO
