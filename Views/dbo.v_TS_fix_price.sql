...
CREATE view [dbo].[v_TS_fix_price] 
(
   formula_num,
   formula_precision,
...
from (select formula_num,
      ...
      where formula_body_type = 'M' and 
        ...
        JOIN dbo.avg_buy_sell_price_term absp
           on absp.formula_num = fb.formula_num
        JOIN dbo.formula f
           on absp.formula_num = f.formula_num
GO
GRANT SELECT ON  [dbo].[v_TS_fix_price] TO [admin_group]
GO
GRANT SELECT ON  [dbo].[v_TS_fix_price] TO [next_usr]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'VIEW', N'v_TS_fix_price', NULL, NULL
GO
