...
CREATE view [dbo].[v_ti_wet_phy_all_rs]
(
   trade_num,
   order_num,
   item_num,
...
select
   ...
   maintb.trans_id,
   null,
   maintb.cost_adj_qty_1,
   maintb.cost_adj_qty_2,
   maintb.pp_qty_adj_rule_num,
   ...
   it.workstation_id,
   it.sequence
from dbo.trade_item_wet_phy maintb
...
select
   ...
   audtb.trans_id,
   audtb.resp_trans_id,
   audtb.cost_adj_qty_1,
   audtb.cost_adj_qty_2,
   audtb.pp_qty_adj_rule_num,
   ...
   it.workstation_id,
   it.sequence
from dbo.aud_trade_item_wet_phy audtb
        left outer join dbo.icts_transaction it
           on audtb.trans_id = it.trans_id
GO
GRANT SELECT ON  [dbo].[v_ti_wet_phy_all_rs] TO [admin_group]
GO
GRANT SELECT ON  [dbo].[v_ti_wet_phy_all_rs] TO [next_usr]
GO
GRANT SELECT ON  [dbo].[v_ti_wet_phy_all_rs] TO [public]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'VIEW', N'v_ti_wet_phy_all_rs', NULL, NULL
GO
