...
CREATE view [dbo].[v_MET_TS_roll_indicator] 
( 
   trade_num, 
   order_num, 
   item_num 
...
from (select
         ...
         tf.order_num,
         tf.item_num
      from dbo.v_MET_TS_swap_trade_formulas tf
      ...
      select 
         ...
         tf.order_num,
         tf.item_num
      from dbo.v_MET_TS_swapflt_trade_formulas tf
      where exists (select 1
                    from dbo.formula f
...
GO
GRANT SELECT ON  [dbo].[v_MET_TS_roll_indicator] TO [next_usr]
GO
