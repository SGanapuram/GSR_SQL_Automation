...
CREATE view [dbo].[v_MET_TS_trade_item]        
(         
   trade_num,         
   order_num,         
   item_num,         
   ...
   finance_bank_num,         
   contr_load_port, -- Added by Subu 06/18/2012    
   contr_disch_port,-- Added by Subu 06/18/2012   
   cmnt_num, -- Added by Subu 12/06/2012  
   trans_id         
)        
as        
select         
   ...
   ti.order_num,        
   ti.item_num,        
   cast(ti.trade_num as varchar) + '/' + cast(ti.order_num as varchar) +        
              '/' + cast(ti.item_num as varchar),        
   ...
   ti.finance_bank_num,        
   lod.loc_name contr_load_port,  -- Added by Subu 06/18/2012    
   disch.loc_name contr_disch_port , -- Added by Subu 06/18/2012    
   cmnt.cmnt_num, -- Added by Subu 12/06/2012  
   ti.trans_id        
from dbo.trade_item ti        
        ...
        LEFT OUTER JOIN dbo.v_MET_TS_commkt_info cm        
        ...
        LEFT OUTER JOIN dbo.location lod -- Added by Subu 06/18/2012    
           ON lod.loc_code = ti.load_port_loc_code    
        LEFT OUTER JOIN dbo.location disch -- Added by Subu 06/18/2012    
           ON disch.loc_code = ti.disch_port_loc_code    
        LEFT OUTER JOIN dbo.v_MET_TS_accumulation_info acc        
           ON ti.trade_num = acc.trade_num and        
              ti.order_num = acc.order_num and        
              ti.item_num = acc.item_num        
        LEFT OUTER JOIN dbo.v_MET_TS_trade_cost_info c        
        ...
        LEFT OUTER JOIN (select distinct 
                         ...
                         where exists (select 1        
                                       from dbo.v_MET_TS_trade_cost_info c        
								...
								/*Added VOUCHED also as a status for indicating OPEN COST*/                                             
                               isnull(cost_status, 'CLOSED') in ('OPEN', 'HELD','VOUCHED' )) c1        
           ON c1.cost_owner_key6 = ti.trade_num and        
              c1.cost_owner_key7 = ti.order_num and        
              c1.cost_owner_key8 = ti.item_num   
GO
GRANT SELECT ON  [dbo].[v_MET_TS_trade_item] TO [next_usr]
GO
