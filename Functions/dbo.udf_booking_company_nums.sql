...
CREATE FUNCTION [dbo].[udf_booking_company_nums]
(
   @user_logon_id varchar(40)
)
RETURNS @booking_company_nums TABLE 
(
...
declare @MultiCompanyDB          char(1),
   ...
   begin
   	  if len(@popular_bookcomp_list) > 0
   	  begin 
         insert into @booking_company_nums
         select acct_num
         from dbo.account b
...
END;
GO
GRANT SELECT ON  [dbo].[udf_booking_company_nums] TO [admin_group]
GO
GRANT SELECT ON  [dbo].[udf_booking_company_nums] TO [next_usr]
GO
GRANT SELECT ON  [dbo].[udf_booking_company_nums] TO [public]
GO
