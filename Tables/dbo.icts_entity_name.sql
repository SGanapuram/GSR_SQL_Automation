...
[entity_status] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [df_icts_entity_name_entity_status] DEFAULT ('A'),
...
begin
   raiserror ('(icts_entity_name) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.oid = d.oid )
begin
   raiserror ('(icts_entity_name) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(icts_entity_name) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[icts_entity_name] ADD CONSTRAINT [chk_icts_entity_name_entity_status] CHECK (([entity_status]='I' OR [entity_status]='A'))
GO
ALTER TABLE [dbo].[icts_entity_name] ADD CONSTRAINT [icts_entity_name_pk] PRIMARY KEY CLUSTERED  ([oid]) ON [PRIMARY]
GO
ALTER TABLE [dbo].[icts_entity_name] ADD CONSTRAINT [UQ_icts_entity_name_entity_name] UNIQUE NONCLUSTERED  ([entity_name]) ON [PRIMARY]
GO
