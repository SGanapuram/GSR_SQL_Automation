...
[rc_code_status] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [df_rc_code_rc_code_status] DEFAULT ('A'),
...
if not update(trans_id) 
	begin 
	 raiserror ('(rc_code) The change needs to be attached with a new trans_id',16,1) 
	 rollback tran 
	 return 
	end 
...
if exists (select * from inserted i, deleted d 
 ...
 i.rc_code = d.rc_code ) 
	begin 
		 raiserror ('(rc_code) new trans_id must not be older than current trans_id.',16,1) 
		 rollback tran 
		 return 
	end 
/* RECORD_STAMP_END */ 
if update(rc_code) 
	begin 
	 ...
	 begin 
		 raiserror ('(rc_code) primary key can not be changed.',16,1) 
		 rollback tran 
		 return 
	 end 
end 
...
/* AUDIT_CODE_END */ 
GO
ALTER TABLE [dbo].[rc_code] ADD CONSTRAINT [chk_rc_code_rc_code_status] CHECK (([rc_code_status]='I' OR [rc_code_status]='A'))
GO
