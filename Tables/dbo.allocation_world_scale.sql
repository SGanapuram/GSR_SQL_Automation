...
[pay_rec_ind] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [DF__allocatio__pay_r__37703C52] DEFAULT ('P'),
...
begin
   raiserror ('(allocation_world_scale) The change needs to be attached with a new trans_id',10,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(allocation_world_scale) new trans_id must not be older than current trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(allocation_world_scale) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[allocation_world_scale] ADD CONSTRAINT [CK__allocatio__pay_r__3864608B] CHECK (([pay_rec_ind]='R' OR [pay_rec_ind]='P'))
GO
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'allocation_world_scale', NULL, NULL
GO
