...
create trigger [dbo].[exec_logistics_details_deltrg]
on [dbo].[exec_logistics_details]
for delete
as
...
begin
   select @errmsg = '(exec_logistics_details) Failed to obtain a valid responsible trans_id.'
   if exists (select 1
              from master.dbo.sysprocesses (nolock)
              where spid = @@spid and
...
insert dbo.aud_exec_logistics_details
   (
	oid,
	group_num,
	line_num,
...
select
	...
	d.mot_desc,
	d.transporter_name,
	d.title_passage_date,
	d.title_date_actual_ind,
	d.trans_id,	
	@atrans_id,
	d.contract_exec_oid 
from deleted d
...
declare @the_sequence       numeric(32, 0),
   ...
   select @the_entity_name = 'ExecLogisticsDetails'
...
CREATE trigger [dbo].[exec_logistics_details_instrg]
on [dbo].[exec_logistics_details]
for insert
as
declare @num_rows       int,
...
create trigger [dbo].[exec_logistics_details_updtrg]
on [dbo].[exec_logistics_details]
for update
as
declare @num_rows         int,
...
begin
   raiserror ('(exec_logistics_details) The change needs to be attached with a new trans_id',10,1)
   rollback tran
   return
end
...
begin
   ...
   begin
      select @errmsg = '(exec_logistics_details) New trans_id must be larger than original trans_id.'
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.oid = d.oid )
begin
   raiserror ('(exec_logistics_details) new trans_id must not be older than current trans_id.',10,1)
   rollback tran
   return
end
...
begin
   ...
   begin
      raiserror ('(exec_logistics_details) primary key can not be changed.',10,1)
      rollback tran
      return
   end
end
