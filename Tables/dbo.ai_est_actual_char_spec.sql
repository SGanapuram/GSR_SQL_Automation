...
begin
   raiserror ('(ai_est_actual_char_spec) The change needs to be attached with a new trans_id',16,1)
...
begin
   raiserror ('(ai_est_actual_char_spec) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(ai_est_actual_char_spec) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
