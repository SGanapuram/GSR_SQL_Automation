...
begin
   raiserror ('(railcar_ptp_rate) The change needs to be attached with a new trans_id',16,1)
...
begin
   raiserror ('(railcar_ptp_rate) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(railcar_ptp_rate) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
