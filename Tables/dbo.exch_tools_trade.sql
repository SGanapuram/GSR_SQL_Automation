...
begin
   raiserror ('(exch_tools_trade) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(exch_tools_trade) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(exch_tools_trade) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
...
GO
CREATE NONCLUSTERED INDEX [exch_tools_trade_idx1] ON [dbo].[exch_tools_trade] ([creation_date]) INCLUDE ([exch_tools_trade_num], [external_trade_oid]) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [exch_tools_trade_idx2] ON [dbo].[exch_tools_trade] ([external_trade_oid], [exch_tools_trade_num], [commodity], [creation_date]) INCLUDE ([buyer_account]) ON [PRIMARY]
GO
