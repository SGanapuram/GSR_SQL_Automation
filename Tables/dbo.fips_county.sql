...
[active_ind] [bit] NOT NULL CONSTRAINT [DF__fips_coun__activ__284DF453] DEFAULT ((1)),
...
begin
   raiserror ('(fips_county) The change needs to be attached with a new trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(fips_county) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'fips_county', NULL, NULL
GO
