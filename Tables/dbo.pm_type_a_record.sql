...
[vehicle_type] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [df_pm_type_a_record_vehicle_type] DEFAULT ('T'),
...
begin
   raiserror ('(pm_type_a_record) The change needs to be attached with a new trans_id.',16,1)
...
begin
   ...
   begin
      raiserror ('(pm_type_a_record) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[pm_type_a_record] ADD CONSTRAINT [chk_pm_type_a_record_vehicle_type] CHECK (([vehicle_type]='X' OR [vehicle_type]='T' OR [vehicle_type]='S' OR [vehicle_type]='R' OR [vehicle_type]='P' OR [vehicle_type]='D' OR [vehicle_type]='B'))
GO
