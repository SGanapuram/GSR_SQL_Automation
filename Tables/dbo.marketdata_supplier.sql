...
begin
   raiserror('(marketdata_supplier) The change needs to be attached with a new trans_id.', 10, 1)
   if @@trancount > 0 rollback tran
   return
end
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.id = d.id)
begin
   raiserror ('(marketdata_supplier) new trans_id must not be older than current trans_id.', 10, 1)
   if @@trancount > 0 rollback tran
   return
end
...
begin
   ...
   begin
      raiserror ('(marketdata_supplier) primary key can not be changed.', 10, 1)
      if @@trancount > 0 rollback tran
      return
   end
end
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'marketdata_supplier', NULL, NULL
GO
