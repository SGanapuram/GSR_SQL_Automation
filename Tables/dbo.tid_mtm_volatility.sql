...
[use_option_skew] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [DF__tid_mtm_v__use_o__233F2673] DEFAULT ('N'),
...
begin
   raiserror ('(tid_mtm_volatility) The change needs to be attached with a new trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 ...
                 i.vol_num = d.vol_num )
begin
   raiserror ('(tid_mtm_volatility) new trans_id must not be older than current trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(tid_mtm_volatility) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[tid_mtm_volatility] ADD CONSTRAINT [CK__tid_mtm_v__use_o__24334AAC] CHECK (([use_option_skew]='N' OR [use_option_skew]='Y'))
GO
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'tid_mtm_volatility', NULL, NULL
GO
