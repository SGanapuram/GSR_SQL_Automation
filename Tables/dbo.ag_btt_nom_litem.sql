CREATE TABLE [dbo].[ag_btt_nom_litem]
(
...
create trigger [dbo].[ag_btt_nom_litem_deltrg]
on [dbo].[ag_btt_nom_litem]
for delete
as
...
begin
   select @errmsg = '(ag_btt_nom_litem) Failed to obtain a valid responsible trans_id.'
   if exists (select 1
              from master.dbo.sysprocesses (nolock)
              where spid = @@spid and
...
insert dbo.aud_ag_btt_nom_litem
(
fdd_oid,
fd_oid,
line_item_num,
...
create trigger [dbo].[ag_btt_nom_litem_updtrg]
on [dbo].[ag_btt_nom_litem]
for update
as
declare @num_rows         int,
...
begin
   raiserror ('(ag_btt_nom_litem) The change needs to be attached with a new trans_id',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      select @errmsg = '(ag_btt_nom_litem) New trans_id must be larger than original trans_id.'
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.fdd_oid = d.fdd_oid )
begin
   raiserror ('(ag_btt_nom_litem) new trans_id must not be older than current trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(ag_btt_nom_litem) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[ag_btt_nom_litem] ADD CONSTRAINT [ag_btt_nom_litem_pk] PRIMARY KEY CLUSTERED  ([fdd_oid]) ON [PRIMARY]
GO
ALTER TABLE [dbo].[ag_btt_nom_litem] ADD CONSTRAINT [ag_btt_nom_litem_fk1] FOREIGN KEY ([fdd_oid]) REFERENCES [dbo].[feed_detail_data] ([oid])
GO
ALTER TABLE [dbo].[ag_btt_nom_litem] ADD CONSTRAINT [ag_btt_nom_litem_fk2] FOREIGN KEY ([fd_oid]) REFERENCES [dbo].[feed_data] ([oid])
GO
GRANT DELETE ON  [dbo].[ag_btt_nom_litem] TO [next_usr]
GO
GRANT INSERT ON  [dbo].[ag_btt_nom_litem] TO [next_usr]
GO
GRANT SELECT ON  [dbo].[ag_btt_nom_litem] TO [next_usr]
GO
GRANT UPDATE ON  [dbo].[ag_btt_nom_litem] TO [next_usr]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'ag_btt_nom_litem', NULL, NULL
GO
