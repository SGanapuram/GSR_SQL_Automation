...
[loading_schedule] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [DF__ext_trade__loadi__414EAC47] DEFAULT ('O'),
...
begin
   raiserror ('(ext_trade_loading_sched) The change needs to be attached with a new trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(ext_trade_loading_sched) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[ext_trade_loading_sched] ADD CONSTRAINT [CK__ext_trade__loadi__4242D080] CHECK (([loading_schedule]='O' OR [loading_schedule]='D' OR [loading_schedule]='T' OR [loading_schedule]='B' OR [loading_schedule]='L'))
GO
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'ext_trade_loading_sched', NULL, NULL
GO
