...
begin  
   raiserror  ('(pl_offset_transfer) The change needs to be attached with a new trans_id' ,10,1)
   rollback tran  
   return  
end  
...
if exists (select * from inserted i, deleted d  
           where i.trans_id < d.trans_id and  
                 i.oid = d.oid)  
begin  
   raiserror  ('(pl_offset_transfer) new trans_id must not be older than current trans_id.' ,10,1) 
   rollback tran  
   return  
end  
...
begin  
   ...
   begin  
      raiserror  ('(pl_offset_transfer) primary key can not be changed.', 10, 1)
      rollback tran  
      return  
   end  
end  
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'pl_offset_transfer', NULL, NULL
GO
