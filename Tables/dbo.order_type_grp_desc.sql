...
begin
   raiserror ('(order_type_grp_desc) The change needs to be attached with a new trans_id',16,1)
...
begin
   raiserror ('(order_type_grp_desc) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(order_type_grp_desc) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
...
GO
GRANT DELETE ON  [dbo].[order_type_grp_desc] TO [next_usr]
GO
GRANT INSERT ON  [dbo].[order_type_grp_desc] TO [next_usr]
GO
GRANT SELECT ON  [dbo].[order_type_grp_desc] TO [next_usr]
GO
GRANT UPDATE ON  [dbo].[order_type_grp_desc] TO [next_usr]
GO
