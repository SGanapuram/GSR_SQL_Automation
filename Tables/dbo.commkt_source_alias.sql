...
begin
   raiserror ('(commkt_source_alias) The change needs to be attached with a new trans_id',10,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(commkt_source_alias) new trans_id must not be older than current trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(commkt_source_alias) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'commkt_source_alias', NULL, NULL
GO
