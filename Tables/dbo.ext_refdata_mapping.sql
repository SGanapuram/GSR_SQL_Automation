...
begin
   raiserror ('(ext_refdata_mapping) The change needs to be attached with a new trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(ext_refdata_mapping) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
GO
GRANT DELETE ON  [dbo].[ext_refdata_mapping] TO [ictspurge]
GO
GRANT SELECT ON  [dbo].[ext_refdata_mapping] TO [ictspurge]
GO
GRANT UPDATE ON  [dbo].[ext_refdata_mapping] TO [ictspurge]
GO
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'ext_refdata_mapping', NULL, NULL
GO
