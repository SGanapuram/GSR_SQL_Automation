...
[cost_code_type_ind] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [df_cost_code_cost_code_type_ind] DEFAULT ('M'),
[cost_code_order_num] [smallint] NULL,
[pl_implication] [varchar] (20) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [df_cost_code_pl_implication] DEFAULT ('OPEN'),
...
begin
   raiserror ('(cost_code) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(cost_code) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(cost_code) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[cost_code] ADD CONSTRAINT [chk_cost_code_cost_code_type_ind] CHECK (([cost_code_type_ind]='N' OR [cost_code_type_ind]='I' OR [cost_code_type_ind]='M'))
GO
ALTER TABLE [dbo].[cost_code] ADD CONSTRAINT [chk_cost_code_pl_implication] CHECK (([pl_implication]='NO_EFFECT' OR [pl_implication]='INVENTORY' OR [pl_implication]='CLOSED' OR [pl_implication]='OPEN'))
GO
