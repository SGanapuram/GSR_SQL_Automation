...
begin
   set @errmsg = '(fb_event_price_term) Failed to obtain a valid responsible trans_id.'
   if exists (select 1
              from master.dbo.sysprocesses (nolock)
              where spid = @@spid and
...
begin
   raiserror('(fb_event_price_term) The change needs to be attached with a new trans_id', 16, 1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      set @errmsg = '(fb_event_price_term) New trans_id must be larger than original trans_id.'
...
begin
   raiserror('(fb_event_price_term) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
