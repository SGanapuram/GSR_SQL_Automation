...
begin
   raiserror ('(inventory_balance) The change needs to be attached with a new trans_id',16,1)
...
begin
   raiserror ('(inventory_balance) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(inventory_balance) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
