...
begin
   raiserror ('(order_term_evergreen) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(order_term_evergreen) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(order_term_evergreen) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
