...
begin
   raiserror ('(acct_cr_bus_category) The change needs to be attached with a new trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(acct_cr_bus_category) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
