...
[status] [char] (16) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [DF__symphony___statu__550B8C31] DEFAULT ('PENDING'),
...
begin
   raiserror ('(symphony_outbound_data) The change needs to be attached with a new trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(symphony_outbound_data) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[symphony_outbound_data] ADD CONSTRAINT [CK__symphony___statu__55FFB06A] CHECK (([status]='ERROR' OR [status]='PROCESSED' OR [status]='DELETED' OR [status]='DUPLICATE' OR [status]='PENDING'))
GO
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'symphony_outbound_data', NULL, NULL
GO
