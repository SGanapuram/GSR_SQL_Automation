...
[no_bus_ind] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [df_country_no_bus_ind] DEFAULT ('Y'),
...
begin
   raiserror ('(country) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(country) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(country) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[country] ADD CONSTRAINT [chk_country_no_bus_ind] CHECK (([no_bus_ind]='N' OR [no_bus_ind]='Y'))
GO
