...
begin
   raiserror('(quote_period_duration) The change needs to be attached with a new trans_id.', 16, 1)
   if @@trancount > 0 rollback tran
   return
end
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.id = d.id)
begin
   raiserror ('(quote_period_duration) new trans_id must not be older than current trans_id.', 16, 1)
   if @@trancount > 0 rollback tran
   return
end
...
begin
   ...
   begin
      raiserror ('(quote_period_duration) primary key can not be changed.', 16, 1)
      if @@trancount > 0 rollback tran
      return
   end
end
