...
[ref_insp_attr_type_ind] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [DF__portfolio__ref_i__6ECB5E34] DEFAULT ('S'),
...
begin
   raiserror('(portfolio_tag_insp_attr) The change needs to be attached with a new trans_id', 16, 1)
   rollback tran
   return
end
...
begin
   raiserror('(portfolio_tag_insp_attr) new trans_id must not be older than current trans_id.', 16, 1)
   rollback tran
   return
end
...
begin
   ...
   begin
      raiserror('(portfolio_tag_insp_attr) primary key can not be changed.', 16, 1)
      rollback tran
      return
   end
end
...
return
GO
ALTER TABLE [dbo].[portfolio_tag_insp_attr] ADD CONSTRAINT [CK__portfolio__ref_i__6FBF826D] CHECK (([ref_insp_attr_type_ind]='D' OR [ref_insp_attr_type_ind]='N' OR [ref_insp_attr_type_ind]='S'))
GO
