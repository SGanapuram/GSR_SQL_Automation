...
[rule_type] [char] (8) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [df_qty_adj_rule_rule_type] DEFAULT ('Two Spec'),
[price_precision] [tinyint] NOT NULL CONSTRAINT [df_qty_adj_rule_price_precision] DEFAULT ((3)),
...
begin
   raiserror ('(qty_adj_rule) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(qty_adj_rule) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(qty_adj_rule) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
