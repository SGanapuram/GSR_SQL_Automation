...
begin
   raiserror ('(position_group_eod) The change needs to be attached with a new trans_id',16,1)
...
begin
   raiserror ('(position_group_eod) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(position_group_eod) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
