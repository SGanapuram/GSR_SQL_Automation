...
create trigger [dbo].[conc_assay_activity_deltrg]
on [dbo].[conc_assay_activity]
for delete
as
...
begin
   set @errmsg = '(conc_assay_activity) Failed to obtain a valid responsible trans_id.'
   if exists (select 1
              from master.dbo.sysprocesses (nolock)
              where spid = @@spid and
...
insert dbo.aud_conc_assay_activity
   (
    oid,
	conc_contract_oid,
	version_num,
...
declare @the_sequence       numeric(32, 0),
   ...
   set @the_entity_name = 'ConcAssayActivity'
...
create trigger [dbo].[conc_assay_activity_instrg]
on [dbo].[conc_assay_activity]
for insert
as
declare @num_rows       int,
...
create trigger [dbo].[conc_assay_activity_updtrg]
on [dbo].[conc_assay_activity]
for update
as
declare @num_rows         int,
...
begin
   raiserror ('(conc_assay_activity) The change needs to be attached with a new trans_id',16,1)
   rollback tran
   return
end
...
begin
   ...
   begin
      set @errmsg = '(conc_assay_activity) New trans_id must be larger than original trans_id.'
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.oid = d.oid)
begin
   raiserror ('(conc_assay_activity) new trans_id must not be older than current trans_id.',16,1)
   rollback tran
   return
end
...
begin
   ...
   begin
      raiserror ('(conc_assay_activity) primary key can not be changed.',16,1)
      rollback tran
      return
   end
end
