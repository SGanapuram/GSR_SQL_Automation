...
begin
   raiserror ('(bc_mail_criteria_time) The change needs to be attached with a new trans_id',10,1)
...
begin
   raiserror ('(bc_mail_criteria_time) new trans_id must not be older than current trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(bc_mail_criteria_time) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'bc_mail_criteria_time', NULL, NULL
GO
