...
begin
   raiserror ('(acct_bc_ot_crinfo) The change needs to be attached with a new trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(acct_bc_ot_crinfo) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[acct_bc_ot_crinfo] ADD CONSTRAINT [CK__acct_bc_o__order__398D8EEE] CHECK (([order_direction]=NULL OR [order_direction]='S' OR [order_direction]='P'))
GO
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'acct_bc_ot_crinfo', NULL, NULL
GO
