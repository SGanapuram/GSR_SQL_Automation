...
begin  
   raiserror ('(inv_actual) The change needs to be attached with a new trans_id.',16,1)  
   if @@trancount > 0 rollback tran  
...
if exists (select * from inserted i, deleted d  
           where i.trans_id < d.trans_id and  
                 i.oid = d.oid )  
begin  
   raiserror ('(inv_actual) new trans_id must not be older than current trans_id.', 16, 1)  
   if @@trancount > 0 rollback tran  
...
begin  
   ...
   begin  
      raiserror ('(inv_actual) primary key can not be changed.', 16, 1)  
      if @@trancount > 0 rollback tran    
      return  
   end  
end  
...
return  
GO
ALTER TABLE [dbo].[inv_actual] ADD CONSTRAINT [chk_inv_actual_build_draw_ind] CHECK (([build_draw_ind]='A' OR [build_draw_ind]='D' OR [build_draw_ind]='B'))
GO
