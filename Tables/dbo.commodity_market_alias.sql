...
[type] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [df_commodity_market_alias_type] DEFAULT ('U'),
...
begin
   raiserror ('(commodity_market_alias) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 ...
                 i.type = d.type )
begin
   raiserror ('(commodity_market_alias) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(commodity_market_alias) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[commodity_market_alias] ADD CONSTRAINT [chk_commodity_market_alias_type] CHECK (([type]='V' OR [type]='O' OR [type]='M' OR [type]='I' OR [type]='F' OR [type]='E' OR [type]='C' OR [type]='U'))
GO
