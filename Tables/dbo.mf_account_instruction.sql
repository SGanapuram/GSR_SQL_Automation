...
begin
   raiserror ('(mf_account_instruction) The change needs to be attached with a new trans_id',10,1)
...
begin
   raiserror ('(mf_account_instruction) new trans_id must not be older than current trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(mf_account_instruction) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'mf_account_instruction', NULL, NULL
GO
