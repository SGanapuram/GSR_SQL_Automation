...
[equiv_source_ind] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NULL CONSTRAINT [df_aud_trade_item_dist_equiv_source_ind] DEFAULT ('N'),
...
GO
CREATE NONCLUSTERED INDEX [aud_trade_item_dist_idx] ON [dbo].[aud_trade_item_dist] ([dist_num], [trans_id], [resp_trans_id]) WITH (DATA_COMPRESSION = PAGE) ON [PRIMARY]
GO
CREATE CLUSTERED INDEX [aud_trade_item_dist] ON [dbo].[aud_trade_item_dist] ([dist_qty], [real_port_num], [item_num], [order_num], [trade_num], [dist_num]) WITH (DATA_COMPRESSION = PAGE) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [aud_trade_item_dist_idx3] ON [dbo].[aud_trade_item_dist] ([trade_num], [order_num], [item_num], [dist_type], [is_equiv_ind], [what_if_ind], [real_port_num], [resp_trans_id]) WITH (DATA_COMPRESSION = PAGE) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [aud_trade_item_dist_idx2] ON [dbo].[aud_trade_item_dist] ([trade_num], [order_num], [item_num], [trans_id], [resp_trans_id]) WITH (DATA_COMPRESSION = PAGE) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [aud_trade_item_dist_idx1] ON [dbo].[aud_trade_item_dist] ([trans_id]) WITH (DATA_COMPRESSION = PAGE) ON [PRIMARY]
GO
