...
begin
   raiserror ('(gl_account_validity) The change needs to be attached with a new trans_id',16,1)
...
begin
   raiserror ('(gl_account_validity) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(gl_account_validity) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
