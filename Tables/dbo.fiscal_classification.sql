...
[fiscal_class_status] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [df_fiscal_classification_fiscal_class_status] DEFAULT ('A'),
...
begin
   raiserror ('(fiscal_classification) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(fiscal_classification) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[fiscal_classification] ADD CONSTRAINT [chk_fiscal_classification_fiscal_class_status] CHECK (([fiscal_class_status]='I' OR [fiscal_class_status]='A'))
GO
