...
[cost_center_status] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [df_cost_center_cost_center_status] DEFAULT ('A'),
...
begin
   raiserror ('(cost_center) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(cost_center) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(cost_center) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[cost_center] ADD CONSTRAINT [chk_cost_center_cost_center_status] CHECK (([cost_center_status]='I' OR [cost_center_status]='A'))
GO
