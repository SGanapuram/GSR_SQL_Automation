...
begin
   raiserror ('(spread_composition) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(spread_composition) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(spread_composition) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[spread_composition] ADD CONSTRAINT [chk_spread_composition_long_short_ind] CHECK (([long_short_ind]='S' OR [long_short_ind]='L'))
GO
ALTER TABLE [dbo].[spread_composition] ADD CONSTRAINT [chk_spread_composition_spread_qty_factor] CHECK (([spread_qty_factor]>(0)))
GO
