...
[tag_required_ind] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [DF__entity_ta__tag_r__184C96B4] DEFAULT ('N'),
[tag_status] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [DF__entity_ta__tag_s__1A34DF26] DEFAULT ('A'),
...
if @num_rows = 0
   ...
   insert into dbo.portfolio_tag_definition
      (tag_name, tag_desc, tag_status, trans_id)
...
begin
   raiserror ('(entity_tag_definition) The change needs to be attached with a new trans_id',10,1)
   if @@trancount > 0 rollback tran
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.oid = d.oid )
begin
   raiserror ('(entity_tag_definition) new trans_id must not be older than current trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(entity_tag_definition) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[entity_tag_definition] ADD CONSTRAINT [CK__entity_ta__tag_r__1940BAED] CHECK (([tag_required_ind]='N' OR [tag_required_ind]='Y'))
GO
ALTER TABLE [dbo].[entity_tag_definition] ADD CONSTRAINT [CK__entity_ta__tag_s__1B29035F] CHECK (([tag_status]='S' OR [tag_status]='I' OR [tag_status]='A'))
GO
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'entity_tag_definition', NULL, NULL
GO
