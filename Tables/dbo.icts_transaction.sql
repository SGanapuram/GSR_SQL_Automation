...
[executor_id] [tinyint] NOT NULL CONSTRAINT [DF__icts_tran__execu__30F9DF9B] DEFAULT ((0))
) ON [PRIMARY]
GO
...
begin
   raiserror ('The icts_transaction table has experienced the BIG IDENTITY GAP problem..Report this problem to your DBA now.',10,1)
...
begin
   ...
   begin
      raiserror ('(icts_transaction) trans_id can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
GO
CREATE NONCLUSTERED INDEX [icts_transaction_idx5] ON [dbo].[icts_transaction] ([parent_trans_id]) ON [PRIMARY]
GO
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'icts_transaction', NULL, NULL
GO
