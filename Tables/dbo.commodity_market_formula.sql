...
begin      
   raiserror ('(commodity_market_formula) The change needs to be attached with a new trans_id',16,1)      
   if @@trancount > 0 rollback tran      
...
begin      
   raiserror ('(commodity_market_formula) new trans_id must not be older than current trans_id.',16,1)      
   if @@trancount > 0 rollback tran      
...
begin      
   ...
   begin      
      raiserror ('(commodity_market_formula) primary key can not be changed.',16,1)      
      if @@trancount > 0 rollback tran      
...
GO
ALTER TABLE [dbo].[commodity_market_formula] ADD CONSTRAINT [commodity_market_formula_uc1] UNIQUE NONCLUSTERED  ([cmf_num]) ON [PRIMARY]
GO
