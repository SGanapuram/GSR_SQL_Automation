...
[connected_ind] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [df_facility_tank_group_connected_ind] DEFAULT ('N'),
...
begin
   raiserror ('(facility_tank_group) The change needs to be attached with a new trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(facility_tank_group) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[facility_tank_group] ADD CONSTRAINT [chk_facility_tank_group_connected_ind] CHECK (([connected_ind]='N' OR [connected_ind]='Y'))
GO
