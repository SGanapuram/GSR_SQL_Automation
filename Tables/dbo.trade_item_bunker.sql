...
create trigger [dbo].[trade_item_bunker_deltrg]
on [dbo].[trade_item_bunker]
for delete
as
...
begin
   select @errmsg = '(trade_item_bunker) Failed to obtain a valid responsible trans_id.'
   if exists (select 1
              from master.dbo.sysprocesses (nolock)
              where spid = @@spid and
...
insert dbo.aud_trade_item_bunker
   (trade_num,
    order_num,
    item_num,
    port_loc_code,	
...
select
   ...
   d.del_date,	
   d.pricing_exp_date,	 
   d.exp_time_zone_code,	
   d.curr_exch_date,	
   d.transp_price_amt,	
   d.transp_price_curr_code,	
   d.transp_price_uom_code,	
...
declare @the_sequence       numeric(32, 0),
   ...
   select @the_entity_name = 'TradeItemBunker'
...
create trigger [dbo].[trade_item_bunker_instrg]
on [dbo].[trade_item_bunker]
for insert
as
declare @num_rows        int,
...
create trigger [dbo].[trade_item_bunker_updtrg]
on [dbo].[trade_item_bunker]
for update
as
declare @num_rows         int,
...
begin
   raiserror ('(trade_item_bunker) The change needs to be attached with a new trans_id',10,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(trade_item_bunker) new trans_id must not be older than current trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(trade_item_bunker) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'trade_item_bunker', NULL, NULL
GO
