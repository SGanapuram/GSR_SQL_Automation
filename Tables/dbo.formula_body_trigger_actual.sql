...
begin
   raiserror ( '(formula_body_trigger_actual) The change needs to be attached with a new trans_id',16,1)
   rollback tran
   return
end
...
begin
   raiserror ( '(formula_body_trigger_actual) new trans_id must not be older than current trans_id.',16,1)
   rollback tran
   return
end
...
begin
   ...
   begin
      raiserror ('(formula_body_trigger_actual) primary key can not be changed.',16,1)
      rollback tran
      return
   end
end
