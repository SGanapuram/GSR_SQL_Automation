...
begin
   raiserror ('(country_group) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.code = d.code)
begin
   raiserror ('(country_group) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(country_group) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
