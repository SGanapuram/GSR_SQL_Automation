...
[multi_instances_ind] [bit] NOT NULL CONSTRAINT [df_server_config_multi_instances_ind] DEFAULT ((0)),
[critical_ind] [bit] NOT NULL CONSTRAINT [df_server_config_critical_ind] DEFAULT ((0)),
[trans_type_mask] [int] NOT NULL CONSTRAINT [df_server_config_trans_type_mask] DEFAULT ((0)),
...
begin
   raiserror ('(server_config) The change needs to be attached with a new trans_id',16,1)
...
begin
   raiserror ('(server_config) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(server_config) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
...
GO
GRANT DELETE ON  [dbo].[server_config] TO [next_usr]
GO
GRANT INSERT ON  [dbo].[server_config] TO [next_usr]
GO
GRANT SELECT ON  [dbo].[server_config] TO [next_usr]
GO
GRANT UPDATE ON  [dbo].[server_config] TO [next_usr]
GO
