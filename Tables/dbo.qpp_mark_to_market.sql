...
begin
   raiserror ('(qpp_mark_to_market) The change needs to be attached with a new trans_id',16,1)
...
begin
   raiserror ('(qpp_mark_to_market) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(qpp_mark_to_market) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[qpp_mark_to_market] ADD CONSTRAINT [qpp_mark_to_market_pk] PRIMARY KEY CLUSTERED  ([trade_num], [order_num], [item_num], [accum_num], [qpp_num], [mtm_pl_asof_date]) WITH (DATA_COMPRESSION = PAGE) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [qpp_mark_to_market_idx1] ON [dbo].[qpp_mark_to_market] ([mtm_pl_asof_date]) WITH (DATA_COMPRESSION = PAGE) ON [PRIMARY]
GO
