...
[status] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NULL CONSTRAINT [df_quote_status] DEFAULT ('N'),
[symbol] [varchar] (15) COLLATE SQL_Latin1_General_CP1_CS_AS NULL,
[use_for_pl] [bit] NOT NULL CONSTRAINT [df_quote_use_for_pl] DEFAULT ((0)),
...
begin
   raiserror('(quote) The change needs to be attached with a new trans_id.', 16, 1)
   if @@trancount > 0 rollback tran
   return
end
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.id = d.id)
begin
   raiserror ('(quote) new trans_id must not be older than current trans_id.', 16, 1)
   if @@trancount > 0 rollback tran
   return
end
...
begin
   ...
   begin
      raiserror ('(quote) primary key can not be changed.', 16, 1)
      if @@trancount > 0 rollback tran
      return
   end
end
...
return
GO
ALTER TABLE [dbo].[quote] ADD CONSTRAINT [chk_quote_status] CHECK (([status]='I' OR [status]='A' OR [status]='N'))
GO
