...
[acct_bookcomp_status] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [df_acct_bookcomp_acct_bookcomp_status] DEFAULT ('A'),
...
begin
   raiserror ('(acct_bookcomp) The change needs to be attached with a new trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(acct_bookcomp) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[acct_bookcomp] ADD CONSTRAINT [chk_acct_bookcomp_acct_bookcomp_status] CHECK (([acct_bookcomp_status]='I' OR [acct_bookcomp_status]='A'))
GO
