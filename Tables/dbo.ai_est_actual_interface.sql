...
begin  
   raiserror ('(ai_est_actual_interface) The change needs to be attached with a new trans_id'  ,10,1)
   if @@trancount > 0 rollback tran  
...
begin  
   raiserror ('(ai_est_actual_interface) new trans_id must not be older than current trans_id.'  ,10,1)
   if @@trancount > 0 rollback tran  
...
begin  
   ...
   begin  
      raiserror ('(ai_est_actual_interface) primary key can not be changed.'  ,10,1)
      if @@trancount > 0 rollback tran  
...
return
GO
ALTER TABLE [dbo].[ai_est_actual_interface] ADD CONSTRAINT [CK__ai_est_ac__rejec__4DCB2873] CHECK (([rejected_load]=NULL OR [rejected_load]='N' OR [rejected_load]='Y'))
GO
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'ai_est_actual_interface', NULL, NULL
GO
