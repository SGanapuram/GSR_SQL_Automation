...
begin
   raiserror ('(market_pricing_term) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.mpt_num = d.mpt_num )
begin
   raiserror ('(market_pricing_term) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(market_pricing_term) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
...
GO
GRANT DELETE ON  [dbo].[market_pricing_term] TO [next_usr]
GO
GRANT INSERT ON  [dbo].[market_pricing_term] TO [next_usr]
GO
GRANT SELECT ON  [dbo].[market_pricing_term] TO [next_usr]
GO
GRANT UPDATE ON  [dbo].[market_pricing_term] TO [next_usr]
GO
