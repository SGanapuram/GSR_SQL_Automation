...
begin
   raiserror ('(department) The change needs to be attached with a new trans_id',10,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(department) new trans_id must not be older than current trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(department) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
GO
GRANT DELETE ON  [dbo].[department] TO [admin_group]
GO
GRANT INSERT ON  [dbo].[department] TO [admin_group]
GO
GRANT SELECT ON  [dbo].[department] TO [admin_group]
GO
GRANT UPDATE ON  [dbo].[department] TO [admin_group]
GO
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'department', NULL, NULL
GO
