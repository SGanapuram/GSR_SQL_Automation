...
begin
   raiserror ('(parent_guarantee_comment) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(parent_guarantee_comment) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(parent_guarantee_comment) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
