...
create trigger [dbo].[conc_brand_deltrg]
on [dbo].[conc_brand]
for delete
as
...
begin
   set @errmsg = '(conc_brand) Failed to obtain a valid responsible trans_id.'
   if exists (select 1
              from master.dbo.sysprocesses (nolock)
              where spid = @@spid and
...
create trigger [dbo].[conc_brand_updtrg]
on [dbo].[conc_brand]
for update
as
declare @num_rows         int,
...
begin
   raiserror ('(conc_brand) The change needs to be attached with a new trans_id',16,1)
   rollback tran
   return
end
...
begin
   ...
   begin
      set @errmsg = '(conc_brand) New trans_id must be larger than original trans_id.'
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.oid = d.oid)
begin
   raiserror ('(conc_brand) new trans_id must not be older than current trans_id.',16,1)
   rollback tran
   return
end
...
begin
   ...
   begin
      raiserror ('(conc_brand) primary key can not be changed.',16,1)
      rollback tran
      return
   end
end
