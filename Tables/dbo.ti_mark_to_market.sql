...
begin
   raiserror ('(ti_mark_to_market) The change needs to be attached with a new trans_id',10,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(ti_mark_to_market) new trans_id must not be older than current trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(ti_mark_to_market) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[ti_mark_to_market] ADD CONSTRAINT [ti_mark_to_market_pk] PRIMARY KEY CLUSTERED  ([trade_num], [order_num], [item_num], [mtm_pl_asof_date]) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [ti_mark_to_market_idx1] ON [dbo].[ti_mark_to_market] ([mtm_pl_asof_date]) ON [PRIMARY]
GO
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'ti_mark_to_market', NULL, NULL
GO
