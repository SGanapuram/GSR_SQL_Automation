...
create trigger [dbo].[conc_assay_deltrg]
on [dbo].[conc_assay]
for delete
as
...
begin
   set @errmsg = '(conc_assay) Failed to obtain a valid responsible trans_id.'
   if exists (select 1
              from master.dbo.sysprocesses (nolock)
              where spid = @@spid and
...
select
	...
	d.per_spec_uom_code,
	d.spec_min_value,
	d.spec_min_value_text,
	d.spec_max_value,
	d.spec_max_value_text,
	d.spec_typical_value,
	d.spec_typical_value_text,
	d.spec_regulatory_limit,
	d.spec_regulatory_limit_text,
	d.primary_type,
	d.secondary_type,
	d.row_order_num,
	d.analysis_basis,
...
create trigger [dbo].[conc_assay_updtrg]
on [dbo].[conc_assay]
for update
as
declare @num_rows         int,
...
begin
   raiserror ('(conc_assay) The change needs to be attached with a new trans_id',16,1)
   rollback tran
   return
end
...
begin
   ...
   begin
      set @errmsg = '(conc_assay) New trans_id must be larger than original trans_id.'
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.oid = d.oid)
begin
   raiserror ('(conc_assay) new trans_id must not be older than current trans_id.',16,1)
   rollback tran
   return
end
...
begin
   ...
   begin
      raiserror ('(conc_assay) primary key can not be changed.',16,1)
      rollback tran
      return
   end
end
