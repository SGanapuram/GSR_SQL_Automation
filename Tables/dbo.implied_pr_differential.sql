...
begin
   raiserror ('(implied_pr_differential) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.oid = d.oid)
begin
   raiserror ('(implied_pr_differential) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(implied_pr_differential) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
