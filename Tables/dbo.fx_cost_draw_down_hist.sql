...
[draw_down_up_ind] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NULL CONSTRAINT [df_fx_cost_draw_down_hist_draw_down_up_ind] DEFAULT ('D'),
...
begin
   raiserror ('(fx_cost_draw_down_hist) The change needs to be attached with a new trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.oid = d.oid )
begin
   raiserror ('(fx_cost_draw_down_hist) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(fx_cost_draw_down_hist) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[fx_cost_draw_down_hist] ADD CONSTRAINT [chk_fx_cost_draw_down_hist_draw_down_up_ind] CHECK (([draw_down_up_ind]='R' OR [draw_down_up_ind]='D'))
GO
