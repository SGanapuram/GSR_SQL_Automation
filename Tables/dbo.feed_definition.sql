...
[active_ind] [bit] NOT NULL CONSTRAINT [DF__feed_defi__activ__7A8729A3] DEFAULT ((1)),
...
begin
   raiserror ('(feed_definition) The change needs to be attached with a new trans_id',10,1)
   if @@trancount > 0 rollback tran
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.oid = d.oid )
begin
   raiserror ('(feed_definition) new trans_id must not be older than current trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(feed_definition) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'feed_definition', NULL, NULL
GO
