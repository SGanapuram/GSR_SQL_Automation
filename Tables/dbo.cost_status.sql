...
begin
   raiserror ('(cost_status) The change needs to be attached with a new trans_id',16,1)
...
begin
   raiserror ('(cost_status) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(cost_status) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
...
GO
GRANT DELETE ON  [dbo].[cost_status] TO [next_usr]
GO
GRANT INSERT ON  [dbo].[cost_status] TO [next_usr]
GO
GRANT SELECT ON  [dbo].[cost_status] TO [next_usr]
GO
GRANT UPDATE ON  [dbo].[cost_status] TO [next_usr]
GO
