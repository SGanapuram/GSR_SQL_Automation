...
[is_default_test] [bit] NOT NULL CONSTRAINT [DF__qual_slat__is_de__4321E620] DEFAULT ((0))
) ON [PRIMARY]
GO
...
begin
   raiserror ('(qual_slate_cmdty_sptest) The change needs to be attached with a new trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(qual_slate_cmdty_sptest) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'qual_slate_cmdty_sptest', NULL, NULL
GO
