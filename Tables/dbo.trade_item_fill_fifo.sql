...
begin
   raiserror ('(trade_item_fill_fifo) The change needs to be attached with a new trans_id',10,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(trade_item_fill_fifo) new trans_id must not be older than current trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[trade_item_fill_fifo] ADD CONSTRAINT [CK__trade_ite__match__62307D25] CHECK (([match_type]='O' OR [match_type]='F'))
GO
ALTER TABLE [dbo].[trade_item_fill_fifo] ADD CONSTRAINT [CK__trade_ite__p_s_i__613C58EC] CHECK (([p_s_ind]='S' OR [p_s_ind]='P'))
GO
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'trade_item_fill_fifo', NULL, NULL
GO
