...
create trigger [dbo].[conc_cost_deltrg]
on [dbo].[conc_cost]
for delete
as
...
begin
   set @errmsg = '(conc_cost) Failed to obtain a valid responsible trans_id.'
   if exists (select 1
              from master.dbo.sysprocesses (nolock)
              where spid = @@spid and
...
select
	...
	d.contract_exec_detail_oid,
	d.strategy_execution_oid,
	d.strategy_execution_detail_oid,
	d.conc_ref_cost_item_oid,
	d.cost_basis,
	d.exp_rev_ind,
	d.cost_unit_price,
...
create trigger [dbo].[conc_cost_instrg]
on [dbo].[conc_cost]
for insert
as
declare @num_rows       int,
...
create trigger [dbo].[conc_cost_updtrg]
on [dbo].[conc_cost]
for update
as
declare @num_rows         int,
...
begin
   raiserror('(conc_cost) The change needs to be attached with a new trans_id', 16, 1)
   rollback tran
   return
end
...
begin
   ...
   begin
      set @errmsg = '(conc_cost) New trans_id must be larger than original trans_id.'
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.oid = d.oid)
begin
   raiserror('(conc_cost) new trans_id must not be older than current trans_id.', 16, 1)
   rollback tran
   return
end
...
begin
   ...
   begin
      raiserror('(conc_cost) primary key can not be changed.', 16, 1)
      rollback tran
      return
   end
end
