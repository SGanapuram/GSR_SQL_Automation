...
[fx_rate_m_d_ind] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [df_fx_linking_fx_rate_m_d_ind] DEFAULT ('M'),
...
[need_rate_computation] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [df_fx_linking_need_rate_computation] DEFAULT ('Y'),
...
begin
   raiserror ('(fx_linking) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.oid = d.oid )
begin
   raiserror ('(fx_linking) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(fx_linking) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[fx_linking] ADD CONSTRAINT [chk_fx_linking_fx_rate_m_d_ind] CHECK (([fx_rate_m_d_ind]='D' OR [fx_rate_m_d_ind]='M'))
GO
ALTER TABLE [dbo].[fx_linking] ADD CONSTRAINT [chk_fx_linking_need_rate_computation] CHECK (([need_rate_computation]='Y' OR [need_rate_computation]='N'))
GO
