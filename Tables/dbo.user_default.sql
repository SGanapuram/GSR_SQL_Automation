...
[may_not_override] [bit] NOT NULL CONSTRAINT [df_user_default_may_not_override] DEFAULT ((0)),
...
begin
   raiserror ('(user_default) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(user_default) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
