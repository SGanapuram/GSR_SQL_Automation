...
[acct_bank_info_status] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [df_account_bank_info_acct_bank_info_status] DEFAULT ('A'),
...
begin
   raiserror ('(account_bank_info) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(account_bank_info) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(account_bank_info) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[account_bank_info] ADD CONSTRAINT [chk_account_bank_info_acct_bank_info_status] CHECK (([acct_bank_info_status]='I' OR [acct_bank_info_status]='A'))
GO
ALTER TABLE [dbo].[account_bank_info] ADD CONSTRAINT [chk_account_bank_info_p_or_r_ind] CHECK (([p_or_r_ind]='r' OR [p_or_r_ind]='p' OR [p_or_r_ind]='R' OR [p_or_r_ind]='P'))
GO
