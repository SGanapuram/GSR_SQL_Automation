...
[mot_status] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NULL CONSTRAINT [df_mot_mot_status] DEFAULT ('A'),
...
begin
   raiserror ('(mot) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(mot) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(mot) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[mot] ADD CONSTRAINT [chk_mot_mot_status] CHECK (([mot_status]='I' OR [mot_status]='A'))
GO
