...
GO
CREATE CLUSTERED INDEX [aud_accumulation] ON [dbo].[aud_accumulation] ([accum_num], [item_num], [order_num], [trade_num]) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [aud_accumulation_idx] ON [dbo].[aud_accumulation] ([trade_num], [order_num], [item_num], [accum_num], [trans_id], [resp_trans_id]) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [aud_accumulation_idx1] ON [dbo].[aud_accumulation] ([trans_id]) ON [PRIMARY]
GO
GRANT DELETE ON  [dbo].[aud_accumulation] TO [ictspurge]
GO
GRANT SELECT ON  [dbo].[aud_accumulation] TO [ictspurge]
GO
GRANT UPDATE ON  [dbo].[aud_accumulation] TO [ictspurge]
GO
GRANT INSERT ON  [dbo].[aud_accumulation] TO [next_usr]
GO
GRANT SELECT ON  [dbo].[aud_accumulation] TO [next_usr]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'aud_accumulation', NULL, NULL
GO
