...
begin
   raiserror ('(fifo_group) The change needs to be attached with a new trans_id',10,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(fifo_group) new trans_id must not be older than current trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[fifo_group] ADD CONSTRAINT [CK__fifo_grou__item___1DD065E0] CHECK (([item_type]='E' OR [item_type]='F'))
GO
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'fifo_group', NULL, NULL
GO
