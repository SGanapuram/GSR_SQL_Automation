...
[status] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [DF__trade_gro__statu__3B16B004] DEFAULT ('A'),
[trade_exclusion_ind] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [DF__trade_gro__trade__3CFEF876] DEFAULT ('N'),
...
begin
   raiserror ('(trade_group_cmdty_mkt) The change needs to be attached with a new trans_id',10,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(trade_group_cmdty_mkt) new trans_id must not be older than current trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(trade_group_cmdty_mkt) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[trade_group_cmdty_mkt] ADD CONSTRAINT [CK__trade_gro__statu__3C0AD43D] CHECK (([status]='I' OR [status]='A'))
GO
ALTER TABLE [dbo].[trade_group_cmdty_mkt] ADD CONSTRAINT [CK__trade_gro__trade__3DF31CAF] CHECK (([trade_exclusion_ind]='N' OR [trade_exclusion_ind]='Y'))
GO
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'trade_group_cmdty_mkt', NULL, NULL
GO
