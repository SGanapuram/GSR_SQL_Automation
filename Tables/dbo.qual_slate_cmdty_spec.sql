...
[mandatory_ind] [bit] NOT NULL CONSTRAINT [df_qual_slate_cmdty_spec_mandatory_ind] DEFAULT ((0)),
...
begin
   set @errmsg = '(qual_slate_cmdty_spec) Failed to obtain a valid responsible trans_id.'
   if exists (select 1
              from master.dbo.sysprocesses (nolock)
              where spid = @@spid and
...
begin
   raiserror ('(qual_slate_cmdty_spec) The change needs to be attached with a new trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      set @errmsg = '(qual_slate_cmdty_spec) New trans_id must be larger than original trans_id.'
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.oid = d.oid)
begin
   set @errmsg = '(qual_slate_cmdty_spec) new trans_id must not be older than current trans_id.'   
   if @num_rows = 1 
   begin
...
begin
   ...
   begin
      raiserror ('(qual_slate_cmdty_spec) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
