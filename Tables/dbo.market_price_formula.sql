...
[status] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [DF__market_pr__statu__5792F321] DEFAULT ('A'),
...
begin
   raiserror ('(market_price_formula) The change needs to be attached with a new trans_id',10,1)
...
begin
   raiserror ('(market_price_formula) new trans_id must not be older than current trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(market_price_formula) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[market_price_formula] ADD CONSTRAINT [CK__market_pr__statu__5887175A] CHECK (([status]='I' OR [status]='A'))
GO
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'market_price_formula', NULL, NULL
GO
