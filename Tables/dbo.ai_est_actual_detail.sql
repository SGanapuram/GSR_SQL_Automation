...
begin
	raiserror ('(ai_est_actual_detail) The change needs to be attached with a new trans_id.', 16, 1)
	if @@trancount > 0 rollback tran  
   return
end
...
begin
   raiserror ('(ai_est_actual_detail) new trans_id must not be older than current trans_id.', 16, 1)
    if @@trancount > 0 rollback tran  
   return
end
...
begin
   ...
   begin
      raiserror ('(ai_est_actual_detail) primary key can not be changed.', 16, 1)
      if @@trancount > 0 rollback tran 
      return
   end
end
