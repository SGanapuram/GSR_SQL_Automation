...
begin
   raiserror ('(cost_scheduled_price) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(cost_scheduled_price) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(cost_scheduled_price) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[cost_scheduled_price] ADD CONSTRAINT [chk_cost_scheduled_price_mini_use_incl_ind] CHECK (([mini_use_incl_ind]='N' OR [mini_use_incl_ind]='Y'))
GO
ALTER TABLE [dbo].[cost_scheduled_price] ADD CONSTRAINT [chk_cost_scheduled_price_mini_usg_test_ind] CHECK (([mini_usg_test_ind]='N' OR [mini_usg_test_ind]='Y'))
GO
ALTER TABLE [dbo].[cost_scheduled_price] ADD CONSTRAINT [chk_cost_scheduled_price_volume_scale] CHECK (([volume_scale]='S' OR [volume_scale]='W'))
GO
