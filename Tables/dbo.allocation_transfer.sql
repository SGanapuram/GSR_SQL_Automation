...
begin    
   raiserror ('(allocation_transfer) The change needs to be attached with a new trans_id',16,1)    
   rollback tran    
   return    
end    
...
if exists (select * from inserted i, deleted d    
           where i.trans_id < d.trans_id and    
                 d.source_alloc_num = i.source_alloc_num and  
             d.source_alloc_item_num = i.source_alloc_item_num and   
      d.target_alloc_num = i.target_alloc_num and  
      d.target_alloc_item_num = i.target_alloc_item_num)    
begin    
   raiserror ('(allocation_transfer) new trans_id must not be older than current trans_id.',16,1)    
   rollback tran    
   return    
end    
...
if update(source_alloc_num) or   
   update(source_alloc_item_num) or   
   update(target_alloc_num) or   
   update(target_alloc_item_num)   
begin    
   select @count_num_rows = (select count(*) from inserted i, deleted d    
                             where d.source_alloc_num = i.source_alloc_num and  
                                   d.source_alloc_item_num = i.source_alloc_item_num and   
                                   d.target_alloc_num = i.target_alloc_num and  
                                  d.target_alloc_item_num = i.target_alloc_item_num)    
   ...
   begin    
      raiserror ('(allocation_transfer) primary key can not be changed.',16,1)    
      rollback tran    
      return    
   end    
end    
...
return
GO
ALTER TABLE [dbo].[allocation_transfer] ADD CONSTRAINT [allocation_transfer_pk] PRIMARY KEY CLUSTERED  ([source_alloc_num], [source_alloc_item_num], [target_alloc_num], [target_alloc_item_num]) ON [PRIMARY]
GO
