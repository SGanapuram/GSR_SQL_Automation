...
create trigger [dbo].[trade_item_exch_opt_deltrg]
on [dbo].[trade_item_exch_opt]
for delete
as
...
begin
   select @errmsg = '(trade_item_exch_opt) Failed to obtain a valid responsible trans_id.'
   if exists (select 1
              from master.dbo.sysprocesses (nolock)
              where spid = @@spid and
...
insert dbo.aud_trade_item_exch_opt
   (trade_num,
    order_num,
    item_num,
    put_call_ind,
...
declare @the_sequence       numeric(32, 0),
   ...
   select @the_entity_name = 'TradeItemExchOpt'
...
create trigger [dbo].[trade_item_exch_opt_instrg]
on [dbo].[trade_item_exch_opt]
for insert
as
declare @num_rows       int,
...
create trigger [dbo].[trade_item_exch_opt_updtrg]
on [dbo].[trade_item_exch_opt]
for update
as
declare @num_rows         int,
...
begin
   raiserror ('(trade_item_exch_opt) The change needs to be attached with a new trans_id',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      select @errmsg = '(trade_item_exch_opt) New trans_id must be larger than original trans_id.'
...
begin
   raiserror ('(trade_item_exch_opt) new trans_id must not be older than current trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(trade_item_exch_opt) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'trade_item_exch_opt', NULL, NULL
GO
