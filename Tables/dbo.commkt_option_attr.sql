...
[margin_type] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [df_commkt_option_attr_margin_type] DEFAULT ('P'),
...
begin
   raiserror ('(commkt_option_attr) The change needs to be attached with a new trans_id.',16,1)
   rollback tran
   return
end
...
begin
   raiserror ('(commkt_option_attr) new trans_id must not be older than current trans_id.',16,1)
   rollback tran
   return
end
...
begin
   ...
   begin
      raiserror ('(commkt_option_attr) primary key can not be changed.',16,1)
      rollback tran
      return
   end
end
...
return
GO
ALTER TABLE [dbo].[commkt_option_attr] ADD CONSTRAINT [chk_commkt_option_attr_margin_type] CHECK (([margin_type]='F' OR [margin_type]='P'))
GO
...
GO
ALTER TABLE [dbo].[commkt_option_attr] ADD CONSTRAINT [commkt_option_attr_fk8] FOREIGN KEY ([calendar_code]) REFERENCES [dbo].[calendar] ([calendar_code])
GO
ALTER TABLE [dbo].[commkt_option_attr] ADD CONSTRAINT [commkt_option_attr_fk9] FOREIGN KEY ([dst_zone]) REFERENCES [dbo].[time_zone] ([time_zone_code])
GO
