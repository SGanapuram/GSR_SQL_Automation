...
[status] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [df_product_status] DEFAULT ('N'),
...
begin
   raiserror('(product) The change needs to be attached with a new trans_id.', 16, 1)
   rollback tran
   return
end
...
begin
   ...
   begin
      raiserror('(product) primary key can not be changed.', 16, 1)
      rollback tran
      return
   end
end
...
return
GO
ALTER TABLE [dbo].[product] ADD CONSTRAINT [chk_product_status] CHECK (([status]='I' OR [status]='A' OR [status]='N'))
GO
