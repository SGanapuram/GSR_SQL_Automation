...
[us_citizen_ind] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [DF__icts_user__us_ci__2AF556D4] DEFAULT ('Y'),
...
--                    from user_job_title))
-- begin
--   raiserror ('An invalid user_job_title was used, please look up the user_job_title table for a valid job title.',10,1)
--   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(icts_user) The change needs to be attached with a new trans_id',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(icts_user) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[icts_user] ADD CONSTRAINT [CK__icts_user__us_ci__2BE97B0D] CHECK (([us_citizen_ind]='N' OR [us_citizen_ind]='Y'))
GO
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'icts_user', NULL, NULL
GO
