...
begin
   raiserror ('(mca_mat_adv_chg_clause) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(mca_mat_adv_chg_clause) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(mca_mat_adv_chg_clause) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
