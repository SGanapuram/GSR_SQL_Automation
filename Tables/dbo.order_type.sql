...
begin
   raiserror ('(order_type) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(order_type) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(order_type) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
...
GO
GRANT DELETE ON  [dbo].[order_type] TO [next_usr]
GO
GRANT INSERT ON  [dbo].[order_type] TO [next_usr]
GO
GRANT SELECT ON  [dbo].[order_type] TO [next_usr]
GO
GRANT UPDATE ON  [dbo].[order_type] TO [next_usr]
GO
