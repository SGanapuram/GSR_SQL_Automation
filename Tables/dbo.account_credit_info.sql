...
[use_dflt_cr_info] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NULL CONSTRAINT [DF__account_c__use_d__22AA2996] DEFAULT ('N'),
...
begin
   raiserror ('(account_credit_info) The change needs to be attached with a new trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(account_credit_info) new trans_id must not be older than current trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(account_credit_info) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[account_credit_info] ADD CONSTRAINT [CK__account_c__use_d__239E4DCF] CHECK (([use_dflt_cr_info]='N' OR [use_dflt_cr_info]='Y'))
GO
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'account_credit_info', NULL, NULL
GO
