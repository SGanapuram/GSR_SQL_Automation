...
begin
   raiserror ('(collateral_type) The change needs to be attached with a new trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(collateral_type) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
...
GO
GRANT DELETE ON  [dbo].[collateral_type] TO [next_usr]
GO
GRANT INSERT ON  [dbo].[collateral_type] TO [next_usr]
GO
GRANT SELECT ON  [dbo].[collateral_type] TO [next_usr]
GO
GRANT UPDATE ON  [dbo].[collateral_type] TO [next_usr]
GO
