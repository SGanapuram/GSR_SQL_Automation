...
begin
   raiserror ('(icts_function) The func_nums between 20000 and 30000 are reserved for PORTFOLIO TAG.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(icts_function) The app_name does not exist in the application table.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(icts_function) The change needs to be attached with a new trans_id',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(icts_function) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'icts_function', NULL, NULL
GO
