...
[priced_ind] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [DF__market_pr__price__5B638405] DEFAULT ('N'),
[end_of_period_ind] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [DF__market_pr__end_o__5C57A83E] DEFAULT ('N'),
...
begin
   raiserror ('(market_price_quote_dates) The change needs to be attached with a new trans_id',10,1)
   if @@trancount > 0 rollback tran
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.cmf_num = d.cmf_num)
begin
   raiserror ('(market_price_quote_dates) new trans_id must not be older than current trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(market_price_quote_dates) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'market_price_quote_dates', NULL, NULL
GO
