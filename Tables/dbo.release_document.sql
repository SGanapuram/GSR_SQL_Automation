...
[release_printed_ind] [bit] NOT NULL CONSTRAINT [df_release_document_release_printed_ind] DEFAULT ((0)),
...
begin
   raiserror ('(release_document) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(release_document) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
