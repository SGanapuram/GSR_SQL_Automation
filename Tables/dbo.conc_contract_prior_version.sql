...
create trigger [dbo].[conc_contract_prior_version_deltrg]
on [dbo].[conc_contract_prior_version]
for delete
as
...
begin
   set @errmsg = '(conc_contract_prior_version) Failed to obtain a valid responsible trans_id.'
   if exists (select 1
              from master.dbo.sysprocesses (nolock)
              where spid = @@spid and
...
insert dbo.aud_conc_contract_prior_version
   (
	oid,
	conc_contract_oid,
	custom_contract_num,
...
declare @the_sequence       numeric(32, 0),
   ...
   set @the_entity_name = 'ConcContractPriorVersion'
...
create trigger [dbo].[conc_contract_prior_version_instrg]
on [dbo].[conc_contract_prior_version]
for insert
as
declare @num_rows       int,
...
create trigger [dbo].[conc_contract_prior_version_updtrg]
on [dbo].[conc_contract_prior_version]
for update
as
declare @num_rows         int,
...
begin
   raiserror ('(conc_contract_prior_version) The change needs to be attached with a new trans_id',16,1)
   rollback tran
   return
end
...
begin
   ...
   begin
      set @errmsg = '(conc_contract_prior_version) New trans_id must be larger than original trans_id.'
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.oid = d.oid )
begin
   raiserror ('(conc_contract_prior_version) new trans_id must not be older than current trans_id.',16,1)
   rollback tran
   return
end
...
begin
   ...
   begin
      raiserror ('(conc_contract_prior_version) primary key can not be changed.',16,1)
      rollback tran
      return
   end
end
...
return
GO
ALTER TABLE [dbo].[conc_contract_prior_version] ADD CONSTRAINT [CK__conc_cont__p_s_i__00CBA98C] CHECK (([p_s_ind]='S' OR [p_s_ind]='P'))
GO
