...
begin
   raiserror ('(ag_truck_actual_details) The change needs to be attached with a new trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(ag_truck_actual_details) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[ag_truck_actual_details] ADD CONSTRAINT [CK__ag_truck___rejec__4EBF4CAC] CHECK (([rejected_load]=NULL OR [rejected_load]='N' OR [rejected_load]='Y'))
GO
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'ag_truck_actual_details', NULL, NULL
GO
