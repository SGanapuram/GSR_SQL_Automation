...
[mapping_type] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [df_external_mapping_mapping_type] DEFAULT ('T'),
...
begin
   raiserror ('(external_mapping) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.oid = d.oid)
begin
   raiserror ('(external_mapping) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(external_mapping) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[external_mapping] ADD CONSTRAINT [chk_external_mapping_mapping_type] CHECK (([mapping_type]='O' OR [mapping_type]='Q' OR [mapping_type]='U' OR [mapping_type]='S' OR [mapping_type]='P' OR [mapping_type]='K' OR [mapping_type]='B' OR [mapping_type]='M' OR [mapping_type]='N' OR [mapping_type]='C' OR [mapping_type]='T'))
GO
