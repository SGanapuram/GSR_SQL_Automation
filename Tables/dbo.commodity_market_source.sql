...
create trigger [dbo].[commodity_market_source_deltrg]
on [dbo].[commodity_market_source]
for delete
as
...
begin
   select @errmsg = '(commodity_market_source) Failed to obtain a valid responsible trans_id.'
   if exists (select 1
              from master.dbo.sysprocesses (nolock)
              where spid = @@spid and
...
insert dbo.aud_commodity_market_source
   (commkt_key,
    price_source_code,
...
select
   d.commkt_key,
   d.price_source_code,
   d.dflt_alias_source_code,
   d.calendar_code,
   d.tvm_use_ind,
   d.option_eval_use_ind,
   d.financial_borrow_use_ind,
   d.financial_lend_use_ind,
   d.quote_price_precision,
   d.trans_id,
   @atrans_id
from deleted d
...
declare @the_sequence       numeric(32, 0),
   ...
   select @the_entity_name = 'CommodityMarketSource'
...
create trigger [dbo].[commodity_market_source_instrg]
on [dbo].[commodity_market_source]
for insert
as
declare @num_rows        int,
...
create trigger [dbo].[commodity_market_source_updtrg]
on [dbo].[commodity_market_source]
for update
as
declare @num_rows         int,
...
begin
   raiserror ('(commodity_market_source) The change needs to be attached with a new trans_id',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      select @errmsg = '(commodity_market_source) New trans_id must be larger than original trans_id.'
...
begin
   raiserror ('(commodity_market_source) new trans_id must not be older than current trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(commodity_market_source) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'commodity_market_source', NULL, NULL
GO
