...
create trigger [dbo].[calendar_detail_deltrg]
on [dbo].[calendar_detail]
for delete
as
...
begin
   select @errmsg = '(calendar_detail) Failed to obtain a valid responsible trans_id.'
   if exists (select 1
              from master.dbo.sysprocesses (nolock)
              where spid = @@spid and
...
insert dbo.aud_calendar_detail
   (calendar_code,
    calendar_date,
    calendar_date_type,
    calendar_date_desc,
...
declare @the_sequence       numeric(32, 0),
   ...
   select @the_entity_name = 'CalendarDetail'
   ...
   begin
      ...
      select a.als_module_group_id,
             'D',
             @the_entity_name,
             convert(varchar(40), d.calendar_code),
...
create trigger [dbo].[calendar_detail_instrg]
on [dbo].[calendar_detail]
for insert
as
declare @num_rows        int,
...
declare @the_sequence       numeric(32, 0),
   ...
   begin
      ...
      select a.als_module_group_id,
             'I',
             @the_entity_name,
             convert(varchar(40), i.calendar_code),
...
create trigger [dbo].[calendar_detail_updtrg]
on [dbo].[calendar_detail]
for update
as
declare @num_rows         int,
...
begin
   raiserror ('(calendar_detail) The change needs to be attached with a new trans_id',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      select @errmsg = '(calendar_detail) New trans_id must be larger than original trans_id.'
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.calendar_code = d.calendar_code and 
                 i.calendar_date = d.calendar_date )
begin
   raiserror ('(calendar_detail) new trans_id must not be older than current trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
if update(calendar_code) or  
   update(calendar_date) 
begin
   select @count_num_rows = (select count(*) from inserted i, deleted d
                             where i.calendar_code = d.calendar_code and 
                                   i.calendar_date = d.calendar_date )
   ...
   begin
      raiserror ('(calendar_detail) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
if @dummy_update = 0
   ...
   where d.calendar_code = i.calendar_code and
         d.calendar_date = i.calendar_date 
...
declare @the_sequence       numeric(32, 0),
   ...
   begin
      ...
      select a.als_module_group_id,
             ...
             @the_entity_name,
             convert(varchar(40), i.calendar_code),
             convert(varchar(40), i.calendar_date, 100),
             null,
             null,
             null,
             null,
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'calendar_detail', NULL, NULL
GO
