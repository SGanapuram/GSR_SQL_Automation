...
begin
   raiserror ('(paper_allocation_item) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(paper_allocation_item) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(paper_allocation_item) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
