...
[default_path_ind] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [df_path_default_path_ind] DEFAULT ('N'),
...
begin
   raiserror ('(path) The change needs to be attached with a new trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(path) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[path] ADD CONSTRAINT [chk_path_default_path_ind] CHECK (([default_path_ind]='N' OR [default_path_ind]='Y'))
GO
