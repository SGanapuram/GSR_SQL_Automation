...
begin
   raiserror ('(entity_tag) The change needs to be attached with a new trans_id',10,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(entity_tag) new trans_id must not be older than current trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(entity_tag) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      ...
      raiserror ('(entity_tag) The <tag_name, key1> for the entity ''Portfolio'' can not be changed.',10,1)
      return
   end                    
...
GO
CREATE NONCLUSTERED INDEX [entity_tag_idx3] ON [dbo].[entity_tag] ([entity_tag_id], [key1], [key2], [key3]) ON [PRIMARY]
GO
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'entity_tag', NULL, NULL
GO
