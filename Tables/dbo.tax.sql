...
begin
   raiserror ('(tax) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.tax_num = d.tax_num )
begin
   raiserror ('(tax) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(tax) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
