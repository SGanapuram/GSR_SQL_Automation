...
[port_locked] [smallint] NULL CONSTRAINT [DF__portfolio__port___697C9932] DEFAULT ((0)),
...
begin
   raiserror ('(portfolio) The change needs to be attached with a new trans_id',10,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(portfolio) new trans_id must not be older than current trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(portfolio) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'portfolio', NULL, NULL
GO
