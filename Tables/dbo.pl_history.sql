...
GO
ALTER TABLE [dbo].[pl_history] ADD CONSTRAINT [pl_history_pk] PRIMARY KEY CLUSTERED  ([pl_asof_date], [pl_record_key], [pl_owner_code], [pl_type]) WITH (ALLOW_PAGE_LOCKS=OFF) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [pl_history_idx2] ON [dbo].[pl_history] ([pl_asof_date], [real_port_num], [pl_primary_owner_key1], [pl_primary_owner_key2], [pl_primary_owner_key3], [pl_primary_owner_key4]) WITH (FILLFACTOR=90, ALLOW_PAGE_LOCKS=OFF) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [pl_history_idx8] ON [dbo].[pl_history] ([pl_record_key], [pl_asof_date], [pl_owner_code], [pl_type]) WITH (FILLFACTOR=90, ALLOW_PAGE_LOCKS=OFF) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [pl_history_idx6] ON [dbo].[pl_history] ([real_port_num], [pl_asof_date], [pos_num], [pl_type], [pl_owner_code], [pl_amt]) WITH (FILLFACTOR=90, ALLOW_PAGE_LOCKS=OFF) ON [PRIMARY]
GO
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'pl_history', NULL, NULL
GO
