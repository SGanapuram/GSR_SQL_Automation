...
[job_status] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [DF__job_sched__job_s__57C7FD4B] DEFAULT ('A'),
[recur_ind] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [DF__job_sched__recur__59B045BD] DEFAULT ('N'),
...
create trigger [dbo].[job_schedule_deltrg]
on [dbo].[job_schedule]
for delete
as
...
begin
   select @errmsg = '(job_schedule) Failed to obtain a valid responsible trans_id.'
   if exists (select 1
              from master.dbo.sysprocesses (nolock)
              where spid = @@spid and
...
select
   ...
   d.job_status,
   d.recur_ind,
   d.trigger_event_code,
   d.trigger_event_status_list,
   d.drop_dead_date,
   d.trans_id,
   @atrans_id
from deleted d
...
declare @the_sequence       numeric(32, 0),
   ...
   select @the_entity_name = 'JobSchedule'
   ...
   begin
      ...
      select a.als_module_group_id,
             'D',
             @the_entity_name,
             convert(varchar(40), d.job_schedule_num),
             null,
             null,
             null,
             null,
...
create trigger [dbo].[job_schedule_instrg]
on [dbo].[job_schedule]
for insert
as
declare @num_rows        int,
...
declare @the_sequence       numeric(32, 0),
   ...
   begin
      ...
      select a.als_module_group_id,
             'I',
             @the_entity_name,
             convert(varchar(40), i.job_schedule_num),
             null,
             null,
             null,
             null,
...
create trigger [dbo].[job_schedule_updtrg]
on [dbo].[job_schedule]
for update
as
declare @num_rows         int,
...
begin
   raiserror ('(job_schedule) The change needs to be attached with a new trans_id',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      select @errmsg = '(job_schedule) New trans_id must be larger than original trans_id.'
...
begin
   raiserror ('(job_schedule) new trans_id must not be older than current trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(job_schedule) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
if @dummy_update = 0
   ...
   where d.job_schedule_num = i.job_schedule_num
...
return
GO
ALTER TABLE [dbo].[job_schedule] ADD CONSTRAINT [CK__job_sched__job_s__58BC2184] CHECK (([job_status]='X' OR [job_status]='C' OR [job_status]='A'))
GO
ALTER TABLE [dbo].[job_schedule] ADD CONSTRAINT [CK__job_sched__recur__5AA469F6] CHECK (([recur_ind]='N' OR [recur_ind]='Y'))
GO
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'job_schedule', NULL, NULL
GO
