...
begin  
   raiserror ('(broker_cost_autogen) The change needs to be attached with a new trans_id'  ,16,1)  
   if @@trancount > 0 rollback tran   
   return  
end  
...
begin  
   raiserror ('(broker_cost_autogen) new trans_id must not be older than current trans_id.'  ,16,1)  
   if @@trancount > 0 rollback tran   
   return  
end  
...
return  
GO
ALTER TABLE [dbo].[broker_cost_autogen] ADD CONSTRAINT [chk_broker_cost_autogen_block_trade_ind] CHECK (([block_trade_ind]=NULL OR [block_trade_ind]='Y' OR [block_trade_ind]='N'))
GO
ALTER TABLE [dbo].[broker_cost_autogen] ADD CONSTRAINT [chk_broker_cost_autogen_cost_eff_date_ind] CHECK (([cost_eff_date_ind]='E' OR [cost_eff_date_ind]='T' OR [cost_eff_date_ind]='F'))
GO
ALTER TABLE [dbo].[broker_cost_autogen] ADD CONSTRAINT [chk_broker_cost_autogen_cost_price_type] CHECK (([cost_price_type]='S' OR [cost_price_type]='F'))
GO
ALTER TABLE [dbo].[broker_cost_autogen] ADD CONSTRAINT [chk_broker_cost_autogen_creation_type] CHECK (([creation_type]='O' OR [creation_type]='M' OR [creation_type]='D' OR [creation_type]='T'))
GO
