...
[trade_modified_ind] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NULL CONSTRAINT [DF__tid_mark___trade__1E7A7156] DEFAULT ('N'),
[commkt_key] [int] NULL,
[pos_num] [int] NULL,
...
GO
ALTER TABLE [dbo].[tid_mark_to_market] ADD CONSTRAINT [tid_mark_to_market_pk] PRIMARY KEY CLUSTERED  ([dist_num], [mtm_pl_asof_date]) WITH (FILLFACTOR=90, ALLOW_PAGE_LOCKS=OFF) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [tid_mark_to_market_idx1] ON [dbo].[tid_mark_to_market] ([mtm_pl_asof_date], [trade_num]) WITH (FILLFACTOR=90, ALLOW_PAGE_LOCKS=OFF) ON [PRIMARY]
GO
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'tid_mark_to_market', NULL, NULL
GO
