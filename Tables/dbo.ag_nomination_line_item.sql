...
begin
   raiserror ('(ag_nomination_line_item) The change needs to be attached with a new trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(ag_nomination_line_item) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
