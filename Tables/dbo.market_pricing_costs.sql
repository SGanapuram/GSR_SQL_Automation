...
[cost_curr_code] [char] (8) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [df_market_pricing_costs_cost_curr_code] DEFAULT ('USD'),
[cost_uom_code] [char] (4) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [df_market_pricing_costs_cost_uom_code] DEFAULT ('BBL'),
...
begin
   raiserror ('(market_pricing_costs) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(market_pricing_costs) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(market_pricing_costs) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
