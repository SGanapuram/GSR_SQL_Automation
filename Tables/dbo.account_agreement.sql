...
[product_type] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [df_account_agreement_product_type] DEFAULT ('F'),
...
[forward_netting_ind] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [df_account_agreement_forward_netting_ind] DEFAULT ('N'),
[voucher_netting_ind] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [df_account_agreement_voucher_netting_ind] DEFAULT ('N'),
...
begin
   raiserror ('(account_agreement) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(account_agreement) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(account_agreement) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[account_agreement] ADD CONSTRAINT [chk_account_agreement_forward_netting_ind] CHECK (([forward_netting_ind]='N' OR [forward_netting_ind]='Y'))
GO
ALTER TABLE [dbo].[account_agreement] ADD CONSTRAINT [chk_account_agreement_product_type] CHECK (([product_type]='E' OR [product_type]='T' OR [product_type]='S' OR [product_type]='P' OR [product_type]='F'))
GO
ALTER TABLE [dbo].[account_agreement] ADD CONSTRAINT [chk_account_agreement_voucher_netting_ind] CHECK (([voucher_netting_ind]='N' OR [voucher_netting_ind]='Y'))
GO
