...
begin
   raiserror ('(delivery_term_alias) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(delivery_term_alias) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(delivery_term_alias) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
