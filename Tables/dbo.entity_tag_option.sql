...
[tag_option_status] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [DF__entity_ta__tag_o__1FEDB87C] DEFAULT ('A'),
...
begin
   raiserror ('(entity_tag_option) The change needs to be attached with a new trans_id',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(entity_tag_option) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[entity_tag_option] ADD CONSTRAINT [CK__entity_ta__tag_o__20E1DCB5] CHECK (([tag_option_status]='I' OR [tag_option_status]='A'))
GO
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'entity_tag_option', NULL, NULL
GO
