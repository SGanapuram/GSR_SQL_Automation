...
[operation_type_mask] [int] NULL CONSTRAINT [DF__als_modul__opera__3B40CD36] DEFAULT ((0))
) ON [PRIMARY]
GO
...
begin
   raiserror ('(als_module_entity) The change needs to be attached with a new trans_id',10,1)
...
begin
   raiserror ('(als_module_entity) new trans_id must not be older than current trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(als_module_entity) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
GO
GRANT DELETE ON  [dbo].[als_module_entity] TO [admin_group]
GO
GRANT INSERT ON  [dbo].[als_module_entity] TO [admin_group]
GO
GRANT SELECT ON  [dbo].[als_module_entity] TO [admin_group]
GO
GRANT UPDATE ON  [dbo].[als_module_entity] TO [admin_group]
GO
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'als_module_entity', NULL, NULL
GO
