...
[mul_div_ind] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [DF__fx_hedge___mul_d__5F9E293D] DEFAULT ('M'),
...
create trigger [dbo].[fx_hedge_rate_deltrg]
on [dbo].[fx_hedge_rate]
for delete
as
...
begin
   select @errmsg = '(fx_hedge_rate) Failed to obtain a valid responsible trans_id.'
   if exists (select 1
              from master.dbo.sysprocesses (nolock)
              where spid = @@spid and
...
declare @the_sequence       numeric(32, 0),
   ...
   select @the_entity_name = 'FxHedgeRate'
   ...
   begin
      ...
      select a.als_module_group_id,
             'D',
             @the_entity_name,
             convert(varchar(40), d.fx_hedge_rate_num),
             null,
             null,
             null,
             null,
...
create trigger [dbo].[fx_hedge_rate_instrg]
on [dbo].[fx_hedge_rate]
for insert
as
declare @num_rows       int,
...
declare @the_sequence       numeric(32, 0),
   ...
   begin
      ...
      select a.als_module_group_id,
             'I',
             @the_entity_name,
             convert(varchar(40), i.fx_hedge_rate_num),
             null,
             null,
             null,
             null,
...
create trigger [dbo].[fx_hedge_rate_updtrg]
on [dbo].[fx_hedge_rate]
for update
as
declare @num_rows       int,
...
begin
   raiserror ('(fx_hedge_rate) The change needs to be attached with a new trans_id',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      select @errmsg = '(fx_hedge_rate) New trans_id must be larger than original trans_id.'
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.fx_hedge_rate_num = d.fx_hedge_rate_num) 
begin
   raiserror ('(fx_hedge_rate) new trans_id must not be older than current trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
if update(fx_hedge_rate_num)
begin
   select @count_num_rows = (select count(*) from inserted i, deleted d
                             where i.fx_hedge_rate_num = d.fx_hedge_rate_num) 
   ...
   begin
      raiserror ('(fx_hedge_rate) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
if @dummy_update = 0
   ...
   where d.fx_hedge_rate_num = i.fx_hedge_rate_num

/* AUDIT_CODE_END */

return
GO
ALTER TABLE [dbo].[fx_hedge_rate] ADD CONSTRAINT [CK__fx_hedge___mul_d__60924D76] CHECK (([mul_div_ind]='D' OR [mul_div_ind]='M'))
GO
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'fx_hedge_rate', NULL, NULL
GO
