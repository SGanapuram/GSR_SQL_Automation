...
create trigger [dbo].[contract_amendable_field_deltrg]                                                                                                              
on [dbo].[contract_amendable_field]                                                                                                                              
for delete                                                                                                                                       
as                                                                                                                                               
...
insert dbo.aud_contract_amendable_field
(                                                                                                                  
   oid,						
   entity_id,				  
   entity_field,			  
...
declare @the_sequence       numeric(32, 0),                                                                                                      
   ...
   set @the_entity_name = 'ContractAmendableField'                                                                                                   
...
GO
 ...
 create trigger [dbo].[contract_amendable_field_instrg]
 on [dbo].[contract_amendable_field]
 for insert
 as
 declare @num_rows       int,
...
create trigger [dbo].[contract_amendable_field_updtrg]
on [dbo].[contract_amendable_field]
for update
as
declare @num_rows         int,
...
begin
   raiserror ('contract_amendable_field) The change needs to be attached with a new trans_id',16,1)
   rollback tran
   return
end
...
begin
   ...
   begin
      raiserror ('(contract_amendable_field) New trans_id must be larger than original trans_id.You can use the the gen_new_transaction procedure to obtain a new trans_id.',16,1)
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.oid = d.oid)
begin
   raiserror ('(contract_amendable_field) new trans_id must not be older than current trans_id.',16,1)
   rollback tran
   return
end
...
begin
   ...
   begin
      raiserror ('contract_amendable_field: primary key can not be changed.',16,1)
      rollback tran
      return
   end
end
