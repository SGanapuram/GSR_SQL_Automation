...
begin
   raiserror ('(cash_forecast_file) The change needs to be attached with a new trans_id',10,1)
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.cff_num = d.cff_num )
begin
   raiserror ('(cash_forecast_file) new trans_id must not be older than current trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(cash_forecast_file) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
GO
ALTER TABLE [dbo].[cash_forecast_file] ADD CONSTRAINT [cash_forecast_file_fk2] FOREIGN KEY ([cff_trans_id]) REFERENCES [dbo].[icts_user] ([user_init])
GO
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'cash_forecast_file', NULL, NULL
GO
