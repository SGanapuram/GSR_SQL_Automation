...
[spec_type] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NULL CONSTRAINT [df_specification_spec_type] DEFAULT ('N'),
...
insert dbo.aud_specification
   (spec_code,
    ...
    spec_val_uom_code,
	  is_ti_val_only,
	  per_spec_val_uom_code,
	  spec_uom_ratio_factor)
select
   d.spec_code,
   d.spec_desc,
   ...
   d.spec_val_uom_code,
   d.is_ti_val_only,
   d.per_spec_val_uom_code,
   d.spec_uom_ratio_factor
from deleted d
...
begin
   raiserror ('(specification) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(specification) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(specification) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[specification] ADD CONSTRAINT [chk_specification_spec_type] CHECK (([spec_type]='S' OR [spec_type]='N' OR [spec_type]='A'))
GO
