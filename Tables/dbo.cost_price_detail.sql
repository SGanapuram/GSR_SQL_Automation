...
begin  
   set @errmsg = '(cost_price_detail) Failed to obtain a valid responsible trans_id.'  
   if exists (select 1  
              from master.dbo.sysprocesses (nolock)  
              where spid = @@spid and  
...
begin  
   raiserror('(cost_price_detail) The change needs to be attached with a new trans_id', 16, 1)
   rollback tran  
   return  
end  
...
begin  
   ...
   begin  
      set @errmsg = '(cost_price_detail) New trans_id must be larger than original trans_id.'  
...
begin  
   raiserror('(cost_price_detail) new trans_id must not be older than current trans_id.', 16, 1)
   rollback tran  
   return  
end  
...
begin  
   ...
   begin  
      raiserror('(cost_price_detail) primary key can not be changed.', 16, 1)
      rollback tran  
      return  
   end  
end  
