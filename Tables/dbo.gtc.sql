...
[creation_date] [datetime] NOT NULL CONSTRAINT [DF__gtc__creation_da__07AC1A97] DEFAULT (getdate()),
...
begin
   raiserror ('(gtc) The change needs to be attached with a new trans_id',10,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(gtc) new trans_id must not be older than current trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(gtc) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'gtc', NULL, NULL
GO
