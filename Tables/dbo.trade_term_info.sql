...
begin
   raiserror ('(trade_term_info) The change needs to be attached with a new trans_id',10,1)
   rollback tran
   return
end
...
begin
   raiserror ('(trade_term_info) new trans_id must not be older than current trans_id.',10,1)
   rollback tran
   return
end
...
begin
   ...
   begin
      raiserror  ('(trade_term_info) primary key can not be changed.',10,1)
      rollback tran
      return
   end
end
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'trade_term_info', NULL, NULL
GO
