...
[standard_ind] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [DF__commodity__stand__61316BF4] DEFAULT ('Y')
) ON [PRIMARY]
GO
...
begin
   select @errmsg = '(commodity_specification) Failed to obtain a valid responsible trans_id.'
   if exists (select 1
              from master.dbo.sysprocesses (nolock)
              where spid = @@spid and
...
begin
   raiserror ('(commodity_specification) The change needs to be attached with a new trans_id',10,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(commodity_specification) new trans_id must not be older than current trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(commodity_specification) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[commodity_specification] ADD CONSTRAINT [CK__commodity__stand__6225902D] CHECK (([standard_ind]='N' OR [standard_ind]='Y'))
GO
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'commodity_specification', NULL, NULL
GO
