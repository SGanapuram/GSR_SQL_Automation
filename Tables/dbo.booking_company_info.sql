...
[pur_grp_mandatory_ind] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [DF__booking_c__pur_g__6442E2C9] DEFAULT ('N'),
...
begin
   raiserror ('(booking_company_info) The change needs to be attached with a new trans_id',10,1)
...
begin
   raiserror ('(booking_company_info) new trans_id must not be older than current trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(booking_company_info) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[booking_company_info] ADD CONSTRAINT [CK__booking_c__pur_g__65370702] CHECK (([pur_grp_mandatory_ind]='N' OR [pur_grp_mandatory_ind]='Y'))
GO
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'booking_company_info', NULL, NULL
GO
