...
begin
   raiserror ('(gdd_attribute_info) The change needs to be attached with a new trans_id',16,1)
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.gdd_num = d.gdd_num )
begin
   raiserror ('(gdd_attribute_info) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(gdd_attribute_info) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
