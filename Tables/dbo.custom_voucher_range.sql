...
[last_num] [int] NOT NULL CONSTRAINT [DF__custom_vo__last___6C6E1476] DEFAULT ((0)),
[max_num] [int] NOT NULL CONSTRAINT [DF__custom_vo__max_n__6D6238AF] DEFAULT ((0)),
...
begin
   raiserror ('(custom_voucher_range) The change needs to be attached with a new trans_id',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(custom_voucher_range) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'custom_voucher_range', NULL, NULL
GO
