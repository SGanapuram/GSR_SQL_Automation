...
create trigger [dbo].[strategy_execution_detail_deltrg]                                                                                                              
on [dbo].[strategy_execution_detail]                                                                                                                              
for delete                                                                                                                                       
as                                                                                                                                               
...
insert dbo.aud_strategy_execution_detail
   (                                                                                                                  
    strategy_id,			
    strat_detail_num,	
    exec_id,				
...
declare @the_sequence       numeric(32, 0),                                                                                                      
   ...
   set @the_entity_name = 'StrategyExecutionDetail'                                                                                                   
   ...
   begin                                                                                                                                         
      ...
      select a.als_module_group_id,                                                                                                              
             'D',                                                                                                                              
             @the_entity_name,                                                                                                                   
			convert(varchar(40),strategy_id),
			convert(varchar(40),strat_detail_num),                                                                                                                               
             null,                                                                                                                               
             null,                                                                                                                               
             null,                                                                                                                               
             null,                                                                                                                               
...
create trigger [dbo].[strategy_execution_detail_instrg]
on [dbo].[strategy_execution_detail]
for insert
as
declare @num_rows        int,
...
create trigger [dbo].[strategy_execution_detail_updtrg]
on [dbo].[strategy_execution_detail]
for update
as
declare @num_rows         int,
...
begin
   raiserror ('strategy_execution_detail) The change needs to be attached with a new trans_id',16,1)
   rollback tran
   return
end
...
begin
   ...
   begin
      raiserror ('(strategy_execution_detail) New trans_id must be larger than original trans_id.You can use the the gen_new_transaction procedure to obtain a new trans_id.',16,1)
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.strategy_id = d.strategy_id and
				 i.strat_detail_num = d.strat_detail_num)
begin
   raiserror ('(strategy_execution_detail) new trans_id must not be older than current trans_id.',16,1)
   rollback tran
   return
end
...
if update(strategy_id) or
   update(strat_detail_num)
begin
   set @count_num_rows = (select count(*) from inserted i, deleted d
                          where i.strategy_id = d.strategy_id and
							 	i.strat_detail_num = d.strat_detail_num)
   if (@count_num_rows = @num_rows)
      set @dummy_update = 1
   else
   begin
      raiserror ('strategy_execution_detail: primary key can not be changed.',16,1)
      rollback tran
      return
   end
end
...
if @dummy_update = 0
   ...
   where d.strategy_id = i.strategy_id and 
         d.strat_detail_num = i.strat_detail_num
