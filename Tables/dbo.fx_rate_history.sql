...
begin
   raiserror ('(fx_rate_history) The change needs to be attached with a new trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(fx_rate_history) new trans_id must not be older than current trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(fx_rate_history) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[fx_rate_history] ADD CONSTRAINT [CK__fx_rate_h__rate___6C040022] CHECK (([rate_multi_div_ind]='D' OR [rate_multi_div_ind]='M'))
GO
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'fx_rate_history', NULL, NULL
GO
