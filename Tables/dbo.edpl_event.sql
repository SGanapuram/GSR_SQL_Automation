...
create trigger [dbo].[edpl_event_deltrg]
on [dbo].[edpl_event]
for delete
as
...
begin
   select @errmsg = '(edpl_event) Failed to obtain a valid responsible trans_id.'
   if exists (select 1
              from master.dbo.sysprocesses (nolock)
              where spid = @@spid and
...
declare @the_sequence       numeric(32, 0),  
   ...
   select @the_entity_name = 'EdplEvent'  
...
create trigger [dbo].[edpl_event_instrg]
on [dbo].[edpl_event]
for insert
as
declare @num_rows        int
...
create trigger [dbo].[edpl_event_updtrg]
on [dbo].[edpl_event]
for update
as
declare @num_rows         int,
...
begin
   raiserror ('(edpl_event) The change needs to be attached with a new trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      select @errmsg = '(edpl_event) New trans_id must be larger than original trans_id.'
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.oid = d.oid)
begin
   select @errmsg = '(edpl_event) new trans_id must not be older than current trans_id.'   
   if @num_rows = 1 
   begin
...
begin
   ...
   begin
      raiserror ('(edpl_event) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'edpl_event', NULL, NULL
GO
