...
create trigger [dbo].[conc_assay_lab_deltrg]
on [dbo].[conc_assay_lab]
for delete
as
...
begin
   set @errmsg = '(conc_assay_lab) Failed to obtain a valid responsible trans_id.'
   if exists (select 1
              from master.dbo.sysprocesses (nolock)
              where spid = @@spid and
...
declare @the_sequence       numeric(32, 0),
   ...
   set @the_entity_name = 'ConcAssayLab'
...
create trigger [dbo].[conc_assay_lab_instrg]
on [dbo].[conc_assay_lab]
for insert
as
declare @num_rows       int,
...
create trigger [dbo].[conc_assay_lab_updtrg]
on [dbo].[conc_assay_lab]
for update
as
declare @num_rows         int,
...
begin
   raiserror ('(conc_assay_lab) The change needs to be attached with a new trans_id',16,1)
   rollback tran
   return
end
...
begin
   ...
   begin
      set @errmsg = '(conc_assay_lab) New trans_id must be larger than original trans_id.'
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.oid = d.oid)
begin
   raiserror('(conc_assay_lab) new trans_id must not be older than current trans_id.',16,1)
   rollback tran
   return
end
...
begin
   ...
   begin
      raiserror ('(conc_assay_lab) primary key can not be changed.',16,1)
      rollback tran
      return
   end
end
...
create trigger [dbo].[conc_brand_instrg]
on [dbo].[conc_assay_lab]
for insert
as
declare @num_rows       int,
...
return
GO
ALTER TABLE [dbo].[conc_assay_lab] ADD CONSTRAINT [CK__conc_assa__final__11021155] CHECK (([final_binding_ind]='N' OR [final_binding_ind]='Y'))
GO
ALTER TABLE [dbo].[conc_assay_lab] ADD CONSTRAINT [CK__conc_assa__umpir__11F6358E] CHECK (([umpire_ind]='N' OR [umpire_ind]='Y'))
GO
