...
[use_ind] [bit] NOT NULL CONSTRAINT [df_reference_tab_list_use_ind] DEFAULT ((1))
) ON [PRIMARY]
GO
...
begin
   raiserror ('(reference_tab_list) The change needs to be attached with a new trans_id',16,1)
...
begin
   raiserror ('(reference_tab_list) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(reference_tab_list) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
