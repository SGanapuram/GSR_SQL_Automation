...
[fdv_id] [int] NOT NULL,
[fd_id] [int] NOT NULL,
[data_type] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [df_function_detail_value_data_type] DEFAULT ('S'),
...
begin
   raiserror ('(function_detail_value) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.fdv_id = d.fdv_id)
begin
   raiserror ('(function_detail_value) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(function_detail_value) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[function_detail_value] ADD CONSTRAINT [chk_function_detail_value_data_type] CHECK (([data_type]='S' OR [data_type]='F' OR [data_type]='D' OR [data_type]='I'))
GO
...
GO
GRANT DELETE ON  [dbo].[function_detail_value] TO [next_usr]
GO
GRANT INSERT ON  [dbo].[function_detail_value] TO [next_usr]
GO
GRANT SELECT ON  [dbo].[function_detail_value] TO [next_usr]
GO
GRANT UPDATE ON  [dbo].[function_detail_value] TO [next_usr]
GO
