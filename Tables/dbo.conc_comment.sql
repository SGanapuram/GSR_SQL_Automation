...
create trigger [dbo].[conc_assay_instrg]
on [dbo].[conc_comment]
for insert
as
...
create trigger [dbo].[conc_comment_deltrg]
on [dbo].[conc_comment]
for delete
as
declare @num_rows    int,
...
begin
   set @errmsg = '(conc_comment) Failed to obtain a valid responsible trans_id.'
   if exists (select 1
              from master.dbo.sysprocesses (nolock)
              where spid = @@spid and
...
select
	...
	d.cmnt_creator_init,
	d.cmnt_mod_init,
	d.cmnt_creation_date,
	d.cmnt_last_mod_date,
	d.trans_id,
    @atrans_id
from deleted d
...
declare @the_sequence       numeric(32, 0),
   ...
   set @the_entity_name = 'ConcComment'
...
create trigger [dbo].[conc_comment_instrg]
on [dbo].[conc_comment]
for insert
as
declare @num_rows       int,
...
create trigger [dbo].[conc_comment_updtrg]
on [dbo].[conc_comment]
for update
as
declare @num_rows         int,
...
begin
   raiserror ('(conc_comment) The change needs to be attached with a new trans_id',16,1)
   rollback tran
   return
end
...
begin
   ...
   begin
      set @errmsg = '(conc_comment) New trans_id must be larger than original trans_id.'
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.oid = d.oid)
begin
   raiserror ('(conc_comment) new trans_id must not be older than current trans_id.',16,1)
   rollback tran
   return
end
...
begin
   ...
   begin
      raiserror ('(conc_comment) primary key can not be changed.',16,1)
      rollback tran
      return
   end
end
...
create trigger [dbo].[conc_del_term_instrg]
on [dbo].[conc_comment]
for insert
as
declare @num_rows       int,
...
create trigger [dbo].[conc_document_instrg]
on [dbo].[conc_comment]
for insert
as
declare @num_rows       int,
...
declare @the_sequence       numeric(32, 0),
   ...
   set @the_entity_name = 'ConcDocument'
