...
[acct_addr_status] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [DF__rg_stagin__acct___74B941B4] DEFAULT ('A'),
...
--begin
--   raiserror ('(rg_staging_acct_address) The change needs to be attached with a new trans_id.',10,1)
...
begin
   ...
   begin
      raiserror ('(rg_staging_acct_address) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[rg_staging_acct_address] ADD CONSTRAINT [CK__rg_stagin__acct___75AD65ED] CHECK (([acct_addr_status]='I' OR [acct_addr_status]='A'))
GO
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'rg_staging_acct_address', NULL, NULL
GO
