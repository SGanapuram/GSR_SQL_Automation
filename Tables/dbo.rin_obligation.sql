...
[status] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [df_rin_obligation_status] DEFAULT ('A'),
...
begin
   raiserror ('(rin_obligation) The change needs to be attached with a new trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(rin_obligation) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[rin_obligation] ADD CONSTRAINT [chk_rin_obligation_status] CHECK (([status]='I' OR [status]='A'))
GO
