...
begin
   raiserror ('(fx_linked_costs) The change needs to be attached with a new trans_id',10,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(fx_linked_costs) new trans_id must not be older than current trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(fx_linked_costs) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[fx_linked_costs] ADD CONSTRAINT [CK__fx_linked__curr___636EBA21] CHECK (([curr_cost_ind]='N' OR [curr_cost_ind]='C'))
GO
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'fx_linked_costs', NULL, NULL
GO
