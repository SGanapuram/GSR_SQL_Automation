...
[man_input_sec_qty_required] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NULL CONSTRAINT [df_account_man_input_sec_qty_required] DEFAULT ('N'),
...
[allows_netout] [bit] NOT NULL CONSTRAINT [df_account_allows_netout] DEFAULT ((0)),
[allows_bookout] [bit] NOT NULL CONSTRAINT [df_account_allows_bookout] DEFAULT ((0)),
...
begin
   raiserror ('(account) The change needs to be attached with a new trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(account) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(account) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[account] ADD CONSTRAINT [chk_account_man_input_sec_qty_required] CHECK (([man_input_sec_qty_required]='N' OR [man_input_sec_qty_required]='Y'))
GO
