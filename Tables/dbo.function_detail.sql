...
[entity_ind] [bit] NOT NULL CONSTRAINT [df_function_detail_entity_ind] DEFAULT ((0)),
...
begin
   raiserror ('(function_detail) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.fd_id = d.fd_id)
begin
   raiserror ('(function_detail) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(function_detail) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
...
GO
GRANT DELETE ON  [dbo].[function_detail] TO [next_usr]
GO
GRANT INSERT ON  [dbo].[function_detail] TO [next_usr]
GO
GRANT SELECT ON  [dbo].[function_detail] TO [next_usr]
GO
GRANT UPDATE ON  [dbo].[function_detail] TO [next_usr]
GO
