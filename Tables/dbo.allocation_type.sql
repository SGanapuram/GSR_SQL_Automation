...
[own_movement_cost_ind] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [df_allocation_type_own_movement_cost_ind] DEFAULT ('N'),
...
begin
   raiserror ('(allocation_type) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(allocation_type) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(allocation_type) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[allocation_type] ADD CONSTRAINT [chk_allocation_type_own_movement_cost_ind] CHECK (([own_movement_cost_ind]='N' OR [own_movement_cost_ind]='Y'))
GO
...
GO
GRANT DELETE ON  [dbo].[allocation_type] TO [next_usr]
GO
GRANT INSERT ON  [dbo].[allocation_type] TO [next_usr]
GO
GRANT SELECT ON  [dbo].[allocation_type] TO [next_usr]
GO
GRANT UPDATE ON  [dbo].[allocation_type] TO [next_usr]
GO
