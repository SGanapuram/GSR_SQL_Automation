...
begin
   raiserror ('(loc_type_by_order) The change needs to be attached with a new trans_id',16,1)
...
begin
   raiserror ('(loc_type_by_order) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(loc_type_by_order) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
...
GO
GRANT DELETE ON  [dbo].[loc_type_by_order] TO [next_usr]
GO
GRANT INSERT ON  [dbo].[loc_type_by_order] TO [next_usr]
GO
GRANT SELECT ON  [dbo].[loc_type_by_order] TO [next_usr]
GO
GRANT UPDATE ON  [dbo].[loc_type_by_order] TO [next_usr]
GO
