...
[equiv_source_ind] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NULL CONSTRAINT [df_position_equiv_source_ind] DEFAULT ('N'),
...
from inserted	
   ...
   */            
   ...
   begin
      raiserror ('Combination of certain columns for the new position record(s) is not unique. Duplicate is not allowed!',16,1)
      rollback tran
      return
   end
...
begin
   raiserror ('(position) The change needs to be attached with a new trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.pos_num = d.pos_num )
begin
   raiserror ('(position) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(position) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
...
GO
CREATE NONCLUSTERED INDEX [position_idx6] ON [dbo].[position] ([pos_status]) INCLUDE ([acct_short_name], [avg_purch_price], [avg_sale_price], [cmdty_code], [commkt_key], [discount_qty], [equiv_source_ind], [formula_body_num], [formula_num], [is_equiv_ind], [is_hedge_ind], [long_qty], [mkt_code], [mkt_long_qty], [mkt_short_qty], [opt_exp_date], [opt_start_date], [option_type], [pos_num], [pos_type], [price_curr_code], [priced_qty], [put_call_ind], [qty_uom_code], [real_port_num], [sec_discount_qty], [sec_long_qty], [sec_mkt_long_qty], [sec_mkt_short_qty], [sec_pos_uom_code], [sec_priced_qty], [sec_short_qty], [settlement_type], [short_qty], [strike_price], [trading_prd], [trans_id]) ON [PRIMARY]
GO
