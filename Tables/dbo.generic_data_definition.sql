...
begin
   raiserror ('(generic_data_definition) The change needs to be attached with a new trans_id',10,1)
   if @@trancount > 0 rollback tran
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.gdd_num = d.gdd_num )
begin
   raiserror ('(generic_data_definition) new trans_id must not be older than current trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(generic_data_definition) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
