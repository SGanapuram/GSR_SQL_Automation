...
begin  
   raiserror ('(icts_timezones) The change needs to be attached with a new trans_id',16,1)  
   rollback tran  
   return  
end  
...
if exists (select * from inserted i, deleted d 
			     where i.trans_id < d.trans_id and 
			           i.oid = d.oid)  
begin  
   raiserror ('(icts_timezones) new trans_id must not be older than current trans_id.',16,1)  
   rollback tran  
   return  
end  
...
begin  
   ...
   begin  
      raiserror ('(icts_timezones) primary key can not be changed.',16,1)  
      rollback tran  
      return  
   end  
end  
