...
begin
   raiserror ('(commodity_rollup_type) The change needs to be attached with a new trans_id',16,1)
...
begin
   raiserror ('(commodity_rollup_type) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(commodity_rollup_type) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
