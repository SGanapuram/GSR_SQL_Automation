...
begin
   raiserror ('(ag_btt_ack_cnfrm_litem) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.fdd_oid = d.fdd_oid )
begin
   raiserror ('(ag_btt_ack_cnfrm_litem) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(ag_btt_ack_cnfrm_litem) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
