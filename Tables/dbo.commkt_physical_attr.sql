...
begin
   raiserror ('(commkt_physical_attr) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(commkt_physical_attr) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(commkt_physical_attr) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
...
GO
ALTER TABLE [dbo].[commkt_physical_attr] ADD CONSTRAINT [commkt_physical_attr_fk8] FOREIGN KEY ([calendar_code]) REFERENCES [dbo].[calendar] ([calendar_code])
GO
ALTER TABLE [dbo].[commkt_physical_attr] ADD CONSTRAINT [commkt_physical_attr_fk9] FOREIGN KEY ([dst_zone]) REFERENCES [dbo].[time_zone] ([time_zone_code])
GO
