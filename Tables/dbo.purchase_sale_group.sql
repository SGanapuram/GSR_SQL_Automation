...
[p_s_ind] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [df_purchase_sale_group_p_s_ind] DEFAULT ('P'),
...
begin
   raiserror ('(purchase_sale_group) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
if exists (select * from inserted i, deleted d
           where i.trans_id < d.trans_id and
                 i.oid = d.oid )
begin
   raiserror ('(purchase_sale_group) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(purchase_sale_group) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[purchase_sale_group] ADD CONSTRAINT [chk_purchase_sale_group_p_s_ind] CHECK (([p_s_ind]='S' OR [p_s_ind]='P'))
GO
