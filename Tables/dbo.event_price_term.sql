...
begin
   select @errmsg = '(event_price_term) Failed to obtain a valid responsible trans_id. '
   if exists (select 1
              from master.dbo.sysprocesses (nolock)
              where spid = @@spid and
...
begin
   raiserror ('(event_price_term) The change needs to be attached with a new trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      select @errmsg = '(event_price_term) New trans_id must be larger than original trans_id.'
...
begin
   select @errmsg = '(event_price_term) new trans_id must not be older than current trans_id.'   
   if @num_rows = 1 
   begin
...
begin
   ...
   begin
      raiserror ('(event_price_term) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'event_price_term', NULL, NULL
GO
