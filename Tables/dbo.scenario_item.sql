...
[buy_sell_ind] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [DF__scenario___buy_s__22800C64] DEFAULT ('B'),
...
begin
   raiserror ('(scenario_item) The change needs to be attached with a new trans_id.',10,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(scenario_item) primary key can not be changed.',10,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[scenario_item] ADD CONSTRAINT [CK__scenario___buy_s__2374309D] CHECK (([buy_sell_ind]='S' OR [buy_sell_ind]='B'))
GO
ALTER TABLE [dbo].[scenario_item] ADD CONSTRAINT [CK__scenario___qty_p__246854D6] CHECK (([qty_periodicity]='V' OR [qty_periodicity]='M' OR [qty_periodicity]='L' OR [qty_periodicity]='D'))
GO
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'TABLE', N'scenario_item', NULL, NULL
GO
