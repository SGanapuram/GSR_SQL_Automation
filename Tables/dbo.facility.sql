...
[stock_location_ind] [char] (1) COLLATE SQL_Latin1_General_CP1_CS_AS NOT NULL CONSTRAINT [df_facility_stock_location_ind] DEFAULT ('Y'),
...
begin
   raiserror ('(facility) The change needs to be attached with a new trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(facility) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
...
return
GO
ALTER TABLE [dbo].[facility] ADD CONSTRAINT [chk_facility_stock_location_ind] CHECK (([stock_location_ind]='N' OR [stock_location_ind]='Y'))
GO
