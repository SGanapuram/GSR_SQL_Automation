...
begin
   raiserror ('(execution_type) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(execution_type) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
