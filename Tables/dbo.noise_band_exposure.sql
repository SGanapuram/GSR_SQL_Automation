...
begin
   raiserror ('(noise_band_exposure) The change needs to be attached with a new trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(noise_band_exposure) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
