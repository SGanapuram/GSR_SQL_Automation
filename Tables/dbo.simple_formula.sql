...
begin
   raiserror ('(simple_formula) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(simple_formula) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(simple_formula) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
