...
begin
   raiserror ('(exposure_detail) The change needs to be attached with a new trans_id',16,1)
   if @@trancount > 0 rollback tran
...
begin
   raiserror ('(exposure_detail) new trans_id must not be older than current trans_id.',16,1)
   if @@trancount > 0 rollback tran
...
begin
   ...
   begin
      raiserror ('(exposure_detail) primary key can not be changed.',16,1)
      if @@trancount > 0 rollback tran
