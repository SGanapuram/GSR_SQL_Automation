...
--trade     
declare @T_inv_pricing_type      varchar(15)     
...
declare @converted_actual_date       datetime = null                    
   ...
   select @TI_trade_num = ti.trade_num,                    
       ...
       @T_inv_pricing_type = t.inv_pricing_type    
   ...
   /* Getting a new alloc_num */    
      ...
      exec get_new_num 'alloc_num',0   
   select @alloc_num = last_num from dbo.new_num where num_col_name = 'alloc_num'                       
 ...
 alloc_pl_status,       
         trans_id)                          
      values(@alloc_num,      
      (SELECT DISTINCT parent_port_num FROM portfolio_group WHERE port_num IN ( SELECT TOP 1 pos_num FROM trade_item_dist WHERE trade_num = @buy_trade_num  AND order_num = @buy_order_num AND item_num = @buy_item_num and dist_type='D' 
	  and real_synth_ind='R' AND is_equiv_ind IS NOT NULL AND is_equiv_ind != 'Y' ORDER BY dist_num )),      
 'I',       
      @trans_id)                          
     set @rows_affected = @@rowcount                         
   end try                    
...
@negotiated_date,                    
   ...
   /* Getting a new inv_num */    
    ...
    exec get_new_num 'inv_num',0   
   select @inv_num = last_num from dbo.new_num where num_col_name = 'inv_num'                     
...
@negotiated_date,                    
 ...
 owner_code,                          
   ...
   /* Getting a new inv_b_d_num */  
    ...
    exec get_new_num 'inv_b_d_num',0   
   select @inv_b_d_num = last_num from dbo.new_num where num_col_name = 'inv_b_d_num'                     
