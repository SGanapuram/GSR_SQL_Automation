...
CREATE procedure [dbo].[x_aud_lc_status_history]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set xact_abort on
declare @lc_num             int,
        @lc_status_code     char(8),
        @lc_status_date     datetime,
        @trans_id           int,
        @resp_trans_id      int,
        @cutoffdate         datetime,
        @rows_deleted       int,
   ...
   begin
      ...
      RAISERROR('aud_lc_status_history: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct lc_num, lc_status_code, lc_status_date
      from dbo.aud_lc_status_history
      order by lc_num, lc_status_code, lc_status_date
 
   open mycursor
   fetch next from mycursor into @lc_num, @lc_status_code, @lc_status_date
   ...
   begin
      ...
      from dbo.aud_lc_status_history aud
      where aud.lc_num = @lc_num and
            aud.lc_status_code = @lc_status_code and
            aud.lc_status_date = @lc_status_date and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@lc_num as varchar) + '/' + @lc_status_code + '/' + convert(varchar, @lc_status_date, 101)
      ...
      begin
         ...
         where aud.lc_num = @lc_num and
               aud.lc_status_code = @lc_status_code and
               aud.lc_status_date = @lc_status_date
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_lc_status_history records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_lc_status_history rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_lc_status_history rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_lc_status_history'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_lc_status_history record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_lc_status_history records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_lc_status_history] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_lc_status_history', NULL, NULL
GO
