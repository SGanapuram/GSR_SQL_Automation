...
CREATE procedure [dbo].[x_aud_release_document]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @release_doc_num    int,
   ...
   begin
      ...
      RAISERROR('aud_release_document: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct release_doc_num
      from dbo.aud_release_document
      order by release_doc_num
 
   open mycursor
   fetch next from mycursor into @release_doc_num
   ...
   begin
      ...
      from dbo.aud_release_document aud
      where aud.release_doc_num = @release_doc_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@release_doc_num as varchar)
      ...
      begin
         ...
         where aud.release_doc_num = @release_doc_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_release_document records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_release_document rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_release_document rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_release_document'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_release_document record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_release_document records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_release_document] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_release_document', NULL, NULL
GO
