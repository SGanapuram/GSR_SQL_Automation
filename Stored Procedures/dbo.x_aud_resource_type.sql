...
CREATE procedure [dbo].[x_aud_resource_type]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @res_type           char(20),
   ...
   begin
      ...
      RAISERROR('aud_resource_type: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct res_type
      from dbo.aud_resource_type
      order by res_type
 
   open mycursor
   fetch next from mycursor into @res_type
   ...
   begin
      ...
      from dbo.aud_resource_type aud
      where aud.res_type = @res_type and
            exists (select 1
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_resource_type records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_resource_type rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_resource_type rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_resource_type'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_resource_type record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_resource_type records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_resource_type] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_resource_type', NULL, NULL
GO
