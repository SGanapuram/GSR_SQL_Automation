...
CREATE procedure [dbo].[x_aud_credit_bus_category]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @cr_bus_code        char(4),
   ...
   begin
      ...
      RAISERROR('aud_credit_bus_category: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct cr_bus_code
      from dbo.aud_credit_bus_category
      order by cr_bus_code
 
   open mycursor
   fetch next from mycursor into @cr_bus_code
   ...
   begin
      ...
      from dbo.aud_credit_bus_category aud
      where aud.cr_bus_code = @cr_bus_code and
            exists (select 1
      ...
      begin
         ...
         where aud.cr_bus_code = @cr_bus_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_credit_bus_category records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_credit_bus_category rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_credit_bus_category rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_credit_bus_category'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_credit_bus_category record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_credit_bus_category records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_credit_bus_category] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_credit_bus_category', NULL, NULL
GO
