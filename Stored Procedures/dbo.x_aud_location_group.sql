...
CREATE procedure [dbo].[x_aud_location_group]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @parent_loc_code    char(8),
   ...
   begin
      ...
      RAISERROR('aud_location_group: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct parent_loc_code, loc_code, loc_type_code
      from dbo.aud_location_group
      order by parent_loc_code, loc_code, loc_type_code
 
   open mycursor
   fetch next from mycursor into @parent_loc_code, @loc_code, @loc_type_code
   ...
   begin
      ...
      from dbo.aud_location_group aud
      where aud.parent_loc_code = @parent_loc_code and
            aud.loc_code = @loc_code and
            aud.loc_type_code = @loc_type_code and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = @parent_loc_code + '/' + @loc_code + '/' + @loc_type_code
      ...
      begin
         ...
         where aud.parent_loc_code = @parent_loc_code and
               aud.loc_code = @loc_code and
               aud.loc_type_code = @loc_type_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_location_group records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_location_group rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_location_group rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_location_group'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_location_group record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_location_group records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_location_group] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_location_group', NULL, NULL
GO
