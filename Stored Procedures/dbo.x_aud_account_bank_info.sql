...
CREATE procedure [dbo].[x_aud_account_bank_info]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @acct_bank_id       int,
   ...
   begin
      ...
      RAISERROR('aud_account_bank_info: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct acct_bank_id
      from dbo.aud_account_bank_info
      order by acct_bank_id
 
   open mycursor
   fetch next from mycursor into @acct_bank_id
   ...
   begin
      ...
      from dbo.aud_account_bank_info aud
      where aud.acct_bank_id = @acct_bank_id and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@acct_bank_id as varchar)
      ...
      begin
         ...
         where aud.acct_bank_id = @acct_bank_id
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_account_bank_info records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_account_bank_info rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_account_bank_info rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_account_bank_info'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_account_bank_info record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_account_bank_info records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_account_bank_info] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_account_bank_info', NULL, NULL
GO
