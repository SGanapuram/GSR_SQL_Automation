...
CREATE procedure [dbo].[usp_get_COST_tags]
(
   @debugon          bit = 0
)                             
as
...
declare @status                       int,
   ...
   set @status = 0
   if object_id('dbo.xx101_costtags', 'U') is not null
      exec('drop table dbo.xx101_costtags')   
   ...
   begin catch
     ...
     exec dbo.usp_save_dashboard_error @report_name = 'PlCompReport',  
                                       @occurred_at = 'SP: usp_get_COST_tags', 
                                       @problem_desc = @smsg,                                           
   ...
   set @sql = 'alter table #xx101_costtags add '
   set @tag_column_list = ''
   ...
   begin
      select @tag_name = tag_name + '(CostTAG)'
      from #tagnames
      where oid = @oid
   ...
   -- alter temp table to add columns for COST tags
   -- DEBUG: print '@sql = ''' + @sql + ''''
   begin try
     exec(@sql)
   end try
   begin catch
     set @smsg = 'Failed to alter temp table ''#xx101_costtags'''  
     set @tempstr = '=> ' + @smsg + ' due to the error:'
     RAISERROR(@tempstr, 0, 1) WITH NOWAIT
     set @dberrmsg = ERROR_MESSAGE()                                                
     set @tempstr = '==> ERROR: ' + @dberrmsg                                                  
   ...
   insert into #tag_column_info
      values('Cost', @tag_column_list)
       
   begin try
     insert into #xx101_costtags
       exec dbo.usp_get_pivot_output_for_an_entity 'Cost', 
                                                   'cast(key1 as int) as cost_num', 
                                                   'cost_num'
   end try
   begin catch
     set @smsg = 'Failed to save data returned from the ''usp_get_pivot_output_for_an_entity'' procedure to the ''#xx101_costtags'' table'  
     set @tempstr = '=> ' + @smsg + ' due to the error:'
     RAISERROR(@tempstr, 0, 1) WITH NOWAIT
     set @dberrmsg = ERROR_MESSAGE()                                                
     set @tempstr = '==> ERROR: ' + @dberrmsg                                                  
     ...
     exec dbo.usp_save_dashboard_error @report_name = 'PlCompReport',  
                                       ...
                                       @problem_desc = @smsg,                                           
                                       @dberror_msg = @dberrmsg,
                                       @sql_stmt = 'insert into #xx101_costtags exec dbo.usp_get_pivot_output_for_an_entity ..',
                                       @debugon = @debugon                                                                                                    
     goto endofsp
   end catch 
   ...
   begin
      ...
      begin
         set @smsg = 'This reference Inspector ''' + @ref_insp_name + ''' is not currently supported by this version of the ''usp_get_COST_tags'' sp'
   ...
   begin
      ...
      begin
         ...
         set @sql = @sql + 'from #xx101_costtags tag left outer join dbo.icts_user u '
      ...
      begin
         ...
         set @sql = @sql + 'from #xx101_costtags tag left outer join dbo.account u '
      ...
      begin catch
        set @smsg = 'Failed to update the ''' + @tag_name + ''' column in the ''#xx101_costtags'' table'  
        set @tempstr = '=> ' + @smsg + ' due to the error:'
        RAISERROR(@tempstr, 0, 1) WITH NOWAIT
        set @dberrmsg = ERROR_MESSAGE()                                                
        set @tempstr = '==> ERROR: ' + @dberrmsg                                                  
...
return @status
GO
GRANT EXECUTE ON  [dbo].[usp_get_COST_tags] TO [next_usr]
GO
