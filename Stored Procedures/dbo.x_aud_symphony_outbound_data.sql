...
CREATE procedure [dbo].[x_aud_symphony_outbound_data]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @row_id             int,
   ...
   begin
      ...
      RAISERROR('aud_symphony_outbound_data: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct row_id
      from dbo.aud_symphony_outbound_data
      order by row_id
 
   open mycursor
   fetch next from mycursor into @row_id
   ...
   begin
      ...
      from dbo.aud_symphony_outbound_data aud
      where aud.row_id = @row_id and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@row_id as varchar)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_symphony_outbound_data records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_symphony_outbound_data rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_symphony_outbound_data rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_symphony_outbound_data'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_symphony_outbound_data record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_symphony_outbound_data records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_symphony_outbound_data] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_symphony_outbound_data', NULL, NULL
GO
