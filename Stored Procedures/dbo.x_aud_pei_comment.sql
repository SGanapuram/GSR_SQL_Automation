...
CREATE procedure [dbo].[x_aud_pei_comment]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @cmnt_num           int,
   ...
   begin
      ...
      RAISERROR('aud_pei_comment: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct cmnt_num
      from dbo.aud_pei_comment
      order by cmnt_num
 
   open mycursor
   fetch next from mycursor into @cmnt_num
   ...
   begin
      ...
      from dbo.aud_pei_comment aud
      where aud.cmnt_num = @cmnt_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@cmnt_num as varchar)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_pei_comment records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_pei_comment rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_pei_comment rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_pei_comment'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_pei_comment record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_pei_comment records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_pei_comment] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_pei_comment', NULL, NULL
GO
