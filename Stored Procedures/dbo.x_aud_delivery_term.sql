...
CREATE procedure [dbo].[x_aud_delivery_term]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @del_term_code      char(8),
   ...
   begin
      ...
      RAISERROR('aud_delivery_term: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct del_term_code
      from dbo.aud_delivery_term
      order by del_term_code
 
   open mycursor
   fetch next from mycursor into @del_term_code
   ...
   begin
      ...
      from dbo.aud_delivery_term aud
      ...
      begin
         ...
         where aud.del_term_code = @del_term_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_delivery_term records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_delivery_term rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_delivery_term rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_delivery_term'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_delivery_term record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_delivery_term records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_delivery_term] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_delivery_term', NULL, NULL
GO
