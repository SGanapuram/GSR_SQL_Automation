...
CREATE procedure [dbo].[x_aud_fx_rate_history]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @cost_num           int,
   ...
   begin
      ...
      RAISERROR('aud_fx_rate_history: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct cost_num, fx_asof_date, real_port_num, fx_exp_num
      from dbo.aud_fx_rate_history
      order by cost_num, fx_asof_date, real_port_num, fx_exp_num
 
   open mycursor
   fetch next from mycursor into @cost_num, @fx_asof_date, @real_port_num, @fx_exp_num
   ...
   begin
      ...
      from dbo.aud_fx_rate_history aud
      where aud.cost_num = @cost_num and
            aud.fx_asof_date = @fx_asof_date and
            aud.real_port_num = @real_port_num and
            aud.fx_exp_num = @fx_exp_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@cost_num as varchar) + '/' + convert(varchar, @fx_asof_date, 101) + '/' + cast(@real_port_num as varchar) + '/' + cast(@fx_exp_num as varchar)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_fx_rate_history records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_fx_rate_history rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_fx_rate_history rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_fx_rate_history'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_fx_rate_history record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_fx_rate_history records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_fx_rate_history] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_fx_rate_history', NULL, NULL
GO
