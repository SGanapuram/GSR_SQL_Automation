...
CREATE procedure [dbo].[x_aud_inventory_build_draw]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @inv_num            int,
   ...
   begin
      ...
      RAISERROR('aud_inventory_build_draw: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct inv_num, inv_b_d_num
      from dbo.aud_inventory_build_draw
      order by inv_num, inv_b_d_num
 
   open mycursor
   fetch next from mycursor into @inv_num, @inv_b_d_num
   ...
   begin
      ...
      from dbo.aud_inventory_build_draw aud
      ...
      begin
         ...
         set @tempstr = cast(@inv_num as varchar) + '/' + cast(@inv_b_d_num as varchar)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_inventory_build_draw records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_inventory_build_draw rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_inventory_build_draw rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_inventory_build_draw'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_inventory_build_draw record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_inventory_build_draw records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_inventory_build_draw] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_inventory_build_draw', NULL, NULL
GO
