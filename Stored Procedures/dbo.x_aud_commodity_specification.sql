...
CREATE procedure [dbo].[x_aud_commodity_specification]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @cmdty_code         char(8),
   ...
   begin
      ...
      RAISERROR('aud_commodity_specification: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct cmdty_code, spec_code
      from dbo.aud_commodity_specification
      order by cmdty_code, spec_code
 
   open mycursor
   fetch next from mycursor into @cmdty_code, @spec_code
   ...
   begin
      ...
      from dbo.aud_commodity_specification aud
      ...
      begin
         ...
         set @tempstr = @cmdty_code + '/' + @spec_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_commodity_specification records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_commodity_specification rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_commodity_specification rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_commodity_specification'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_commodity_specification record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_commodity_specification records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_commodity_specification] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_commodity_specification', NULL, NULL
GO
