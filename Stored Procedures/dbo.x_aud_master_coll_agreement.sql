...
CREATE procedure [dbo].[x_aud_master_coll_agreement]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @mca_num            int,
   ...
   begin
      ...
      RAISERROR('aud_master_coll_agreement: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct mca_num
      from dbo.aud_master_coll_agreement
      order by mca_num
 
   open mycursor
   fetch next from mycursor into @mca_num
   ...
   begin
      ...
      from dbo.aud_master_coll_agreement aud
      ...
      begin
         ...
         set @tempstr = cast(@mca_num as varchar)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_master_coll_agreement records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_master_coll_agreement rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_master_coll_agreement rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_master_coll_agreement'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_master_coll_agreement record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_master_coll_agreement records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_master_coll_agreement] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_master_coll_agreement', NULL, NULL
GO
