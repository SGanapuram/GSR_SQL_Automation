...
CREATE PROC [dbo].[MDS_inventory_detail]    
@PortNum int=null,    
...
insert into #inv_det    
 ...
 from #inv_det i,    
  (SELECT BuildDrawAllocNum, BuildDrawAllocItemNum,    
   ...
   FROM v_MDS_allocation_reach_detail v, #inv_det i    
  ...
  (SELECT InvAllocNum, InvAllocItemNum,    
   stuff((     
    ...
    from (SELECT distinct i.InvAllocNum,i.InvAllocItemNum, cas_num_desc1    
     FROM v_MDS_allocation_reach_detail v, #inv_det i  , inventory inv, trade_item ti, allocation_item ai  
...
-- Storage Side Works--  
 ...
 from #inv_det i,    
  (SELECT InvAllocNum,InvAllocItemNum,     
   ...
   FROM v_MDS_allocation_reach_detail v, #inv_det i,allocation_item ai,  trade_item ti    
   WHERE ai.alloc_num=v.alloc_num    
   and ai.alloc_item_num=v.alloc_item_num    
   and ai.trade_num=ti.trade_num    
   and ai.order_num=ti.order_num    
...
END    
GO
GRANT EXECUTE ON  [dbo].[MDS_inventory_detail] TO [next_usr]
GO
