...
CREATE procedure [dbo].[x_aud_vat_trans_nature]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @trans_nature_code  char(8),
        @trans_id           int,
        @resp_trans_id      int,
        @cutoffdate         datetime,
        @rows_deleted       int,
   ...
   begin
      ...
      RAISERROR('aud_vat_trans_nature: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct trans_nature_code
      from dbo.aud_vat_trans_nature
      order by trans_nature_code
 
   open mycursor
   fetch next from mycursor into @trans_nature_code
   ...
   begin
      ...
      from dbo.aud_vat_trans_nature aud
      where aud.trans_nature_code = @trans_nature_code and
            exists (select 1
      ...
      begin
         ...
         where aud.trans_nature_code = @trans_nature_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_vat_trans_nature records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_vat_trans_nature rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_vat_trans_nature rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_vat_trans_nature'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_vat_trans_nature record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_vat_trans_nature records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_vat_trans_nature] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_vat_trans_nature', NULL, NULL
GO
