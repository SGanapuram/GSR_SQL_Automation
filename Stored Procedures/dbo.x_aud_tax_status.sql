...
CREATE procedure [dbo].[x_aud_tax_status]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @tax_status_code    char(8),
        @trans_id           int,
        @resp_trans_id      int,
        @cutoffdate         datetime,
        @rows_deleted       int,
   ...
   begin
      ...
      RAISERROR('aud_tax_status: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct tax_status_code
      from dbo.aud_tax_status
      order by tax_status_code
 
   open mycursor
   fetch next from mycursor into @tax_status_code
   ...
   begin
      ...
      from dbo.aud_tax_status aud
      where aud.tax_status_code = @tax_status_code and
            exists (select 1
      ...
      begin
         ...
         where aud.tax_status_code = @tax_status_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_tax_status records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_tax_status rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_tax_status rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_tax_status'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_tax_status record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_tax_status records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_tax_status] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_tax_status', NULL, NULL
GO
