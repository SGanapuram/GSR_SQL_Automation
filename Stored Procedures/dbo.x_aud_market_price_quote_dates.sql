...
CREATE procedure [dbo].[x_aud_market_price_quote_dates]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @cmf_num            int,
   ...
   begin
      ...
      RAISERROR('aud_market_price_quote_dates: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct cmf_num, calendar_date
      from dbo.aud_market_price_quote_dates
      order by cmf_num, calendar_date
 
   open mycursor
   fetch next from mycursor into @cmf_num, @calendar_date
   ...
   begin
      ...
      from dbo.aud_market_price_quote_dates aud
      where aud.cmf_num = @cmf_num and
            aud.calendar_date = @calendar_date and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@cmf_num as varchar) + '/' + convert(varchar, @calendar_date, 101)
      ...
      begin
         ...
         where aud.cmf_num = @cmf_num and
               aud.calendar_date = @calendar_date
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_market_price_quote_dates records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_market_price_quote_dates rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_market_price_quote_dates rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_market_price_quote_dates'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_market_price_quote_dates record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_market_price_quote_dates records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_market_price_quote_dates] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_market_price_quote_dates', NULL, NULL
GO
