...
CREATE procedure [dbo].[x_aud_risk_transfer_indicator]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @risk_transfer_ind_code char(2),
        @trans_id               int,
        @resp_trans_id          int,
        @cutoffdate             datetime,
        @rows_deleted           int,
   ...
   begin
      ...
      RAISERROR('aud_risk_transfer_indicator: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct risk_transfer_ind_code
      from dbo.aud_risk_transfer_indicator
      order by risk_transfer_ind_code
 
   open mycursor
   fetch next from mycursor into @risk_transfer_ind_code
   ...
   begin
      ...
      from dbo.aud_risk_transfer_indicator aud
      where aud.risk_transfer_ind_code = @risk_transfer_ind_code and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = @risk_transfer_ind_code
      ...
      begin
         ...
         where aud.risk_transfer_ind_code = @risk_transfer_ind_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_risk_transfer_indicator records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_risk_transfer_indicator rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_risk_transfer_indicator rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_risk_transfer_indicator'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_risk_transfer_indicator record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_risk_transfer_indicator records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_risk_transfer_indicator] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_risk_transfer_indicator', NULL, NULL
GO
