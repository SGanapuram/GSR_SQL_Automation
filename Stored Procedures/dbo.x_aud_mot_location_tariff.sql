...
CREATE procedure [dbo].[x_aud_mot_location_tariff]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @mot_code            char(8),
        @loc_code            char(8),
        @loc_code_to         char(8),
        @ppl_tariff_eff_date datetime,
        @trans_id            int,
        @resp_trans_id       int,
        @cutoffdate          datetime,
        @rows_deleted        int,
   ...
   begin
      ...
      RAISERROR('aud_mot_location_tariff: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct mot_code, loc_code, loc_code_to, ppl_tariff_eff_date
      from dbo.aud_mot_location_tariff
      order by mot_code, loc_code, loc_code_to, ppl_tariff_eff_date
 
   open mycursor
   fetch next from mycursor into @mot_code, @loc_code, @loc_code_to, @ppl_tariff_eff_date
   ...
   begin
      ...
      from dbo.aud_mot_location_tariff aud
      where aud.mot_code = @mot_code and
            aud.loc_code = @loc_code and
            aud.loc_code_to = @loc_code_to and
            aud.ppl_tariff_eff_date = @ppl_tariff_eff_date and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = @mot_code + '/' + @loc_code + '/' + @loc_code_to + '/' + convert(varchar, @ppl_tariff_eff_date, 101)
      ...
      begin
         ...
         where aud.mot_code = @mot_code and
               aud.loc_code = @loc_code and
               aud.loc_code_to = @loc_code_to and
               aud.ppl_tariff_eff_date = @ppl_tariff_eff_date
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_mot_location_tariff records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_mot_location_tariff rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_mot_location_tariff rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_mot_location_tariff'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_mot_location_tariff record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_mot_location_tariff records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_mot_location_tariff] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_mot_location_tariff', NULL, NULL
GO
