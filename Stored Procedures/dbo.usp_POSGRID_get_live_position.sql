...
CREATE procedure [dbo].[usp_POSGRID_get_live_position]  
(
...
declare @rows_affected     int,
   ...
   begin try 
	   ...
	   from dbo.v_POSGRID_risk_position pos with(NOLOCK)                    
		         LEFT OUTER JOIN
     ...
     where exists (select 1 
	                 from #children c1 
	                 where c1.port_num = pos.real_port_num)
	   set @rows_affected = @@rowcount
	 end try
	 begin catch
	   set @smsg = '=> Failed to fetch records from the ''v_POSGRID_risk_position'' view due to the error:'
	 ...
	 if @debugon = 1
	 begin
	    set @smsg = '=> ' + cast(@rows_affected as varchar) + ' records being fetched from the ''v_POSGRID_risk_position'' view'
	    RAISERROR(@smsg, 0, 1) WITH NOWAIT
