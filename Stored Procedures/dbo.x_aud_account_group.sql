...
CREATE procedure [dbo].[x_aud_account_group]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @related_acct_num     int,
   ...
   begin
      ...
      RAISERROR('aud_account_group: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct related_acct_num, acct_num, acct_group_type_code
      from dbo.aud_account_group
      order by related_acct_num, acct_num, acct_group_type_code
 
   open mycursor
   fetch next from mycursor into @related_acct_num, @acct_num, @acct_group_type_code
   ...
   begin
      ...
      from dbo.aud_account_group aud
      where aud.related_acct_num = @related_acct_num and
            aud.acct_num = @acct_num and
            aud.acct_group_type_code = @acct_group_type_code and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@related_acct_num as varchar) + '/' + cast(@acct_num as varchar) + '/' + @acct_group_type_code
      ...
      begin
         ...
         where aud.related_acct_num = @related_acct_num and
               aud.acct_num = @acct_num and
               aud.acct_group_type_code = @acct_group_type_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_account_group records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_account_group rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_account_group rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_account_group'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_account_group record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_account_group records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_account_group] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_account_group', NULL, NULL
GO
