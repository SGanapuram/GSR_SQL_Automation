...
CREATE procedure [dbo].[x_aud_voucher_duedate]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @voucher_num        int,
        @voudue_duedate     datetime,
        @voudue_seq_num     smallint,
        @trans_id           int,
        @resp_trans_id      int,
        @cutoffdate         datetime,
        @rows_deleted       int,
   ...
   begin
      ...
      RAISERROR('aud_voucher_duedate: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct voucher_num, voudue_duedate, voudue_seq_num
      from dbo.aud_voucher_duedate
      order by voucher_num, voudue_duedate, voudue_seq_num
 
   open mycursor
   fetch next from mycursor into @voucher_num, @voudue_duedate, @voudue_seq_num
   ...
   begin
      ...
      from dbo.aud_voucher_duedate aud
      where aud.voucher_num = @voucher_num and
            aud.voudue_duedate = @voudue_duedate and
            aud.voudue_seq_num = @voudue_seq_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@voucher_num as varchar) + '/' + convert(varchar, @voudue_duedate, 101) + '/' + cast(@voudue_seq_num as varchar)
      ...
      begin
         ...
         where aud.voucher_num = @voucher_num and
               aud.voudue_duedate = @voudue_duedate and
               aud.voudue_seq_num = @voudue_seq_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_voucher_duedate records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_voucher_duedate rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_voucher_duedate rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_voucher_duedate'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_voucher_duedate record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_voucher_duedate records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_voucher_duedate] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_voucher_duedate', NULL, NULL
GO
