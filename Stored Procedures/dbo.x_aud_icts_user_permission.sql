...
CREATE procedure [dbo].[x_aud_icts_user_permission]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @user_init          char(3),
   ...
   begin
      ...
      RAISERROR('aud_icts_user_permission: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct user_init, fdv_id
      from dbo.aud_icts_user_permission
      order by user_init, fdv_id
 
   open mycursor
   fetch next from mycursor into @user_init, @fdv_id
   ...
   begin
      ...
      from dbo.aud_icts_user_permission aud
      where aud.user_init = @user_init and
            aud.fdv_id = @fdv_id and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = @user_init + '/' + cast(@fdv_id as varchar)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_icts_user_permission records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_icts_user_permission rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_icts_user_permission rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_icts_user_permission'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_icts_user_permission record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_icts_user_permission records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_icts_user_permission] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_icts_user_permission', NULL, NULL
GO
