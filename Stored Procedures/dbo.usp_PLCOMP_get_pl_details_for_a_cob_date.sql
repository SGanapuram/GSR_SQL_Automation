...
declare @status            int,                    
   ...
   if @first_date_flag = 1                    
    ...
    when pl_record_qty is null and                     
         ...
         case when ti.p_s_ind = 'S' then -1                     
                     ...
                     end * ( case when ti.contr_qty is not null then ti.contr_qty else  0  end ), /* contr_qty */                  
  ...
  case when ti.p_s_ind = 'S' then -1                    
        ...
        case when ti.order_type_code='SWAP'        
        then case when ti.p_s_ind = 'P' then -1                     
                          else 1                     
                     end *CONVERT(FLOAT, case when isnumeric(sfp.swap_fixedPrice)=1 then sfp.swap_fixedPrice else '0' end)+ isnull(pl_amt,0)/isnull(ti.contr_qty,1)        
 ...
 --ADSO-3127 - Commented below            
   ...
   ai.order_num = pl.pl_secondary_owner_key2 and                     
    ...
    on ai.alloc_num = a.alloc_num   */
     ...
     -- ADSO-9055 Changes --
  LEFT OUTER JOIN trade_item_dist tid ON tid.trade_num = ti.trade_num and tid.order_num = ti.order_num and tid.item_num = ti.item_num and tid.dist_type = 'D'
     
  LEFT OUTER JOIN tid_mark_to_market tidMtm
  ON tidMtm.dist_num = pl.pl_record_owner_key
  AND tidMtm.trade_num = pl.pl_primary_owner_key1
  AND tidMtm.order_num = pl.pl_primary_owner_key2
  AND tidMtm.item_num = pl.pl_primary_owner_key3
  AND tidMtm.mtm_pl_asof_date = CASE WHEN exists(SELECT 1 FROM tid_mark_to_market tidMtm2 WHERE tidMtm2.dist_num = pl.pl_record_owner_key AND tidMtm2.mtm_pl_asof_date = pl.pl_asof_date) THEN pl.pl_asof_date 
									 ELSE (SELECT MAX(tidMtm3.mtm_pl_asof_date) FROM tid_mark_to_market tidMtm3 WHERE tidMtm3.dist_num = pl.pl_record_owner_key ) END
     else                    
          select                      
        pl_record_key,                                                                            
        pl.pl_asof_date,                                                                             
    ...
    when pl_owner_code = 'IPO' then oinv.sale_item_num else pl.pl_secondary_owner_key3 end, /* item_num */                
        ...
        pl.pl_type_desc,                                                                          
        case when @first_date_flag = 1                     
                then null                                 when pl_owner_code in ('I', 'P')                     
                then 'INVENTORY'                          
             else isnull(isnull(case when ti.order_type_code in ('SWAP','SWAPFLT')                     
                                        then 'SWAP'                                                 
                                     when pl_owner_sub_code in ('ADDLA', 'ADDLAA', 'ADDLAI',                     
        ...
        case when @first_date_flag = 1                     
          ...
          (case when pl_type  ='U'  Then          
            ...
            (pl_amt /( (pl_amt +(select tempPHist.pl_amt from pl_history as tempPHist where tempPHist.pl_record_key= pl.pl_record_key                  and  tempPHist.pl_type ='R' and tempPHist.pl_owner_code in ('I', 'P')  and tempPHist.pl_asof_date = pl.
pl_asof_date ))/pl.pl_record_qty))        
            else 0 end)          
          else          
            (case when pl_type  ='R' then          
              (case when pl_amt != 0 then           
               (pl_amt /( (pl_amt +(select tempPHist.pl_amt from pl_history as tempPHist where tempPHist.pl_record_key= pl.pl_record_key and              tempPHist.pl_type ='U' and tempPHist.pl_owner_code in ('I', 'P')  and tempPHist.pl_asof_date = 
               pl.pl_asof_date ))/pl.pl_record_qty ))         
              else 0 end)          
            else          
           pl_record_qty          
            end)          
