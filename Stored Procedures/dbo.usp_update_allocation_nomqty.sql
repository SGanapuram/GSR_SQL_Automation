...
CREATE PROC [dbo].[usp_update_allocation_nomqty]    
(
   @alloc_num      int,    
   @alloc_item_num int,    
   @nom_qty_max    float,    
...
DECLARE @ai_est_actual_num          int, 
        @primary_uom                char(8), 
        @sec_uom                    char(8), 
        @dest_loc_code              char(8), 
        @sec_conversion_factor      float, 
        @cmdty_code                 char(8), 
        @sequence                   numeric, 
        @trans_id                   int
                                          
DECLARE @conv_ratio_gross           float,
        @conv_ratio_net             float,
        @ai_est_actual_gross_qty    float,
        @ai_est_actual_net_qty      float,
        @secondary_actual_gross_qty float,   
        @secondary_actual_net_qty   float,
        @trade_num                  int,   
        @order_num                  int, 
        @item_num                   int,
        @secondary_net_qty          float,
        @num_of_days                int    
...
END  */  
 ...
 where @num_of_days is null    
  ...
  exec gen_new_transaction 'usp_update_allocation_nomqty','U'                                    
...
GO
GRANT EXECUTE ON  [dbo].[usp_update_allocation_nomqty] TO [next_usr]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'usp_update_allocation_nomqty', NULL, NULL
GO
