...
CREATE procedure [dbo].[x_aud_bus_cost_fate]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @bc_fate_num        smallint,
   ...
   begin
      ...
      RAISERROR('aud_bus_cost_fate: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct bc_fate_num
      from dbo.aud_bus_cost_fate
      order by bc_fate_num
 
   open mycursor
   fetch next from mycursor into @bc_fate_num
   ...
   begin
      ...
      from dbo.aud_bus_cost_fate aud
      where aud.bc_fate_num = @bc_fate_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@bc_fate_num as varchar)
      ...
      begin
         ...
         where aud.bc_fate_num = @bc_fate_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_bus_cost_fate records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_bus_cost_fate rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_bus_cost_fate rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_bus_cost_fate'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_bus_cost_fate record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_bus_cost_fate records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_bus_cost_fate] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_bus_cost_fate', NULL, NULL
GO
