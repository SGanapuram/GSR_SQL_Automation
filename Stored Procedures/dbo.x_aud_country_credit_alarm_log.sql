...
CREATE procedure [dbo].[x_aud_country_credit_alarm_log]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @credit_alarm_log_num int,
        @country_code         char(8),
        @trans_id             int,
        @resp_trans_id        int,
        @cutoffdate           datetime,
   ...
   begin
      ...
      RAISERROR('aud_country_credit_alarm_log: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct credit_alarm_log_num, country_code
      from dbo.aud_country_credit_alarm_log
      order by credit_alarm_log_num, country_code
 
   open mycursor
   fetch next from mycursor into @credit_alarm_log_num, @country_code
   ...
   begin
      ...
      from dbo.aud_country_credit_alarm_log aud
      where aud.credit_alarm_log_num = @credit_alarm_log_num and
            aud.country_code = @country_code and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@credit_alarm_log_num as varchar) + '/' + @country_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_country_credit_alarm_log records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_country_credit_alarm_log rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_country_credit_alarm_log rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_country_credit_alarm_log'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_country_credit_alarm_log record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_country_credit_alarm_log records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_country_credit_alarm_log] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_country_credit_alarm_log', NULL, NULL
GO
