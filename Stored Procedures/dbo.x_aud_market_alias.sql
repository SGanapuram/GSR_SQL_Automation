...
CREATE procedure [dbo].[x_aud_market_alias]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @mkt_code           char(8),
   ...
   begin
      ...
      RAISERROR('aud_market_alias: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct mkt_code, alias_source_code
      from dbo.aud_market_alias
      order by mkt_code, alias_source_code
 
   open mycursor
   fetch next from mycursor into @mkt_code, @alias_source_code
   ...
   begin
      ...
      from dbo.aud_market_alias aud
      where aud.mkt_code = @mkt_code and
      ...
      begin
         ...
         set @tempstr = @mkt_code + '/' + @alias_source_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_market_alias records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_market_alias rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_market_alias rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_market_alias'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_market_alias record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_market_alias records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_market_alias] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_market_alias', NULL, NULL
GO
