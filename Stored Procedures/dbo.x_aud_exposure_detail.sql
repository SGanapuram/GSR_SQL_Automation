...
CREATE procedure [dbo].[x_aud_exposure_detail]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @cost_num           int,
   ...
   begin
      ...
      RAISERROR('aud_exposure_detail: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct cost_num, exposure_num
      from dbo.aud_exposure_detail
      order by cost_num, exposure_num
 
   open mycursor
   fetch next from mycursor into @cost_num, @exposure_num
   ...
   begin
      ...
      from dbo.aud_exposure_detail aud
      where aud.cost_num = @cost_num and
            aud.exposure_num = @exposure_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@cost_num as varchar) + '/' + cast(@exposure_num as varchar)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_exposure_detail records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_exposure_detail rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_exposure_detail rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_exposure_detail'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_exposure_detail record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_exposure_detail records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_exposure_detail] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_exposure_detail', NULL, NULL
GO
