...
CREATE procedure [dbo].[x_aud_order_instruction]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @order_instr_code   char(8),
        @trans_id           int,
        @resp_trans_id      int,
        @cutoffdate         datetime,
        @rows_deleted       int,
   ...
   begin
      ...
      RAISERROR('aud_order_instruction: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct order_instr_code
      from dbo.aud_order_instruction
      order by order_instr_code
 
   open mycursor
   fetch next from mycursor into @order_instr_code
   ...
   begin
      ...
      from dbo.aud_order_instruction aud
      where aud.order_instr_code = @order_instr_code and
            exists (select 1
      ...
      begin
         ...
         where aud.order_instr_code = @order_instr_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_order_instruction records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_order_instruction rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_order_instruction rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_order_instruction'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_order_instruction record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_order_instruction records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_order_instruction] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_order_instruction', NULL, NULL
GO
