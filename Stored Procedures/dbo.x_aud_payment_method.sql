...
CREATE procedure [dbo].[x_aud_payment_method]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @pay_method_code    char(8),
        @trans_id           int,
        @resp_trans_id      int,
        @cutoffdate         datetime,
        @rows_deleted       int,
   ...
   begin
      ...
      RAISERROR('aud_payment_method: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct pay_method_code
      from dbo.aud_payment_method
      order by pay_method_code
 
   open mycursor
   fetch next from mycursor into @pay_method_code
   ...
   begin
      ...
      from dbo.aud_payment_method aud
      where aud.pay_method_code = @pay_method_code and
            exists (select 1
      ...
      begin
         ...
         where aud.pay_method_code = @pay_method_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_payment_method records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_payment_method rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_payment_method rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_payment_method'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_payment_method record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_payment_method records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_payment_method] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_payment_method', NULL, NULL
GO
