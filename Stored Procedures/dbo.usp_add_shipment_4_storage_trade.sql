...
declare @new_sequence_num           table (seq_num  int)  
   ...
   begin try        
     ...
     where num_col_name = 'shipment_oid'   
   end try  
   begin catch  
     RAISERROR('=> Failed to update the new_num table for a new shipment OID due to the error:', 0, 1) WITH NOWAIT  
   ...
   begin try        
     update dbo.new_num   
     set last_num = last_num + 2   /* There are 2 parcels needed, so reserve 2 parcel OIDs */  
           output inserted.last_num into @new_sequence_num  
     where num_col_name = 'parcel_oid'   
   end try  
   begin catch  
     RAISERROR('=> Failed to update the new_num table for a new parcel OID due to the error:', 0, 1) WITH NOWAIT  
     set @errmsg = ERROR_MESSAGE()  
     RAISERROR('==> ERROR: %s', 0, 1, @errmsg) WITH NOWAIT    
     set @errcode = ERROR_NUMBER()  
     goto endofsp      
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'usp_add_shipment_4_storage_trade', NULL, NULL
GO
