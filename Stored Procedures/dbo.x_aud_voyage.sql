...
CREATE procedure [dbo].[x_aud_voyage]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @voyage_code        char(20),
   ...
   begin
      ...
      RAISERROR('aud_voyage: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct voyage_code
      from dbo.aud_voyage
      order by voyage_code
 
   open mycursor
   fetch next from mycursor into @voyage_code
   ...
   begin
      ...
      from dbo.aud_voyage aud
      where aud.voyage_code = @voyage_code and
            exists (select 1
      ...
      begin
         ...
         where aud.voyage_code = @voyage_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_voyage records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_voyage rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_voyage rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_voyage'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_voyage record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_voyage records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_voyage] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_voyage', NULL, NULL
GO
