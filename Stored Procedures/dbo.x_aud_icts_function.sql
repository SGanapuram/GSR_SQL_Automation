...
CREATE procedure [dbo].[x_aud_icts_function]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @function_num       int,
   ...
   begin
      ...
      RAISERROR('aud_icts_function: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
   ...
   open mycursor
   fetch next from mycursor into @function_num
   ...
   begin
      ...
      from dbo.aud_icts_function aud
      where aud.function_num = @function_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@function_num as varchar)
      ...
      begin
         ...
         where aud.function_num = @function_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_icts_function records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_icts_function rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_icts_function rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_icts_function'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_icts_function record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_icts_function records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_icts_function] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_icts_function', NULL, NULL
GO
