...
CREATE procedure [dbo].[x_aud_ag_external_codes]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @oid                int,
   ...
   begin
      ...
      RAISERROR('aud_ag_external_codes: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct oid
      from dbo.aud_ag_external_codes
      order by oid
   ...
   begin
      ...
      from dbo.aud_ag_external_codes aud
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_ag_external_codes records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_ag_external_codes rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_ag_external_codes rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_ag_external_codes'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_ag_external_codes record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_ag_external_codes records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_ag_external_codes] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_ag_external_codes', NULL, NULL
GO
