...
CREATE procedure [dbo].[x_aud_uom]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @uom_code           char(4),
   ...
   begin
      ...
      RAISERROR('aud_uom: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
   ...
   open mycursor
   fetch next from mycursor into @uom_code
   ...
   begin
      ...
      from dbo.aud_uom aud
      where aud.uom_code = @uom_code and
            exists (select 1
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_uom records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_uom rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_uom rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_uom'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_uom record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_uom records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_uom] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_uom', NULL, NULL
GO
