...
CREATE procedure [dbo].[x_aud_fx_linked_costs]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @fx_link_oid        int,
   ...
   begin
      ...
      RAISERROR('aud_fx_linked_costs: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct fx_link_oid, cost_num
      from dbo.aud_fx_linked_costs
      order by fx_link_oid, cost_num
 
   open mycursor
   fetch next from mycursor into @fx_link_oid, @cost_num
   ...
   begin
      ...
      from dbo.aud_fx_linked_costs aud
      where aud.fx_link_oid = @fx_link_oid and
      ...
      begin
         ...
         set @tempstr = cast(@fx_link_oid as varchar) + '/' + cast(@cost_num as varchar)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_fx_linked_costs records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_fx_linked_costs rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_fx_linked_costs rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_fx_linked_costs'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_fx_linked_costs record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_fx_linked_costs records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_fx_linked_costs] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_fx_linked_costs', NULL, NULL
GO
