...
CREATE procedure [dbo].[x_aud_reprice_event]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @reprice_event_oid  int,
   ...
   begin
      ...
      RAISERROR('aud_reprice_event: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct reprice_event_oid
      from dbo.aud_reprice_event
      order by reprice_event_oid
 
   open mycursor
   fetch next from mycursor into @reprice_event_oid
   ...
   begin
      ...
      from dbo.aud_reprice_event aud
      ...
      begin
         ...
         set @tempstr = cast(@reprice_event_oid as varchar)
      ...
      begin
         ...
         where aud.reprice_event_oid = @reprice_event_oid
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_reprice_event records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_reprice_event rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_reprice_event rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_reprice_event'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_reprice_event record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_reprice_event records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_reprice_event] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_reprice_event', NULL, NULL
GO
