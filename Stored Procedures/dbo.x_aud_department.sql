...
CREATE procedure [dbo].[x_aud_department]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @dept_code          char(8),
   ...
   begin
      ...
      RAISERROR('aud_department: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct dept_code
      from dbo.aud_department
      order by dept_code
 
   open mycursor
   fetch next from mycursor into @dept_code
   ...
   begin
      ...
      from dbo.aud_department aud
      where aud.dept_code = @dept_code and
            exists (select 1
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_department records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_department rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_department rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_department'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_department record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_department records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_department] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_department', NULL, NULL
GO
