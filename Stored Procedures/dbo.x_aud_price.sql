...
CREATE procedure [dbo].[x_aud_price]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @commkt_key         int,
   ...
   begin
      ...
      RAISERROR('aud_price: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct commkt_key, price_source_code, trading_prd, price_quote_date
      from dbo.aud_price
      order by commkt_key, price_source_code, trading_prd, price_quote_date
 
   open mycursor
   fetch next from mycursor into @commkt_key, @price_source_code, @trading_prd, @price_quote_date
   ...
   begin
      ...
      begin
         ...
         set @tempstr = cast(@commkt_key as varchar) + '/' + @price_source_code + '/' + @trading_prd + '/' + convert(varchar, @price_quote_date, 101)
      ...
      begin
         ...
         where aud.commkt_key = @commkt_key and
               aud.price_source_code = @price_source_code and
               aud.trading_prd = @trading_prd and
               aud.price_quote_date = @price_quote_date
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_price records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_price rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_price rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_price'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_price record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_price records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_price] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_price', NULL, NULL
GO
