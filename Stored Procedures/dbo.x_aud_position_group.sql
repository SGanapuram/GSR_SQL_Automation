...
CREATE procedure [dbo].[x_aud_position_group]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @pos_group_num      int,
   ...
   begin
      ...
      RAISERROR('aud_position_group: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct pos_group_num
      from dbo.aud_position_group
      order by pos_group_num
 
   open mycursor
   fetch next from mycursor into @pos_group_num
   ...
   begin
      ...
      from dbo.aud_position_group aud
      where aud.pos_group_num = @pos_group_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@pos_group_num as varchar)
      ...
      begin
         ...
         where aud.pos_group_num = @pos_group_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_position_group records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_position_group rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_position_group rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_position_group'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_position_group record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_position_group records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_position_group] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_position_group', NULL, NULL
GO
