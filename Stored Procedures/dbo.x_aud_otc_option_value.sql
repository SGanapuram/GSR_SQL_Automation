...
CREATE procedure [dbo].[x_aud_otc_option_value]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @otc_opt_code       char(8),
        @otc_opt_quote_date datetime,
        @trans_id           int,
        @resp_trans_id      int,
        @cutoffdate         datetime,
        @rows_deleted       int,
   ...
   begin
      ...
      RAISERROR('aud_otc_option_value: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct otc_opt_code, otc_opt_quote_date
      from dbo.aud_otc_option_value
      order by otc_opt_code, otc_opt_quote_date
 
   open mycursor
   fetch next from mycursor into @otc_opt_code, @otc_opt_quote_date
   ...
   begin
      ...
      from dbo.aud_otc_option_value aud
      where aud.otc_opt_code = @otc_opt_code and
            aud.otc_opt_quote_date = @otc_opt_quote_date and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = @otc_opt_code + '/' + convert(varchar, @otc_opt_quote_date, 101)
      ...
      begin
         ...
         where aud.otc_opt_code = @otc_opt_code and
               aud.otc_opt_quote_date = @otc_opt_quote_date
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_otc_option_value records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_otc_option_value rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_otc_option_value rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_otc_option_value'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_otc_option_value record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_otc_option_value records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_otc_option_value] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_otc_option_value', NULL, NULL
GO
