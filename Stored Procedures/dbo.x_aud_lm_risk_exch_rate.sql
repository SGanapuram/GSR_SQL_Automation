...
CREATE procedure [dbo].[x_aud_lm_risk_exch_rate]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @exch_code          char(8),
        @quote_date         datetime,
        @from_curr_code     char(8),
        @to_curr_code       char(8),
        @trans_id           int,
        @resp_trans_id      int,
        @cutoffdate         datetime,
   ...
   begin
      ...
      RAISERROR('aud_lm_risk_exch_rate: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct exch_code, quote_date, from_curr_code, to_curr_code
      from dbo.aud_lm_risk_exch_rate
      order by exch_code, quote_date, from_curr_code, to_curr_code
 
   open mycursor
   fetch next from mycursor into @exch_code, @quote_date, @from_curr_code, @to_curr_code
   ...
   begin
      ...
      from dbo.aud_lm_risk_exch_rate aud
      where aud.exch_code = @exch_code and
            aud.quote_date = @quote_date and
            aud.from_curr_code = @from_curr_code and
            aud.to_curr_code = @to_curr_code and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = @exch_code + '/' + convert(varchar, @quote_date, 101) + '/' + @from_curr_code + '/' + @to_curr_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_lm_risk_exch_rate records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_lm_risk_exch_rate rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_lm_risk_exch_rate rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_lm_risk_exch_rate'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_lm_risk_exch_rate record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_lm_risk_exch_rate records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_lm_risk_exch_rate] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_lm_risk_exch_rate', NULL, NULL
GO
