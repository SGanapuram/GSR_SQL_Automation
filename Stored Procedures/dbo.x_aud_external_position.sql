...
CREATE procedure [dbo].[x_aud_external_position]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @ext_pos_num        int,
   ...
   begin
      ...
      RAISERROR('aud_external_position: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct ext_pos_num
      from dbo.aud_external_position
      order by ext_pos_num
 
   open mycursor
   fetch next from mycursor into @ext_pos_num
   ...
   begin
      ...
      from dbo.aud_external_position aud
      where aud.ext_pos_num = @ext_pos_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@ext_pos_num as varchar)
      ...
      begin
         ...
         where aud.ext_pos_num = @ext_pos_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_external_position records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_external_position rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_external_position rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_external_position'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_external_position record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_external_position records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_external_position] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_external_position', NULL, NULL
GO
