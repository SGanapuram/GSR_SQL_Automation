...
CREATE procedure [dbo].[x_aud_voucher_vat]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @voucher_num        int,
   ...
   begin
      ...
      RAISERROR('aud_voucher_vat: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct voucher_num
      from dbo.aud_voucher_vat
   ...
   begin
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_voucher_vat records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_voucher_vat rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_voucher_vat rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_voucher_vat'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_voucher_vat record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_voucher_vat records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_voucher_vat] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_voucher_vat', NULL, NULL
GO
