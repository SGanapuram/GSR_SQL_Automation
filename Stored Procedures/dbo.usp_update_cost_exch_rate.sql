...
CREATE PROC [dbo].[usp_update_cost_exch_rate]                                          
AS                                          
BEGIN                                          
                                    
--VOUCHED /PAID Cost in with Future Cost Due Date                                           
declare @oid int                                          
declare @trans_id2 int                                          
...
-- Update all costs PAID/Partially Paid cost with a rate if the cost did not get a rate from above scripts (Paid on a holiday)--              
  ...
  and cost_amt<>0                                                            
  and cost_type_code not like 'POMAX%'                                                            
/*  and not exists (select 1 from v_MDS_price_detail pr where pr.trading_prd='SPOT' and pr.cmdty_code=c.cost_price_curr_code              
     and pr.price_quote_date=convert(char,cost_paid_date,101)  and pr.price_source_code='INTERNAL'                 
     )              
  and not exists (select 1 from v_MDS_price_detail pr where pr.trading_prd='SPOT' and pr.mkt_code=c.cost_price_curr_code              
     and pr.price_quote_date=convert(char,cost_paid_date,101)  and pr.price_source_code='INTERNAL'                 
     )   */           
...
begin tran                                      
 ...
 declare @trans_id4 int,@price_quote_date datetime, @avg_price float                                      
 exec gen_new_transaction                                      
