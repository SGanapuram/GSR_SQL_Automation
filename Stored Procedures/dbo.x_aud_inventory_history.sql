...
CREATE procedure [dbo].[x_aud_inventory_history]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @asof_date          datetime,
   ...
   begin
      ...
      RAISERROR('aud_inventory_history: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct asof_date, real_port_num, inv_num, cost_num, rcpt_alloc_num
      from dbo.aud_inventory_history
      order by asof_date, real_port_num, inv_num, cost_num, rcpt_alloc_num
 
   open mycursor
   fetch next from mycursor into @asof_date, @real_port_num, @inv_num, @cost_num, @rcpt_alloc_num
   ...
   begin
      ...
      from dbo.aud_inventory_history aud
      where aud.asof_date = @asof_date and
            aud.real_port_num = @real_port_num and
            aud.inv_num = @inv_num and
            aud.cost_num = @cost_num and
            aud.rcpt_alloc_num = @rcpt_alloc_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = convert(varchar, @asof_date, 101) + '/' + cast(@real_port_num as varchar) + '/' + cast(@inv_num as varchar) + '/' + cast(@cost_num as varchar) + '/' + cast(@rcpt_alloc_num as varchar)
      ...
      begin
         ...
         where aud.asof_date = @asof_date and
               ...
               aud.inv_num = @inv_num and
               aud.cost_num = @cost_num and
               aud.rcpt_alloc_num = @rcpt_alloc_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_inventory_history records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_inventory_history rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_inventory_history rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_inventory_history'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_inventory_history record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_inventory_history records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_inventory_history] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_inventory_history', NULL, NULL
GO
