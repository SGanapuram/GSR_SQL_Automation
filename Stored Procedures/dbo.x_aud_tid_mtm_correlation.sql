...
CREATE procedure [dbo].[x_aud_tid_mtm_correlation]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @dist_num1          int,
        @dist_num2          int,
        @mtm_pl_asof_date   datetime,
        @trans_id           int,
        @resp_trans_id      int,
        @cutoffdate         datetime,
        @rows_deleted       int,
   ...
   begin
      ...
      RAISERROR('aud_tid_mtm_correlation: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct dist_num1, dist_num2, mtm_pl_asof_date
      from dbo.aud_tid_mtm_correlation
      order by dist_num1, dist_num2, mtm_pl_asof_date
 
   open mycursor
   fetch next from mycursor into @dist_num1, @dist_num2, @mtm_pl_asof_date
   ...
   begin
      ...
      from dbo.aud_tid_mtm_correlation aud
      where aud.dist_num1 = @dist_num1 and
            aud.dist_num2 = @dist_num2 and
            aud.mtm_pl_asof_date = @mtm_pl_asof_date and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@dist_num1 as varchar) + '/' + cast(@dist_num2 as varchar) + '/' + convert(varchar, @mtm_pl_asof_date, 101)
      ...
      begin
         ...
         where aud.dist_num1 = @dist_num1 and
               aud.dist_num2 = @dist_num2 and
               aud.mtm_pl_asof_date = @mtm_pl_asof_date
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_tid_mtm_correlation records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_tid_mtm_correlation rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_tid_mtm_correlation rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_tid_mtm_correlation'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_tid_mtm_correlation record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_tid_mtm_correlation records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_tid_mtm_correlation] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_tid_mtm_correlation', NULL, NULL
GO
