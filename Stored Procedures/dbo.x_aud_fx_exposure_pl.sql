...
CREATE procedure [dbo].[x_aud_fx_exposure_pl]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @pl_asof_date       datetime,
   ...
   begin
      ...
      RAISERROR('aud_fx_exposure_pl: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct pl_asof_date, exp_key_type, exp_key_num
      from dbo.aud_fx_exposure_pl
      order by pl_asof_date, exp_key_type, exp_key_num
 
   open mycursor
   fetch next from mycursor into @pl_asof_date, @exp_key_type, @exp_key_num
   ...
   begin
      ...
      from dbo.aud_fx_exposure_pl aud
      where aud.pl_asof_date = @pl_asof_date and
            aud.exp_key_type = @exp_key_type and
            aud.exp_key_num = @exp_key_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = convert(varchar, @pl_asof_date, 101) + '/' + @exp_key_type + '/' + cast(@exp_key_num as varchar)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_fx_exposure_pl records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_fx_exposure_pl rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_fx_exposure_pl rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_fx_exposure_pl'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_fx_exposure_pl record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_fx_exposure_pl records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_fx_exposure_pl] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_fx_exposure_pl', NULL, NULL
GO
