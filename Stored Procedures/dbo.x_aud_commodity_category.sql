...
CREATE procedure [dbo].[x_aud_commodity_category]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @cmdty_category_code char(8),
        @trans_id            int,
        @resp_trans_id       int,
        @cutoffdate          datetime,
        @rows_deleted        int,
   ...
   begin
      ...
      RAISERROR('aud_commodity_category: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct cmdty_category_code
      from dbo.aud_commodity_category
      order by cmdty_category_code
 
   open mycursor
   fetch next from mycursor into @cmdty_category_code
   ...
   begin
      ...
      from dbo.aud_commodity_category aud
      where aud.cmdty_category_code = @cmdty_category_code and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = @cmdty_category_code
      ...
      begin
         ...
         where aud.cmdty_category_code = @cmdty_category_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_commodity_category records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_commodity_category rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_commodity_category rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_commodity_category'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_commodity_category record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_commodity_category records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_commodity_category] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_commodity_category', NULL, NULL
GO
