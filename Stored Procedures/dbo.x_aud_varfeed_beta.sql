...
CREATE procedure [dbo].[x_aud_varfeed_beta]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @commkt_key         int,
   ...
   begin
      ...
      RAISERROR('aud_varfeed_beta: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct commkt_key, price_source_code, val_type
      from dbo.aud_varfeed_beta
      order by commkt_key, price_source_code, val_type
 
   open mycursor
   fetch next from mycursor into @commkt_key, @price_source_code, @val_type
   ...
   begin
      ...
      begin
         ...
         set @tempstr = cast(@commkt_key as varchar) + '/' + @price_source_code + '/' + @val_type
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_varfeed_beta records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_varfeed_beta rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_varfeed_beta rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_varfeed_beta'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_varfeed_beta record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_varfeed_beta records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_varfeed_beta] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_varfeed_beta', NULL, NULL
GO
