...
CREATE procedure [dbo].[x_aud_simple_formula]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @simple_formula_num int,
        @trans_id           int,
        @resp_trans_id      int,
        @cutoffdate         datetime,
        @rows_deleted       int,
   ...
   begin
      ...
      RAISERROR('aud_simple_formula: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct simple_formula_num
      from dbo.aud_simple_formula
      order by simple_formula_num
 
   open mycursor
   fetch next from mycursor into @simple_formula_num
   ...
   begin
      ...
      from dbo.aud_simple_formula aud
      where aud.simple_formula_num = @simple_formula_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@simple_formula_num as varchar)
      ...
      begin
         ...
         where aud.simple_formula_num = @simple_formula_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_simple_formula records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_simple_formula rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_simple_formula rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_simple_formula'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_simple_formula record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_simple_formula records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_simple_formula] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_simple_formula', NULL, NULL
GO
