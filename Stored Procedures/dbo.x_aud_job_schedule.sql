...
CREATE procedure [dbo].[x_aud_job_schedule]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @job_schedule_num   int,
   ...
   begin
      ...
      RAISERROR('aud_job_schedule: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct job_schedule_num
      from dbo.aud_job_schedule
      order by job_schedule_num
 
   open mycursor
   fetch next from mycursor into @job_schedule_num
   ...
   begin
      ...
      from dbo.aud_job_schedule aud
      where aud.job_schedule_num = @job_schedule_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@job_schedule_num as varchar)
      ...
      begin
         ...
         where aud.job_schedule_num = @job_schedule_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_job_schedule records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_job_schedule rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_job_schedule rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_job_schedule'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_job_schedule record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_job_schedule records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_job_schedule] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_job_schedule', NULL, NULL
GO
