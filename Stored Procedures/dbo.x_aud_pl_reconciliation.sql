...
CREATE procedure [dbo].[x_aud_pl_reconciliation]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @pl_reconciliation_num int,
        @trans_id              int,
        @resp_trans_id         int,
        @cutoffdate            datetime,
        @rows_deleted          int,
   ...
   begin
      ...
      RAISERROR('aud_pl_reconciliation: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct pl_reconciliation_num
      from dbo.aud_pl_reconciliation
      order by pl_reconciliation_num
 
   open mycursor
   fetch next from mycursor into @pl_reconciliation_num
   ...
   begin
      ...
      from dbo.aud_pl_reconciliation aud
      where aud.pl_reconciliation_num = @pl_reconciliation_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@pl_reconciliation_num as varchar)
      ...
      begin
         ...
         where aud.pl_reconciliation_num = @pl_reconciliation_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_pl_reconciliation records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_pl_reconciliation rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_pl_reconciliation rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_pl_reconciliation'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_pl_reconciliation record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_pl_reconciliation records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_pl_reconciliation] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_pl_reconciliation', NULL, NULL
GO
