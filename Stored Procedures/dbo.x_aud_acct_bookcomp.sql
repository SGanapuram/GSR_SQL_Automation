...
CREATE procedure [dbo].[x_aud_acct_bookcomp]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @acct_bookcomp_key  int,
   ...
   begin
      ...
      RAISERROR('aud_acct_bookcomp: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct acct_bookcomp_key
      from dbo.aud_acct_bookcomp
   ...
   begin
      ...
      from dbo.aud_acct_bookcomp aud
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_acct_bookcomp records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_acct_bookcomp rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_acct_bookcomp rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_acct_bookcomp'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_acct_bookcomp record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_acct_bookcomp records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_acct_bookcomp] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_acct_bookcomp', NULL, NULL
GO
