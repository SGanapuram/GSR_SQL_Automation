...
CREATE procedure [dbo].[x_aud_risk_cover]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @risk_cover_num     int,
   ...
   begin
      ...
      RAISERROR('aud_risk_cover: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct risk_cover_num
      from dbo.aud_risk_cover
      order by risk_cover_num
 
   open mycursor
   fetch next from mycursor into @risk_cover_num
   ...
   begin
      ...
      from dbo.aud_risk_cover aud
      where aud.risk_cover_num = @risk_cover_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@risk_cover_num as varchar)
      ...
      begin
         ...
         where aud.risk_cover_num = @risk_cover_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_risk_cover records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_risk_cover rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_risk_cover rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_risk_cover'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_risk_cover record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_risk_cover records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_risk_cover] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_risk_cover', NULL, NULL
GO
