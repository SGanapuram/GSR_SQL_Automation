...
CREATE procedure [dbo].[x_aud_calendar_detail]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @calendar_code      char(8),
   ...
   begin
      ...
      RAISERROR('aud_calendar_detail: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct calendar_code, calendar_date
      from dbo.aud_calendar_detail
      order by calendar_code, calendar_date
 
   open mycursor
   fetch next from mycursor into @calendar_code, @calendar_date
   ...
   begin
      ...
      from dbo.aud_calendar_detail aud
      where aud.calendar_code = @calendar_code and
      ...
      begin
         ...
         set @tempstr = @calendar_code + '/' + convert(varchar, @calendar_date, 101)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_calendar_detail records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_calendar_detail rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_calendar_detail rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_calendar_detail'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_calendar_detail record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_calendar_detail records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_calendar_detail] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_calendar_detail', NULL, NULL
GO
