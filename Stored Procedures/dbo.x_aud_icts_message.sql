...
CREATE procedure [dbo].[x_aud_icts_message]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @oid                int,
   ...
   begin
      ...
      RAISERROR('aud_icts_message: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
   ...
   begin
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_icts_message records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_icts_message rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_icts_message rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_icts_message'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_icts_message record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_icts_message records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_icts_message] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_icts_message', NULL, NULL
GO
