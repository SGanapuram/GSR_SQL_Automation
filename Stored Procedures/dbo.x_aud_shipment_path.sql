...
CREATE procedure [dbo].[x_aud_shipment_path]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @shipment_oid       int,
   ...
   begin
      ...
      RAISERROR('aud_shipment_path: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct shipment_oid
      from dbo.aud_shipment_path
      order by shipment_oid
 
   open mycursor
   fetch next from mycursor into @shipment_oid
   ...
   begin
      ...
      from dbo.aud_shipment_path aud
      where aud.shipment_oid = @shipment_oid and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@shipment_oid as varchar)
      ...
      begin
         ...
         where aud.shipment_oid = @shipment_oid
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_shipment_path records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_shipment_path rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_shipment_path rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_shipment_path'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_shipment_path record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_shipment_path records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_shipment_path] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_shipment_path', NULL, NULL
GO
