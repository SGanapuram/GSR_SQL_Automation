...
CREATE procedure [dbo].[x_aud_uom_conversion]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @uom_conv_num       int,
   ...
   begin
      ...
      RAISERROR('aud_uom_conversion: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct uom_conv_num
      from dbo.aud_uom_conversion
      order by uom_conv_num
 
   open mycursor
   fetch next from mycursor into @uom_conv_num
   ...
   begin
      ...
      from dbo.aud_uom_conversion aud
      where aud.uom_conv_num = @uom_conv_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@uom_conv_num as varchar)
      ...
      begin
         ...
         where aud.uom_conv_num = @uom_conv_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_uom_conversion records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_uom_conversion rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_uom_conversion rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_uom_conversion'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_uom_conversion record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_uom_conversion records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_uom_conversion] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_uom_conversion', NULL, NULL
GO
