...
CREATE procedure [dbo].[x_aud_substituted_text]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @alias_source_name  varchar(50),
        @keyword            varchar(50),
        @trans_id           int,
        @resp_trans_id      int,
        @cutoffdate         datetime,
   ...
   begin
      ...
      RAISERROR('aud_substituted_text: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct alias_source_name, keyword
      from dbo.aud_substituted_text
      order by alias_source_name, keyword
 
   open mycursor
   fetch next from mycursor into @alias_source_name, @keyword
   ...
   begin
      ...
      from dbo.aud_substituted_text aud
      where aud.alias_source_name = @alias_source_name and
            aud.keyword = @keyword and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = @alias_source_name + '/' + @keyword
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_substituted_text records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_substituted_text rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_substituted_text rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_substituted_text'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_substituted_text record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_substituted_text records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_substituted_text] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_substituted_text', NULL, NULL
GO
