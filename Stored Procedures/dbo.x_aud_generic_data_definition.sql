...
CREATE procedure [dbo].[x_aud_generic_data_definition]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @gdd_num            int,
   ...
   begin
      ...
      RAISERROR('aud_generic_data_definition: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct gdd_num
      from dbo.aud_generic_data_definition
      order by gdd_num
 
   open mycursor
   fetch next from mycursor into @gdd_num
   ...
   begin
      ...
      from dbo.aud_generic_data_definition aud
      where aud.gdd_num = @gdd_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@gdd_num as varchar)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_generic_data_definition records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_generic_data_definition rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_generic_data_definition rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_generic_data_definition'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_generic_data_definition record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_generic_data_definition records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_generic_data_definition] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_generic_data_definition', NULL, NULL
GO
