...
CREATE procedure [dbo].[x_aud_contract_status]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @contr_status_code  char(8),
        @trans_id           int,
        @resp_trans_id      int,
        @cutoffdate         datetime,
        @rows_deleted       int,
   ...
   begin
      ...
      RAISERROR('aud_contract_status: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct contr_status_code
      from dbo.aud_contract_status
      order by contr_status_code
 
   open mycursor
   fetch next from mycursor into @contr_status_code
   ...
   begin
      ...
      from dbo.aud_contract_status aud
      where aud.contr_status_code = @contr_status_code and
            exists (select 1
      ...
      begin
         ...
         where aud.contr_status_code = @contr_status_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_contract_status records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_contract_status rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_contract_status rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_contract_status'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_contract_status record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_contract_status records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_contract_status] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_contract_status', NULL, NULL
GO
