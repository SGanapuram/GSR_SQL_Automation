...
CREATE procedure [dbo].[x_aud_pipeline_cycle]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @pipeline_cycle_num int,
        @trans_id           int,
        @resp_trans_id      int,
        @cutoffdate         datetime,
        @rows_deleted       int,
   ...
   begin
      ...
      RAISERROR('aud_pipeline_cycle: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct pipeline_cycle_num
      from dbo.aud_pipeline_cycle
      order by pipeline_cycle_num
 
   open mycursor
   fetch next from mycursor into @pipeline_cycle_num
   ...
   begin
      ...
      from dbo.aud_pipeline_cycle aud
      where aud.pipeline_cycle_num = @pipeline_cycle_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@pipeline_cycle_num as varchar)
      ...
      begin
         ...
         where aud.pipeline_cycle_num = @pipeline_cycle_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_pipeline_cycle records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_pipeline_cycle rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_pipeline_cycle rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_pipeline_cycle'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_pipeline_cycle record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_pipeline_cycle records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_pipeline_cycle] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_pipeline_cycle', NULL, NULL
GO
