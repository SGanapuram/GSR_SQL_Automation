...
CREATE procedure [dbo].[x_aud_option_strike]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @commkt_key         int,
   ...
   begin
      ...
      RAISERROR('aud_option_strike: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct commkt_key, trading_prd, opt_strike_price, put_call_ind
      from dbo.aud_option_strike
      order by commkt_key, trading_prd, opt_strike_price, put_call_ind
 
   open mycursor
   fetch next from mycursor into @commkt_key, @trading_prd, @opt_strike_price, @put_call_ind
   ...
   begin
      ...
      from dbo.aud_option_strike aud
      ...
      begin
         ...
         set @tempstr = cast(@commkt_key as varchar) + '/' + @trading_prd + '/' + rtrim(ltrim(str(@opt_strike_price, 20, 8))) + '/' + @put_call_ind
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_option_strike records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_option_strike rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_option_strike rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_option_strike'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_option_strike record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_option_strike records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_option_strike] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_option_strike', NULL, NULL
GO
