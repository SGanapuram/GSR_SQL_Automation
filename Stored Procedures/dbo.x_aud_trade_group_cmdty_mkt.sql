...
CREATE procedure [dbo].[x_aud_trade_group_cmdty_mkt]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @trade_group_num    int,
   ...
   begin
      ...
      RAISERROR('aud_trade_group_cmdty_mkt: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct trade_group_num, commkt_key
      from dbo.aud_trade_group_cmdty_mkt
      order by trade_group_num, commkt_key
 
   open mycursor
   fetch next from mycursor into @trade_group_num, @commkt_key
   ...
   begin
      ...
      from dbo.aud_trade_group_cmdty_mkt aud
      where aud.trade_group_num = @trade_group_num and
      ...
      begin
         ...
         set @tempstr = cast(@trade_group_num as varchar) + '/' + cast(@commkt_key as varchar)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_trade_group_cmdty_mkt records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_trade_group_cmdty_mkt rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_trade_group_cmdty_mkt rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_trade_group_cmdty_mkt'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_trade_group_cmdty_mkt record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_trade_group_cmdty_mkt records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_trade_group_cmdty_mkt] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_trade_group_cmdty_mkt', NULL, NULL
GO
