...
CREATE procedure [dbo].[inhouse_search]
(
...
declare @status int
   ...
   if (@result > 0) 
   begin
      /* delete all those trades which don't have matching 
         trader init to either buy or sell side */
    ...
    begin
       /* delete all those trades which don't have matching 
          portfolio to either buy or sell side */
       delete from #inhouse_search
...
GO
GRANT EXECUTE ON  [dbo].[inhouse_search] TO [next_usr]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'inhouse_search', NULL, NULL
GO
