...
CREATE procedure [dbo].[usp_get_primary_cost_invoice_data]
(
   @voucherNum    int
)
as
set nocount on
select	bc.acct_full_name						AS BookingCompFullName,
...
(CASE WHEN ship.oid IS NULL THEN '' ELSE    
  convert(VARCHAR,ship.oid) END)         							AS ShipmentOidForCost,    
(CASE WHEN prcl.oid IS NULL THEN '' ELSE    
 convert(VARCHAR,prcl.oid) END)         							AS ParcelOidForCost,    
(CASE WHEN c.cost_owner_key3 IS NULL THEN '' ELSE    
convert(VARCHAR,c.cost_owner_key3) END)        						AS ActualNumStrForCost,    
...
ON ship.alloc_num = c.cost_owner_key1  AND (c.cost_owner_code IN('AI','AA','A'))  
LEFT OUTER JOIN parcel prcl    
on prcl.alloc_num=c.cost_owner_key1 and prcl.alloc_item_num=c.cost_owner_key2  AND (c.cost_owner_code IN('AI','AA','A'))  
LEFT OUTER JOIN ai_est_actual actual    
on actual.alloc_num=c.cost_owner_key1 and actual.alloc_item_num=c.cost_owner_key2 AND actual.ai_est_actual_num =c.cost_owner_key3  AND (c.cost_owner_code IN('AI','AA','A'))  
...
where v.voucher_num = @voucherNum   AND (c.cost_type_code IN('WPP','PR','PO') OR (c.cost_type_code='WO' AND c.cost_owner_code = 'V'))
GO
GRANT EXECUTE ON  [dbo].[usp_get_primary_cost_invoice_data] TO [next_usr]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'usp_get_primary_cost_invoice_data', NULL, NULL
GO
