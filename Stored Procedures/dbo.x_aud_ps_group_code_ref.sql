...
CREATE procedure [dbo].[x_aud_ps_group_code_ref]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @purchase_sale_group_code char(15),
        @trans_id                 int,
        @resp_trans_id            int,
        @cutoffdate               datetime,
        @rows_deleted             int,
   ...
   begin
      ...
      RAISERROR('aud_ps_group_code_ref: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct purchase_sale_group_code
      from dbo.aud_ps_group_code_ref
      order by purchase_sale_group_code
 
   open mycursor
   fetch next from mycursor into @purchase_sale_group_code
   ...
   begin
      ...
      from dbo.aud_ps_group_code_ref aud
      where aud.purchase_sale_group_code = @purchase_sale_group_code and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = @purchase_sale_group_code
      ...
      begin
         ...
         where aud.purchase_sale_group_code = @purchase_sale_group_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_ps_group_code_ref records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_ps_group_code_ref rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_ps_group_code_ref rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_ps_group_code_ref'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_ps_group_code_ref record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_ps_group_code_ref records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_ps_group_code_ref] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_ps_group_code_ref', NULL, NULL
GO
