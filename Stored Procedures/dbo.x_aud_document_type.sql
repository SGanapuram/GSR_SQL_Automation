...
CREATE procedure [dbo].[x_aud_document_type]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @doc_type_code      char(1),
   ...
   begin
      ...
      RAISERROR('aud_document_type: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct doc_type_code
      from dbo.aud_document_type
      order by doc_type_code
 
   open mycursor
   fetch next from mycursor into @doc_type_code
   ...
   begin
      ...
      from dbo.aud_document_type aud
      where aud.doc_type_code = @doc_type_code and
            exists (select 1
      ...
      begin
         ...
         where aud.doc_type_code = @doc_type_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_document_type records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_document_type rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_document_type rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_document_type'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_document_type record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_document_type records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_document_type] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_document_type', NULL, NULL
GO
