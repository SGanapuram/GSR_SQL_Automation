...
CREATE procedure [dbo].[x_aud_inv_pricing_period]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @inv_num            int,
   ...
   begin
      ...
      RAISERROR('aud_inv_pricing_period: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct inv_num
      from dbo.aud_inv_pricing_period
      order by inv_num
 
   open mycursor
   fetch next from mycursor into @inv_num
   ...
   begin
      ...
      from dbo.aud_inv_pricing_period aud
      ...
      begin
         ...
         set @tempstr = cast(@inv_num as varchar)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_inv_pricing_period records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_inv_pricing_period rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_inv_pricing_period rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_inv_pricing_period'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_inv_pricing_period record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_inv_pricing_period records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_inv_pricing_period] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_inv_pricing_period', NULL, NULL
GO
