...
as
set nocount on
declare @my_after_asof_date     datetime,
        @my_before_asof_date    datetime,
   ...
   set @my_before_asof_date = @before_asof_date
   if @max_years_back > 0
   begin
      set @number_years_back = -1 * @max_years_back
      set @my_after_asof_date = dateadd(year, @number_years_back, convert(datetime, @my_before_asof_date))
      ...
      where exists (select 1
                    ...
                    where tidmtm.dist_num = tid.dist_num and           
                          ...
                          tid.bus_date <= @my_before_asof_date) and					  
            tidmtm.mtm_pl_asof_date between @my_after_asof_date and @my_before_asof_date
   ...
   begin
      ...
      where exists (select 1
                    ...
                    where tidmtm.dist_num = tid.dist_num and           
					      tid.is_equiv_ind = 'Y' and 
                          tid.bus_date <= @my_before_asof_date) and					  
            tidmtm.mtm_pl_asof_date <= @my_before_asof_date
      group by tidmtm.dist_num
	  OPTION (MAXDOP 12)
   end
