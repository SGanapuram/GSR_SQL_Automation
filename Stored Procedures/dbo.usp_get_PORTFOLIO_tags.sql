...
CREATE procedure [dbo].[usp_get_PORTFOLIO_tags]
(
   @debugon          bit = 0
)                             
as
...
declare @status                       int,
   ...
   set @status = 0
   if object_id('dbo.xx101_porttags', 'U') is not null
      exec('drop table dbo.xx101_porttags')   
   ...
   begin catch
     ...
     exec dbo.usp_save_dashboard_error @report_name = 'PlCompReport',  
                                       @occurred_at = 'SP: usp_get_PORTFOLIO_tags', 
                                       @problem_desc = @smsg,                                           
   ...
   set @sql = 'alter table #xx101_porttags add '
   set @tag_column_list = ''
   ...
   begin
      select @tag_name = tag_name + '(PortfolioTAG)'
      from #tagnames
      where oid = @oid
   ...
   -- alter temp table to add columns for portfolio tags
   -- DEBUG: print '@sql = ''' + @sql + ''''
   begin try
     exec(@sql)
   end try
   begin catch
     set @smsg = 'Failed to alter temp table ''#xx101_porttags'''  
     set @tempstr = '=> ' + @smsg + ' due to the error:'
     RAISERROR(@tempstr, 0, 1) WITH NOWAIT
     set @dberrmsg = ERROR_MESSAGE()                                                
     set @tempstr = '==> ERROR: ' + @dberrmsg                                                  
   ...
   insert into #tag_column_info
      values('Portfolio', @tag_column_list)
       
   begin try
     insert into #xx101_porttags
       exec dbo.usp_get_pivot_output_for_an_entity 'Portfolio', 
                                                   'cast(key1 as int) as port_num', 
                                                   'port_num'
   end try
   begin catch
     set @smsg = 'Failed to save data returned from the ''usp_get_pivot_output_for_an_entity'' procedure to the ''#xx101_porttags'' table'  
     set @tempstr = '=> ' + @smsg + ' due to the error:'
     RAISERROR(@tempstr, 0, 1) WITH NOWAIT
     set @dberrmsg = ERROR_MESSAGE()                                                
     set @tempstr = '==> ERROR: ' + @dberrmsg                                                  
     ...
     exec dbo.usp_save_dashboard_error @report_name = 'PlCompReport',  
                                       ...
                                       @problem_desc = @smsg,                                           
                                       @dberror_msg = @dberrmsg,
                                       @sql_stmt = 'insert into #xx101_porttags exec dbo.usp_get_pivot_output_for_an_entity ..',
                                       @debugon = @debugon                                                                                                    
     goto endofsp
   end catch 
   ...
   begin
      ...
      begin
         set @smsg = 'This reference Inspector ''' + @ref_insp_name + ''' is not currently supported by this version of the ''usp_get_PORTFOLIO_tags'' sp'
   ...
   begin
      select @ref_insp_name = ref_insp_name,
             @tag_name = tag_name + '(PortfolioTAG)'
      from #tagnames
      where oid = @oid
      ...
      begin
         ...
         set @sql = @sql + 'from #xx101_porttags tag left outer join dbo.icts_user u '
      ...
      begin
         ...
         set @sql = @sql + 'from #xx101_porttags tag left outer join dbo.account u '
      ...
      begin catch
        set @smsg = 'Failed to update the ''' + @tag_name + ''' column in the ''#xx101_porttags'' table'  
        set @tempstr = '=> ' + @smsg + ' due to the error:'
        RAISERROR(@tempstr, 0, 1) WITH NOWAIT
        set @dberrmsg = ERROR_MESSAGE()                                                
        set @tempstr = '==> ERROR: ' + @dberrmsg                                                  
...
return @status
GO
GRANT EXECUTE ON  [dbo].[usp_get_PORTFOLIO_tags] TO [next_usr]
GO
