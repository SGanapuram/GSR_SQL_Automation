...
CREATE procedure [dbo].[x_aud_facility_tank_group]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @facility_code      char(8),
   ...
   begin
      ...
      RAISERROR('aud_facility_tank_group: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct facility_code, tank_num
      from dbo.aud_facility_tank_group
      order by facility_code, tank_num
 
   open mycursor
   fetch next from mycursor into @facility_code, @tank_num
   ...
   begin
      ...
      from dbo.aud_facility_tank_group aud
      where aud.facility_code = @facility_code and
            aud.tank_num = @tank_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = @facility_code + '/' + cast(@tank_num as varchar)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_facility_tank_group records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_facility_tank_group rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_facility_tank_group rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_facility_tank_group'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_facility_tank_group record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_facility_tank_group records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_facility_tank_group] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_facility_tank_group', NULL, NULL
GO
