...
CREATE procedure [dbo].[x_aud_broker_cost_step_price]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @cost_autogen_num   int,
   ...
   begin
      ...
      RAISERROR('aud_broker_cost_step_price: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct cost_autogen_num, step_price_num
      from dbo.aud_broker_cost_step_price
      order by cost_autogen_num, step_price_num
 
   open mycursor
   fetch next from mycursor into @cost_autogen_num, @step_price_num
   ...
   begin
      ...
      from dbo.aud_broker_cost_step_price aud
      where aud.cost_autogen_num = @cost_autogen_num and
            aud.step_price_num = @step_price_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@cost_autogen_num as varchar) + '/' + cast(@step_price_num as varchar)
      ...
      begin
         ...
         where aud.cost_autogen_num = @cost_autogen_num and
               aud.step_price_num = @step_price_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_broker_cost_step_price records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_broker_cost_step_price rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_broker_cost_step_price rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_broker_cost_step_price'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_broker_cost_step_price record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_broker_cost_step_price records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_broker_cost_step_price] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_broker_cost_step_price', NULL, NULL
GO
