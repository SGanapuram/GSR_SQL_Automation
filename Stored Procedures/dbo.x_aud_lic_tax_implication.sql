...
CREATE procedure [dbo].[x_aud_lic_tax_implication]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @license_num        int,
        @license_covers_num int,
        @tax_code           char(8),
        @trans_id           int,
        @resp_trans_id      int,
        @cutoffdate         datetime,
   ...
   begin
      ...
      RAISERROR('aud_lic_tax_implication: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct license_num, license_covers_num, tax_code
      from dbo.aud_lic_tax_implication
      order by license_num, license_covers_num, tax_code
 
   open mycursor
   fetch next from mycursor into @license_num, @license_covers_num, @tax_code
   ...
   begin
      ...
      from dbo.aud_lic_tax_implication aud
      where aud.license_num = @license_num and
            aud.license_covers_num = @license_covers_num and
            aud.tax_code = @tax_code and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@license_num as varchar) + '/' + cast(@license_covers_num as varchar) + '/' + @tax_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_lic_tax_implication records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_lic_tax_implication rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_lic_tax_implication rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_lic_tax_implication'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_lic_tax_implication record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_lic_tax_implication records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_lic_tax_implication] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_lic_tax_implication', NULL, NULL
GO
