...
CREATE procedure [dbo].[x_aud_trade_default]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @dflt_num           int,
   ...
   begin
      ...
      RAISERROR('aud_trade_default: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct dflt_num
      from dbo.aud_trade_default
      order by dflt_num
 
   open mycursor
   fetch next from mycursor into @dflt_num
   ...
   begin
      ...
      from dbo.aud_trade_default aud
      where aud.dflt_num = @dflt_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@dflt_num as varchar)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_trade_default records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_trade_default rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_trade_default rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_trade_default'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_trade_default record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_trade_default records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_trade_default] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_trade_default', NULL, NULL
GO
