...
CREATE procedure [dbo].[x_aud_margin_call]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @margin_call_num    int,
   ...
   begin
      ...
      RAISERROR('aud_margin_call: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct margin_call_num
      from dbo.aud_margin_call
      order by margin_call_num
 
   open mycursor
   fetch next from mycursor into @margin_call_num
   ...
   begin
      ...
      from dbo.aud_margin_call aud
      where aud.margin_call_num = @margin_call_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@margin_call_num as varchar)
      ...
      begin
         ...
         where aud.margin_call_num = @margin_call_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_margin_call records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_margin_call rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_margin_call rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_margin_call'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_margin_call record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_margin_call records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_margin_call] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_margin_call', NULL, NULL
GO
