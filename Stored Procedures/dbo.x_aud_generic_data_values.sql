...
CREATE procedure [dbo].[x_aud_generic_data_values]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @gdv_num            int,
   ...
   begin
      ...
      RAISERROR('aud_generic_data_values: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct gdv_num, gdd_num
      from dbo.aud_generic_data_values
      order by gdv_num, gdd_num
 
   open mycursor
   fetch next from mycursor into @gdv_num, @gdd_num
   ...
   begin
      ...
      from dbo.aud_generic_data_values aud
      where aud.gdv_num = @gdv_num and
            aud.gdd_num = @gdd_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@gdv_num as varchar) + '/' + cast(@gdd_num as varchar)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_generic_data_values records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_generic_data_values rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_generic_data_values rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_generic_data_values'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_generic_data_values record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_generic_data_values records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_generic_data_values] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_generic_data_values', NULL, NULL
GO
