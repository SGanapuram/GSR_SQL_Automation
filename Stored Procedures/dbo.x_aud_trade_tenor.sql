...
CREATE procedure [dbo].[x_aud_trade_tenor]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @tenor_code         char(4),
   ...
   begin
      ...
      RAISERROR('aud_trade_tenor: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct tenor_code
      from dbo.aud_trade_tenor
      order by tenor_code
 
   open mycursor
   fetch next from mycursor into @tenor_code
   ...
   begin
      ...
      from dbo.aud_trade_tenor aud
      where aud.tenor_code = @tenor_code and
            exists (select 1
      ...
      begin
         ...
         where aud.tenor_code = @tenor_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_trade_tenor records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_trade_tenor rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_trade_tenor rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_trade_tenor'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_trade_tenor record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_trade_tenor records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_trade_tenor] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_trade_tenor', NULL, NULL
GO
