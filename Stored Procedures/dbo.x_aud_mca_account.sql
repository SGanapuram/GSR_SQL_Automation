...
CREATE procedure [dbo].[x_aud_mca_account]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @mca_num            int,
   ...
   begin
      ...
      RAISERROR('aud_mca_account: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct mca_num, coll_party_num, acct_num
      from dbo.aud_mca_account
      order by mca_num, coll_party_num, acct_num
 
   open mycursor
   fetch next from mycursor into @mca_num, @coll_party_num, @acct_num
   ...
   begin
      ...
      from dbo.aud_mca_account aud
      where aud.mca_num = @mca_num and
            aud.coll_party_num = @coll_party_num and
      ...
      begin
         ...
         set @tempstr = cast(@mca_num as varchar) + '/' + cast(@coll_party_num as varchar) + '/' + cast(@acct_num as varchar)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_mca_account records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_mca_account rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_mca_account rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_mca_account'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_mca_account record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_mca_account records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_mca_account] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_mca_account', NULL, NULL
GO
