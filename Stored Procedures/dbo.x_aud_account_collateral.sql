...
CREATE procedure [dbo].[x_aud_account_collateral]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @acct_num             int,
   ...
   begin
      ...
      RAISERROR('aud_account_collateral: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct acct_num, collateral_type_code
      from dbo.aud_account_collateral
      order by acct_num, collateral_type_code
 
   open mycursor
   fetch next from mycursor into @acct_num, @collateral_type_code
   ...
   begin
      ...
      from dbo.aud_account_collateral aud
      where aud.acct_num = @acct_num and
            aud.collateral_type_code = @collateral_type_code and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@acct_num as varchar) + '/' + @collateral_type_code
      ...
      begin
         ...
         where aud.acct_num = @acct_num and
               aud.collateral_type_code = @collateral_type_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_account_collateral records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_account_collateral rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_account_collateral rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_account_collateral'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_account_collateral record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_account_collateral records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_account_collateral] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_account_collateral', NULL, NULL
GO
