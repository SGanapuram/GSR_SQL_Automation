...
union all  
   ...
   'Forex',   
    ...
    case when isnull(cost_paid_date,cost_due_date)>= convert(char,getdate(),101) then convert(varchar,datepart(yyyy,isnull(cost_paid_date,cost_due_date)))                                              
...
where abs(c.cost_amt) >= 0.001
and cost_type_code in ('CPR','CPP')                                                            
and cost_status in ('VOUCHED','OPEN')   
and c.port_num in (select port_num from #children)                                                            
and exists (select 1 from cost_ext_info cei where isnull(fx_exp_num,0) =0 and cei.cost_num=c.cost_num) and @includePastDueCurrCosts='Y'
  end try  
  begin catch  
