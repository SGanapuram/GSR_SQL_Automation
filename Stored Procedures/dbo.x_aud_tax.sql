...
CREATE procedure [dbo].[x_aud_tax]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @tax_num            int,
   ...
   begin
      ...
      RAISERROR('aud_tax: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct tax_num
      from dbo.aud_tax
      order by tax_num
 
   open mycursor
   fetch next from mycursor into @tax_num
   ...
   begin
      ...
      from dbo.aud_tax aud
      where aud.tax_num = @tax_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@tax_num as varchar)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_tax records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_tax rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_tax rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_tax'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_tax record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_tax records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_tax] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_tax', NULL, NULL
GO
