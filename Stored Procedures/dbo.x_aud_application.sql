...
CREATE procedure [dbo].[x_aud_application]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @app_name           varchar(20),
   ...
   begin
      ...
      RAISERROR('aud_application: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct app_name
      from dbo.aud_application
      order by app_name
 
   open mycursor
   fetch next from mycursor into @app_name
   ...
   begin
      ...
      from dbo.aud_application aud
      where aud.app_name = @app_name and
            exists (select 1
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_application records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_application rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_application rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_application'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_application record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_application records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_application] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_application', NULL, NULL
GO
