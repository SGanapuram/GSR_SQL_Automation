...
CREATE procedure [dbo].[x_aud_tax_location]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @tax_authority_num  int,
   ...
   begin
      ...
      RAISERROR('aud_tax_location: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct tax_authority_num, loc_code
      from dbo.aud_tax_location
      order by tax_authority_num, loc_code
 
   open mycursor
   fetch next from mycursor into @tax_authority_num, @loc_code
   ...
   begin
      ...
      from dbo.aud_tax_location aud
      where aud.tax_authority_num = @tax_authority_num and
      ...
      begin
         ...
         set @tempstr = cast(@tax_authority_num as varchar) + '/' + @loc_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_tax_location records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_tax_location rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_tax_location rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_tax_location'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_tax_location record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_tax_location records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_tax_location] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_tax_location', NULL, NULL
GO
