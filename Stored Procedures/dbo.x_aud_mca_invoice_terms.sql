...
CREATE procedure [dbo].[x_aud_mca_invoice_terms]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @invoice_num        int,
   ...
   begin
      ...
      RAISERROR('aud_mca_invoice_terms: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct invoice_num
      from dbo.aud_mca_invoice_terms
      order by invoice_num
 
   open mycursor
   fetch next from mycursor into @invoice_num
   ...
   begin
      ...
      from dbo.aud_mca_invoice_terms aud
      where aud.invoice_num = @invoice_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@invoice_num as varchar)
      ...
      begin
         ...
         where aud.invoice_num = @invoice_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_mca_invoice_terms records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_mca_invoice_terms rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_mca_invoice_terms rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_mca_invoice_terms'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_mca_invoice_terms record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_mca_invoice_terms records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_mca_invoice_terms] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_mca_invoice_terms', NULL, NULL
GO
