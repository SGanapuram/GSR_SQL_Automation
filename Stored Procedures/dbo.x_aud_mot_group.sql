...
CREATE procedure [dbo].[x_aud_mot_group]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @parent_mot_code    char(8),
        @mot_code           char(8),
        @trans_id           int,
        @resp_trans_id      int,
        @cutoffdate         datetime,
   ...
   begin
      ...
      RAISERROR('aud_mot_group: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct parent_mot_code, mot_code
      from dbo.aud_mot_group
      order by parent_mot_code, mot_code
 
   open mycursor
   fetch next from mycursor into @parent_mot_code, @mot_code
   ...
   begin
      ...
      from dbo.aud_mot_group aud
      where aud.parent_mot_code = @parent_mot_code and
      ...
      begin
         ...
         set @tempstr = @parent_mot_code + '/' + @mot_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_mot_group records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_mot_group rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_mot_group rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_mot_group'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_mot_group record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_mot_group records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_mot_group] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_mot_group', NULL, NULL
GO
