...
set nocount on        
 ...
 -- pick expired futures for which the details do not exist in one of the runs      
  ...
  into #plHist_EF_sum       
  from #plhist1 a      
  where pl_owner_code='EF' and a.resp_trans_id=@resp_trans_id_NEW      
  ...
  and a.real_port_num=b.real_port_num and  b.resp_trans_id=@resp_trans_id_OLD      
   and pl_owner_code='EF')      
      
      
insert into #plHist_EF_sum      
  select *       
  from #plhist1 a      
  where pl_owner_code='EF' and a.resp_trans_id=@resp_trans_id_OLD      
  and not exists (select 1 from #plhist1 b where a.pos_num=b.pos_num       
...
-- pick all the tid records for which expired future exists in one of the runs      
  ...
  from #plhist1      
  where ((pl_owner_code='T' and pl_type!='W') or pl_owner_code='C')      
  and exists (select pos_num from #plhist1 where pl_owner_code='EF')      
      
--- compare EF, M against sum for T, U      
--- compare EF, L against sum for C      
--- compare EF, C against sum for T, C      
--- compare EF, R against sum for T, R with Amt <0.0      
--- compare EF, U against sum for T, R with Amt > 0.0      
        
  create table #plHist_EF_sumFromTID (pos_num int, real_port_num int, pl_owner_code char(8), pl_type char(8), sumPlAmt float)      
        
        
  insert into #plHist_EF_sumFromTID      
  select pos_num, real_port_num, pl_owner_code, 'M', SUM(convert(float, pl_amt))      
  from #plHist_EF_detailFromTID where pl_owner_code='T' and pl_type='U'      
  group by pos_num, real_port_num, pl_owner_code      
        
  insert into #plHist_EF_sumFromTID      
  select pos_num, real_port_num, pl_owner_code, 'C', SUM(convert(float, pl_amt))      
  from #plHist_EF_detailFromTID where pl_owner_code='T' and pl_type='C'  and CONVERT(float, pl_amt) <0.0      
  group by pos_num, real_port_num, pl_owner_code      
  ...
  from #plHist_EF_detailFromTID where pl_owner_code='T' and pl_type='R' and CONVERT(float, pl_amt) <0.0      
  group by pos_num, real_port_num, pl_owner_code      
  ...
  from #plHist_EF_detailFromTID where pl_owner_code='C'      
  group by pos_num, real_port_num, pl_owner_code      
        
--- compare EF, M against sum for T, U       
select 'Expired Future', ef_Sum.pos_num, ef_Sum.pl_type, ef_Sum.pl_amt as 'EF_Sum', tid_sum.sumPlAmt as 'TID_Sum', ef_Sum.pl_amt-tid_sum.sumPlAmt as 'Diff' from       
#plHist_EF_sum ef_Sum      
inner join #plHist_EF_sumFromTID tid_sum on ef_Sum.pos_num=tid_sum.pos_num and ef_Sum.pl_type=tid_sum.pl_type      
