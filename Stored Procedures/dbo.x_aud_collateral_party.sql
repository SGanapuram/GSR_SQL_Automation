...
CREATE procedure [dbo].[x_aud_collateral_party]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @coll_party_num     int,
   ...
   begin
      ...
      RAISERROR('aud_collateral_party: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct coll_party_num
      from dbo.aud_collateral_party
      order by coll_party_num
 
   open mycursor
   fetch next from mycursor into @coll_party_num
   ...
   begin
      ...
      from dbo.aud_collateral_party aud
      where aud.coll_party_num = @coll_party_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@coll_party_num as varchar)
      ...
      begin
         ...
         where aud.coll_party_num = @coll_party_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_collateral_party records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_collateral_party rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_collateral_party rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_collateral_party'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_collateral_party record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_collateral_party records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_collateral_party] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_collateral_party', NULL, NULL
GO
