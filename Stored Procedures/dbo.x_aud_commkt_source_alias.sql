...
CREATE procedure [dbo].[x_aud_commkt_source_alias]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @commkt_key         int,
   ...
   begin
      ...
      from dbo.aud_commkt_source_alias aud
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_commkt_source_alias records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_commkt_source_alias rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_commkt_source_alias rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_commkt_source_alias'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_commkt_source_alias record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_commkt_source_alias records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_commkt_source_alias] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_commkt_source_alias', NULL, NULL
GO
