...
(            
   ...
   --below for ADSO-4248
   @low_trans_enabled	bit = 0,
   @low_trans			bit = 0,
   @storage_id			varchar(40)=null   
...
select @trans_id, 'U', @init, GETDATE(), 'usp_CreateStorageAlloc', null, @@SPID, @workstation_name,null, @executor_id                                 
                  
   /* ****************************************************************** */            
   /* Getting a new trade_num */  
   
   exec get_new_num 'trade_num',0   
   select @trade_num = last_num from dbo.new_num where num_col_name = 'trade_num'              
   ...
   begin try            
     insert into dbo.trade             
       ...
       select @trade_num,                 
              ...
              contr_status_code,                  
              1,  
			  case when @low_trans_enabled = 1 and @low_trans = 1 then 'LOWTRAN' else null end,
              is_long_term_ind,                  
              @trans_id                  
       from dbo.trade                   
   ...
   begin try                  
     insert into dbo.trade_order                  
       ...
       select @trade_num,                  
              ...
              'N',                  
              bal_ind,
			  case when @low_trans_enabled = 1 and @storage_id is not null then @storage_id else null END,
              2,               
              @trans_id                  
   ...
   begin try                        
     insert into dbo.trade_item                  
      ...
      select @trade_num,                  
             ...
             p.risk_mkt_code,                  
             p.title_mkt_code,                  
            case when @low_trans_enabled = 1 and @low_trans = 1 then null else p.trading_prd end,                  
     ...
     insert into dbo.trade_item_storage                  
        (trade_num,                  
         ...
         select @trade_num,                  
             1,                  
             1,                  
             case when @low_trans_enabled = 1 and @low_trans = 1 then null else w.del_date_from end,                  
             case when @low_trans_enabled = 1 and @low_trans = 1 then null else w.del_date_to end,                  
             w.del_loc_code,                  
             'STORAGE',                  
             w.pay_days,                  
             w.pay_term_code,                  
