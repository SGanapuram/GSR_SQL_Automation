...
CREATE procedure [dbo].[x_aud_posting_account]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @posting_account_num int,
        @trans_id            int,
        @resp_trans_id       int,
        @cutoffdate          datetime,
        @rows_deleted        int,
   ...
   begin
      ...
      RAISERROR('aud_posting_account: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct posting_account_num
      from dbo.aud_posting_account
      order by posting_account_num
 
   open mycursor
   fetch next from mycursor into @posting_account_num
   ...
   begin
      ...
      from dbo.aud_posting_account aud
      where aud.posting_account_num = @posting_account_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@posting_account_num as varchar)
      ...
      begin
         ...
         where aud.posting_account_num = @posting_account_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_posting_account records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_posting_account rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_posting_account rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_posting_account'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_posting_account record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_posting_account records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_posting_account] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_posting_account', NULL, NULL
GO
