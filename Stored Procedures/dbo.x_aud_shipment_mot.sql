...
CREATE procedure [dbo].[x_aud_shipment_mot]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @shipment_num       int,
   ...
   begin
      ...
      RAISERROR('aud_shipment_mot: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct shipment_num, mot_code
      from dbo.aud_shipment_mot
      order by shipment_num, mot_code
 
   open mycursor
   fetch next from mycursor into @shipment_num, @mot_code
   ...
   begin
      ...
      begin
         ...
         set @tempstr = cast(@shipment_num as varchar) + '/' + @mot_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_shipment_mot records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_shipment_mot rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_shipment_mot rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_shipment_mot'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_shipment_mot record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_shipment_mot records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_shipment_mot] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_shipment_mot', NULL, NULL
GO
