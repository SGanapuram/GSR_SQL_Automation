...
CREATE procedure [dbo].[x_aud_specification_group]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @spec_group_code    char(8),
        @trans_id           int,
        @resp_trans_id      int,
        @cutoffdate         datetime,
        @rows_deleted       int,
   ...
   begin
      ...
      RAISERROR('aud_specification_group: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct spec_group_code
      from dbo.aud_specification_group
      order by spec_group_code
 
   open mycursor
   fetch next from mycursor into @spec_group_code
   ...
   begin
      ...
      from dbo.aud_specification_group aud
      where aud.spec_group_code = @spec_group_code and
            exists (select 1
      ...
      begin
         ...
         where aud.spec_group_code = @spec_group_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_specification_group records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_specification_group rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_specification_group rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_specification_group'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_specification_group record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_specification_group records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_specification_group] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_specification_group', NULL, NULL
GO
