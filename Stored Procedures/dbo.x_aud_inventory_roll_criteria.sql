...
CREATE procedure [dbo].[x_aud_inventory_roll_criteria]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @roll_criteria_num  int,
   ...
   begin
      ...
      RAISERROR('aud_inventory_roll_criteria: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct roll_criteria_num
      from dbo.aud_inventory_roll_criteria
      order by roll_criteria_num
 
   open mycursor
   fetch next from mycursor into @roll_criteria_num
   ...
   begin
      ...
      from dbo.aud_inventory_roll_criteria aud
      where aud.roll_criteria_num = @roll_criteria_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@roll_criteria_num as varchar)
      ...
      begin
         ...
         where aud.roll_criteria_num = @roll_criteria_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_inventory_roll_criteria records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_inventory_roll_criteria rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_inventory_roll_criteria rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_inventory_roll_criteria'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_inventory_roll_criteria record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_inventory_roll_criteria records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_inventory_roll_criteria] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_inventory_roll_criteria', NULL, NULL
GO
