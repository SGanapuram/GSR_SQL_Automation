...
CREATE procedure [dbo].[x_aud_mca_mat_adv_chg_clause]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @mca_num            int,
   ...
   begin
      ...
      RAISERROR('aud_mca_mat_adv_chg_clause: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct mca_num, macc_num
      from dbo.aud_mca_mat_adv_chg_clause
      order by mca_num, macc_num
 
   open mycursor
   fetch next from mycursor into @mca_num, @macc_num
   ...
   begin
      ...
      from dbo.aud_mca_mat_adv_chg_clause aud
      where aud.mca_num = @mca_num and
            aud.macc_num = @macc_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@mca_num as varchar) + '/' + cast(@macc_num as varchar)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_mca_mat_adv_chg_clause records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_mca_mat_adv_chg_clause rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_mca_mat_adv_chg_clause rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_mca_mat_adv_chg_clause'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_mca_mat_adv_chg_clause record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_mca_mat_adv_chg_clause records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_mca_mat_adv_chg_clause] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_mca_mat_adv_chg_clause', NULL, NULL
GO
