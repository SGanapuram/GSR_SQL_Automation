...
CREATE procedure [dbo].[x_aud_account_type]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @acct_type_code     char(8),
   ...
   begin
      ...
      RAISERROR('aud_account_type: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct acct_type_code
      from dbo.aud_account_type
      order by acct_type_code
 
   open mycursor
   fetch next from mycursor into @acct_type_code
   ...
   begin
      ...
      begin
         ...
         where aud.acct_type_code = @acct_type_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_account_type records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_account_type rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_account_type rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_account_type'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_account_type record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_account_type records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_account_type] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_account_type', NULL, NULL
GO
