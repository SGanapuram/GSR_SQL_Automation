...
CREATE procedure [dbo].[x_aud_cost_code]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @cost_code          char(8),
   ...
   begin
      ...
      RAISERROR('aud_cost_code: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct cost_code
      from dbo.aud_cost_code
      order by cost_code
 
   open mycursor
   fetch next from mycursor into @cost_code
   ...
   begin
      ...
      from dbo.aud_cost_code aud
      where aud.cost_code = @cost_code and
            exists (select 1
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_cost_code records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_cost_code rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_cost_code rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_cost_code'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_cost_code record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_cost_code records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_cost_code] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_cost_code', NULL, NULL
GO
