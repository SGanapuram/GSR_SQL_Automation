...
CREATE procedure [dbo].[x_aud_price_change]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @price_change_num   int,
   ...
   begin
      ...
      RAISERROR('aud_price_change: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct price_change_num
      from dbo.aud_price_change
      order by price_change_num
 
   open mycursor
   fetch next from mycursor into @price_change_num
   ...
   begin
      ...
      from dbo.aud_price_change aud
      where aud.price_change_num = @price_change_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@price_change_num as varchar)
      ...
      begin
         ...
         where aud.price_change_num = @price_change_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_price_change records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_price_change rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_price_change rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_price_change'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_price_change record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_price_change records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_price_change] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_price_change', NULL, NULL
GO
