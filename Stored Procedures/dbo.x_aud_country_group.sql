...
CREATE procedure [dbo].[x_aud_country_group]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @code               char(8),
   ...
   begin
      ...
      RAISERROR('aud_country_group: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct code
      from dbo.aud_country_group
      order by code
   ...
   begin
      ...
      from dbo.aud_country_group aud
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_country_group records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_country_group rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_country_group rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_country_group'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_country_group record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_country_group records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_country_group] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_country_group', NULL, NULL
GO
