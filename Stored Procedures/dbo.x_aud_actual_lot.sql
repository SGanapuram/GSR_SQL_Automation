...
CREATE procedure [dbo].[x_aud_actual_lot]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @actual_lot_num     int,
   ...
   begin
      ...
      RAISERROR('aud_actual_lot: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct actual_lot_num
      from dbo.aud_actual_lot
      order by actual_lot_num
 
   open mycursor
   fetch next from mycursor into @actual_lot_num
   ...
   begin
      ...
      from dbo.aud_actual_lot aud
      where aud.actual_lot_num = @actual_lot_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@actual_lot_num as varchar)
      ...
      begin
         ...
         where aud.actual_lot_num = @actual_lot_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_actual_lot records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_actual_lot rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_actual_lot rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_actual_lot'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_actual_lot record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_actual_lot records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_actual_lot] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_actual_lot', NULL, NULL
GO
