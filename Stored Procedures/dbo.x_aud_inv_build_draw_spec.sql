...
CREATE procedure [dbo].[x_aud_inv_build_draw_spec]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @inv_num            int,
   ...
   begin
      ...
      RAISERROR('aud_inv_build_draw_spec: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct inv_num, inv_b_d_num, spec_code
      from dbo.aud_inv_build_draw_spec
      order by inv_num, inv_b_d_num, spec_code
 
   open mycursor
   fetch next from mycursor into @inv_num, @inv_b_d_num, @spec_code
   ...
   begin
      ...
      from dbo.aud_inv_build_draw_spec aud
      where aud.inv_num = @inv_num and
            aud.inv_b_d_num = @inv_b_d_num and
      ...
      begin
         ...
         set @tempstr = cast(@inv_num as varchar) + '/' + cast(@inv_b_d_num as varchar) + '/' + @spec_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_inv_build_draw_spec records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_inv_build_draw_spec rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_inv_build_draw_spec rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_inv_build_draw_spec'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_inv_build_draw_spec record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_inv_build_draw_spec records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_inv_build_draw_spec] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_inv_build_draw_spec', NULL, NULL
GO
