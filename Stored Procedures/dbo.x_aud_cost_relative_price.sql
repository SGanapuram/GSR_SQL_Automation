...
CREATE procedure [dbo].[x_aud_cost_relative_price]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @cost_num           int,
   ...
   begin
      ...
      RAISERROR('aud_cost_relative_price: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct cost_num, seq_num
      from dbo.aud_cost_relative_price
      order by cost_num, seq_num
 
   open mycursor
   fetch next from mycursor into @cost_num, @seq_num
   ...
   begin
      ...
      from dbo.aud_cost_relative_price aud
      where aud.cost_num = @cost_num and
            aud.seq_num = @seq_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@cost_num as varchar) + '/' + cast(@seq_num as varchar)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_cost_relative_price records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_cost_relative_price rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_cost_relative_price rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_cost_relative_price'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_cost_relative_price record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_cost_relative_price records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_cost_relative_price] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_cost_relative_price', NULL, NULL
GO
