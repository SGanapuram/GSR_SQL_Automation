...
CREATE procedure [dbo].[x_aud_broker_commission_default]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @brkr_comm_dflt_num int,
        @trans_id           int,
        @resp_trans_id      int,
        @cutoffdate         datetime,
        @rows_deleted       int,
   ...
   begin
      ...
      RAISERROR('aud_broker_commission_default: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct brkr_comm_dflt_num
      from dbo.aud_broker_commission_default
      order by brkr_comm_dflt_num
 
   open mycursor
   fetch next from mycursor into @brkr_comm_dflt_num
   ...
   begin
      ...
      from dbo.aud_broker_commission_default aud
      where aud.brkr_comm_dflt_num = @brkr_comm_dflt_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@brkr_comm_dflt_num as varchar)
      ...
      begin
         ...
         where aud.brkr_comm_dflt_num = @brkr_comm_dflt_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_broker_commission_default records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_broker_commission_default rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_broker_commission_default rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_broker_commission_default'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_broker_commission_default record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_broker_commission_default records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_broker_commission_default] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_broker_commission_default', NULL, NULL
GO
