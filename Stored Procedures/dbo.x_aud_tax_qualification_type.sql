...
CREATE procedure [dbo].[x_aud_tax_qualification_type]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @code               char(8),
   ...
   begin
      ...
      RAISERROR('aud_tax_qualification_type: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct code
      from dbo.aud_tax_qualification_type
      order by code
   ...
   begin
      ...
      from dbo.aud_tax_qualification_type aud
      where aud.code = @code and
            exists (select 1
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_tax_qualification_type records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_tax_qualification_type rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_tax_qualification_type rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_tax_qualification_type'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_tax_qualification_type record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_tax_qualification_type records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_tax_qualification_type] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_tax_qualification_type', NULL, NULL
GO
