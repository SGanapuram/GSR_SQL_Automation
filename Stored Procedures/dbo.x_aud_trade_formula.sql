...
CREATE procedure [dbo].[x_aud_trade_formula]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @trade_num          int,
   ...
   begin
      ...
      RAISERROR('aud_trade_formula: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct trade_num, order_num, item_num, formula_num
      from dbo.aud_trade_formula
      order by trade_num, order_num, item_num, formula_num
 
   open mycursor
   fetch next from mycursor into @trade_num, @order_num, @item_num, @formula_num
   ...
   begin
      ...
      begin
         ...
         set @tempstr = cast(@trade_num as varchar) + '/' + cast(@order_num as varchar) + '/' + cast(@item_num as varchar) + '/' + cast(@formula_num as varchar)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_trade_formula records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_trade_formula rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_trade_formula rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_trade_formula'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_trade_formula record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_trade_formula records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_trade_formula] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_trade_formula', NULL, NULL
GO
