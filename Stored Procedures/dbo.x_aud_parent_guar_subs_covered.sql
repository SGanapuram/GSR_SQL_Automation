...
CREATE procedure [dbo].[x_aud_parent_guar_subs_covered]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @pg_num             int,
   ...
   begin
      ...
      RAISERROR('aud_parent_guar_subs_covered: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct pg_num, pg_subsidiary_num
      from dbo.aud_parent_guar_subs_covered
      order by pg_num, pg_subsidiary_num
 
   open mycursor
   fetch next from mycursor into @pg_num, @pg_subsidiary_num
   ...
   begin
      ...
      from dbo.aud_parent_guar_subs_covered aud
      where aud.pg_num = @pg_num and
            aud.pg_subsidiary_num = @pg_subsidiary_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@pg_num as varchar) + '/' + cast(@pg_subsidiary_num as varchar)
      ...
      begin
         ...
         where aud.pg_num = @pg_num and
               aud.pg_subsidiary_num = @pg_subsidiary_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_parent_guar_subs_covered records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_parent_guar_subs_covered rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_parent_guar_subs_covered rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_parent_guar_subs_covered'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_parent_guar_subs_covered record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_parent_guar_subs_covered records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_parent_guar_subs_covered] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_parent_guar_subs_covered', NULL, NULL
GO
