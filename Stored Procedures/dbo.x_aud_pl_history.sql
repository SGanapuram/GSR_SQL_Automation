...
CREATE procedure [dbo].[x_aud_pl_history]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @pl_asof_date       datetime,
   ...
   begin
      ...
      RAISERROR('aud_pl_history: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct pl_asof_date, pl_record_key, pl_owner_code, pl_type
      from dbo.aud_pl_history
      order by pl_asof_date, pl_record_key, pl_owner_code, pl_type
 
   open mycursor
   fetch next from mycursor into @pl_asof_date, @pl_record_key, @pl_owner_code, @pl_type
   ...
   begin
      ...
      from dbo.aud_pl_history aud
      where aud.pl_asof_date = @pl_asof_date and
            aud.pl_record_key = @pl_record_key and
            aud.pl_owner_code = @pl_owner_code and
            aud.pl_type = @pl_type and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = convert(varchar, @pl_asof_date, 101) + '/' + cast(@pl_record_key as varchar) + '/' + @pl_owner_code + '/' + @pl_type
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_pl_history records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_pl_history rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_pl_history rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_pl_history'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_pl_history record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_pl_history records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_pl_history] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_pl_history', NULL, NULL
GO
