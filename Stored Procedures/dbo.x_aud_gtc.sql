...
CREATE procedure [dbo].[x_aud_gtc]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @gtc_code           char(15),
   ...
   begin
      ...
      RAISERROR('aud_gtc: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct gtc_code
      from dbo.aud_gtc
      order by gtc_code
 
   open mycursor
   fetch next from mycursor into @gtc_code
   ...
   begin
      ...
      from dbo.aud_gtc aud
      where aud.gtc_code = @gtc_code and
            exists (select 1
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_gtc records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_gtc rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_gtc rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_gtc'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_gtc record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_gtc records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_gtc] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_gtc', NULL, NULL
GO
