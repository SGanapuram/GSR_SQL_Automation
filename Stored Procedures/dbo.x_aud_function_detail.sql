...
CREATE procedure [dbo].[x_aud_function_detail]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @fd_id              int,
   ...
   begin
      ...
      RAISERROR('aud_function_detail: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct fd_id
      from dbo.aud_function_detail
      order by fd_id
 
   open mycursor
   fetch next from mycursor into @fd_id
   ...
   begin
      ...
      from dbo.aud_function_detail aud
      where aud.fd_id = @fd_id and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@fd_id as varchar)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_function_detail records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_function_detail rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_function_detail rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_function_detail'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_function_detail record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_function_detail records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_function_detail] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_function_detail', NULL, NULL
GO
