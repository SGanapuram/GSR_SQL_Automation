...
CREATE procedure [dbo].[x_aud_credit_alarm_log_comment]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @alarm_log_cmnt_num int,
        @trans_id           int,
        @resp_trans_id      int,
        @cutoffdate         datetime,
        @rows_deleted       int,
   ...
   begin
      ...
      RAISERROR('aud_credit_alarm_log_comment: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct alarm_log_cmnt_num
      from dbo.aud_credit_alarm_log_comment
      order by alarm_log_cmnt_num
 
   open mycursor
   fetch next from mycursor into @alarm_log_cmnt_num
   ...
   begin
      ...
      from dbo.aud_credit_alarm_log_comment aud
      where aud.alarm_log_cmnt_num = @alarm_log_cmnt_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@alarm_log_cmnt_num as varchar)
      ...
      begin
         ...
         where aud.alarm_log_cmnt_num = @alarm_log_cmnt_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_credit_alarm_log_comment records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_credit_alarm_log_comment rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_credit_alarm_log_comment rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_credit_alarm_log_comment'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_credit_alarm_log_comment record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_credit_alarm_log_comment records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_credit_alarm_log_comment] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_credit_alarm_log_comment', NULL, NULL
GO
