...
CREATE procedure [dbo].[x_aud_position_mark_to_market]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @pos_num            int,
   ...
   begin
      ...
      RAISERROR('aud_position_mark_to_market: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct pos_num, mtm_asof_date
      from dbo.aud_position_mark_to_market
      order by pos_num, mtm_asof_date
 
   open mycursor
   fetch next from mycursor into @pos_num, @mtm_asof_date
   ...
   begin
      ...
      from dbo.aud_position_mark_to_market aud
      where aud.pos_num = @pos_num and
            aud.mtm_asof_date = @mtm_asof_date and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@pos_num as varchar) + '/' + convert(varchar, @mtm_asof_date, 101)
      ...
      begin
         ...
         where aud.pos_num = @pos_num and
               aud.mtm_asof_date = @mtm_asof_date
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_position_mark_to_market records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_position_mark_to_market rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_position_mark_to_market rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_position_mark_to_market'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_position_mark_to_market record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_position_mark_to_market records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_position_mark_to_market] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_position_mark_to_market', NULL, NULL
GO
