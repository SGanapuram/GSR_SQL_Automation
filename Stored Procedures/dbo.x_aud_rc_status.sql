...
CREATE procedure [dbo].[x_aud_rc_status]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @rc_status_code     char(8),
   ...
   begin
      ...
      RAISERROR('aud_rc_status: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct rc_status_code
      from dbo.aud_rc_status
      order by rc_status_code
 
   open mycursor
   fetch next from mycursor into @rc_status_code
   ...
   begin
      ...
      from dbo.aud_rc_status aud
      where aud.rc_status_code = @rc_status_code and
            exists (select 1
      ...
      begin
         ...
         where aud.rc_status_code = @rc_status_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_rc_status records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_rc_status rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_rc_status rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_rc_status'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_rc_status record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_rc_status records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_rc_status] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_rc_status', NULL, NULL
GO
