...
CREATE procedure [dbo].[x_aud_inhouse_post]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @trade_num          int,
   ...
   begin
      ...
      RAISERROR('aud_inhouse_post: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct trade_num
      from dbo.aud_inhouse_post
      order by trade_num
   ...
   begin
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_inhouse_post records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_inhouse_post rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_inhouse_post rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_inhouse_post'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_inhouse_post record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_inhouse_post records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_inhouse_post] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_inhouse_post', NULL, NULL
GO
