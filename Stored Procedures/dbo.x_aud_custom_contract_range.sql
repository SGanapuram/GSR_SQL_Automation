...
CREATE procedure [dbo].[x_aud_custom_contract_range]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @range_num          int,
   ...
   begin
      ...
      RAISERROR('aud_custom_contract_range: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct range_num
      from dbo.aud_custom_contract_range
      order by range_num
 
   open mycursor
   fetch next from mycursor into @range_num
   ...
   begin
      ...
      from dbo.aud_custom_contract_range aud
      where aud.range_num = @range_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@range_num as varchar)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_custom_contract_range records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_custom_contract_range rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_custom_contract_range rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_custom_contract_range'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_custom_contract_range record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_custom_contract_range records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_custom_contract_range] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_custom_contract_range', NULL, NULL
GO
