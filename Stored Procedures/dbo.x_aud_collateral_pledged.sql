...
CREATE procedure [dbo].[x_aud_collateral_pledged]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @coll_pledged_num   int,
   ...
   begin
      ...
      RAISERROR('aud_collateral_pledged: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct coll_pledged_num
      from dbo.aud_collateral_pledged
      order by coll_pledged_num
 
   open mycursor
   fetch next from mycursor into @coll_pledged_num
   ...
   begin
      ...
      from dbo.aud_collateral_pledged aud
      where aud.coll_pledged_num = @coll_pledged_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@coll_pledged_num as varchar)
      ...
      begin
         ...
         where aud.coll_pledged_num = @coll_pledged_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_collateral_pledged records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_collateral_pledged rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_collateral_pledged rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_collateral_pledged'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_collateral_pledged record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_collateral_pledged records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_collateral_pledged] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_collateral_pledged', NULL, NULL
GO
