...
CREATE procedure [dbo].[x_aud_cost_equalization_rate]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @cost_num           int,
   ...
   begin
      ...
      RAISERROR('aud_cost_equalization_rate: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct cost_num, spec_code, effective_date
      from dbo.aud_cost_equalization_rate
      order by cost_num, spec_code, effective_date
 
   open mycursor
   fetch next from mycursor into @cost_num, @spec_code, @effective_date
   ...
   begin
      ...
      from dbo.aud_cost_equalization_rate aud
      where aud.cost_num = @cost_num and
            aud.spec_code = @spec_code and
            aud.effective_date = @effective_date and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@cost_num as varchar) + '/' + @spec_code + '/' + convert(varchar, @effective_date, 101)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_cost_equalization_rate records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_cost_equalization_rate rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_cost_equalization_rate rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_cost_equalization_rate'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_cost_equalization_rate record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_cost_equalization_rate records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_cost_equalization_rate] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_cost_equalization_rate', NULL, NULL
GO
