...
CREATE procedure [dbo].[pl_records_tp]
(   
   @top_portfolio         int,  
   @asofdate              datetime,  
...
declare @deskETid          int,
   ...
   (
      ...
      port_short_name      varchar(40) null
   )      
...
*/
   ...
   set desired_pl_curr_code = rp.desired_pl_curr_code,           
       port_short_name = convert(varchar, rp.port_num) + ' - ' + rp.port_short_name
   ...
   select   
      ...
      c.cmdty_short_name,
      m.mkt_short_name,
	    po.port_short_name as portShortName
   from (select   
             real_port_num,  
             pos_num,  
             pl_asof_date,  
             ...
             pl_amt  
         from dbo.pl_history
	       where pl_type not in ('W') and  
		           pl_asof_date = @asofdate) pl,   
        dbo.position p
         ...
         from dbo.pl_history
	       where pl_type not in ('W') and  
	             pl_asof_date = @asofdate and
		           not (pl_owner_code = 'C' and 
		                pl_owner_sub_code = 'SWBC')) pl, /* need to skip broker costs */	   
        dbo.position p
           LEFT OUTER JOIN dbo.commodity c with (nolock)
              ON c.cmdty_code = p.cmdty_code
           LEFT OUTER JOIN dbo.market m with (nolock)
   ...
   return  
GO
GRANT EXECUTE ON  [dbo].[pl_records_tp] TO [next_usr]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'pl_records_tp', NULL, NULL
GO
