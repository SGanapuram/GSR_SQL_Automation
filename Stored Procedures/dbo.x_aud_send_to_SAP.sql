...
CREATE procedure [dbo].[x_aud_send_to_SAP]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @row_id             int,
   ...
   begin
      ...
      RAISERROR('aud_send_to_SAP: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct row_id
      from dbo.aud_send_to_SAP
      order by row_id
   ...
   begin
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_send_to_SAP records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_send_to_SAP rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_send_to_SAP rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_send_to_SAP'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_send_to_SAP record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_send_to_SAP records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_send_to_SAP] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_send_to_SAP', NULL, NULL
GO
