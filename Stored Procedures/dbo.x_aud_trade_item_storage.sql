...
CREATE procedure [dbo].[x_aud_trade_item_storage]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @trade_num          int,
   ...
   begin
      ...
      RAISERROR('aud_trade_item_storage: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
   ...
   begin
      ...
      from dbo.aud_trade_item_storage aud
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_trade_item_storage records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_trade_item_storage rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_trade_item_storage rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_trade_item_storage'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_trade_item_storage record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_trade_item_storage records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_trade_item_storage] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_trade_item_storage', NULL, NULL
GO
