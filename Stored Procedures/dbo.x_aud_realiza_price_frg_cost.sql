...
CREATE procedure [dbo].[x_aud_realiza_price_frg_cost]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @crude_cmdty_code   char(8),
        @mkt_code           char(8),
        @trans_id           int,
        @resp_trans_id      int,
        @cutoffdate         datetime,
   ...
   begin
      ...
      RAISERROR('aud_realiza_price_frg_cost: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct crude_cmdty_code, mkt_code
      from dbo.aud_realiza_price_frg_cost
      order by crude_cmdty_code, mkt_code
 
   open mycursor
   fetch next from mycursor into @crude_cmdty_code, @mkt_code
   ...
   begin
      ...
      from dbo.aud_realiza_price_frg_cost aud
      where aud.crude_cmdty_code = @crude_cmdty_code and
      ...
      begin
         ...
         set @tempstr = @crude_cmdty_code + '/' + @mkt_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_realiza_price_frg_cost records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_realiza_price_frg_cost rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_realiza_price_frg_cost rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_realiza_price_frg_cost'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_realiza_price_frg_cost record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_realiza_price_frg_cost records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_realiza_price_frg_cost] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_realiza_price_frg_cost', NULL, NULL
GO
