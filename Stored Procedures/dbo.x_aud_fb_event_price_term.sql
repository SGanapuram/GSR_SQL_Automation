...
CREATE procedure [dbo].[x_aud_fb_event_price_term]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @formula_num        int,
   ...
   begin
      ...
      RAISERROR('aud_fb_event_price_term: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct formula_num, formula_body_num
      from dbo.aud_fb_event_price_term
      order by formula_num, formula_body_num
 
   open mycursor
   fetch next from mycursor into @formula_num, @formula_body_num
   ...
   begin
      ...
      from dbo.aud_fb_event_price_term aud
      ...
      begin
         ...
         set @tempstr = cast(@formula_num as varchar) + '/' + cast(@formula_body_num as varchar)
      ...
      begin
         ...
         where aud.formula_num = @formula_num and
               aud.formula_body_num = @formula_body_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_fb_event_price_term records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_fb_event_price_term rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_fb_event_price_term rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_fb_event_price_term'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_fb_event_price_term record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_fb_event_price_term records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_fb_event_price_term] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_fb_event_price_term', NULL, NULL
GO
