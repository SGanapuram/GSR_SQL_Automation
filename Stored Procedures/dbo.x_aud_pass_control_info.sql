...
CREATE procedure [dbo].[x_aud_pass_control_info]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @pass_control_id    int,
   ...
   begin
      ...
      RAISERROR('aud_pass_control_info: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct pass_control_id
      from dbo.aud_pass_control_info
      order by pass_control_id
 
   open mycursor
   fetch next from mycursor into @pass_control_id
   ...
   begin
      ...
      from dbo.aud_pass_control_info aud
      where aud.pass_control_id = @pass_control_id and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@pass_control_id as varchar)
      ...
      begin
         ...
         where aud.pass_control_id = @pass_control_id
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_pass_control_info records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_pass_control_info rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_pass_control_info rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_pass_control_info'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_pass_control_info record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_pass_control_info records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_pass_control_info] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_pass_control_info', NULL, NULL
GO
