...
CREATE procedure [dbo].[x_aud_lc_document]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @lc_num             int,
   ...
   begin
      ...
      RAISERROR('aud_lc_document: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct lc_num, lc_doc_code
      from dbo.aud_lc_document
      order by lc_num, lc_doc_code
 
   open mycursor
   fetch next from mycursor into @lc_num, @lc_doc_code
   ...
   begin
      ...
      from dbo.aud_lc_document aud
      where aud.lc_num = @lc_num and
            aud.lc_doc_code = @lc_doc_code and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@lc_num as varchar) + '/' + @lc_doc_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_lc_document records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_lc_document rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_lc_document rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_lc_document'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_lc_document record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_lc_document records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_lc_document] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_lc_document', NULL, NULL
GO
