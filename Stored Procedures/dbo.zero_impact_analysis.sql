...
declare @smsg     varchar(255)      
   ...
   if @mode is not null      
   begin      
       if @mode > 19 or @mode < 1      
       begin      
          print '=> The value for the argument ''mode'' must be a NULL, or a value between 1 and 19!'      
          return      
       end      
   end      
   ...
   if @mode is null OR @mode = 18   
      exec dbo.usp_compare_portfolio_group_eod @resp_trans_id_NEW,       
                                                   ...
                                                   @digits_for_scale4,       
                                                   @digits_for_scale7 
	if @mode is null OR @mode = 19 
      exec dbo.usp_compare_position 			@resp_trans_id_NEW,       
                                                   @resp_trans_id_OLD,       
                                                   @portnum,       
                                                   @digits_for_scale4,       
                                                   @digits_for_scale7
