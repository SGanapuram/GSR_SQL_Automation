...
CREATE procedure [dbo].[x_aud_mca_trade_item]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @mca_num            int,
   ...
   begin
      ...
      RAISERROR('aud_mca_trade_item: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct mca_num, trade_num, order_num, item_num
      from dbo.aud_mca_trade_item
      order by mca_num, trade_num, order_num, item_num
 
   open mycursor
   fetch next from mycursor into @mca_num, @trade_num, @order_num, @item_num
   ...
   begin
      ...
      from dbo.aud_mca_trade_item aud
      where aud.mca_num = @mca_num and
            aud.trade_num = @trade_num and
      ...
      begin
         ...
         set @tempstr = cast(@mca_num as varchar) + '/' + cast(@trade_num as varchar) + '/' + cast(@order_num as varchar) + '/' + cast(@item_num as varchar)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_mca_trade_item records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_mca_trade_item rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_mca_trade_item rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_mca_trade_item'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_mca_trade_item record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_mca_trade_item records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_mca_trade_item] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_mca_trade_item', NULL, NULL
GO
