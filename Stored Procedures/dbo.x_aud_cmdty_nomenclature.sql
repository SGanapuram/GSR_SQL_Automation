...
CREATE procedure [dbo].[x_aud_cmdty_nomenclature]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @cmdty_nomenclature_id int,
        @trans_id              int,
        @resp_trans_id         int,
        @cutoffdate            datetime,
        @rows_deleted          int,
   ...
   begin
      ...
      RAISERROR('aud_cmdty_nomenclature: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct cmdty_nomenclature_id
      from dbo.aud_cmdty_nomenclature
      order by cmdty_nomenclature_id
 
   open mycursor
   fetch next from mycursor into @cmdty_nomenclature_id
   ...
   begin
      ...
      from dbo.aud_cmdty_nomenclature aud
      where aud.cmdty_nomenclature_id = @cmdty_nomenclature_id and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@cmdty_nomenclature_id as varchar)
      ...
      begin
         ...
         where aud.cmdty_nomenclature_id = @cmdty_nomenclature_id
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_cmdty_nomenclature records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_cmdty_nomenclature rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_cmdty_nomenclature rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_cmdty_nomenclature'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_cmdty_nomenclature record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_cmdty_nomenclature records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_cmdty_nomenclature] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_cmdty_nomenclature', NULL, NULL
GO
