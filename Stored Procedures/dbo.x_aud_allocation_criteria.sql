...
CREATE procedure [dbo].[x_aud_allocation_criteria]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @alloc_criteria_num int,
        @trans_id           int,
        @resp_trans_id      int,
        @cutoffdate         datetime,
        @rows_deleted       int,
   ...
   begin
      ...
      RAISERROR('aud_allocation_criteria: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct alloc_criteria_num
      from dbo.aud_allocation_criteria
      order by alloc_criteria_num
 
   open mycursor
   fetch next from mycursor into @alloc_criteria_num
   ...
   begin
      ...
      from dbo.aud_allocation_criteria aud
      where aud.alloc_criteria_num = @alloc_criteria_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@alloc_criteria_num as varchar)
      ...
      begin
         ...
         where aud.alloc_criteria_num = @alloc_criteria_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_allocation_criteria records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_allocation_criteria rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_allocation_criteria rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_allocation_criteria'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_allocation_criteria record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_allocation_criteria records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_allocation_criteria] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_allocation_criteria', NULL, NULL
GO
