...
CREATE procedure [dbo].[x_aud_user_group]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @user_group_code    char(8),
   ...
   begin
      ...
      RAISERROR('aud_user_group: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct user_group_code
      from dbo.aud_user_group
      order by user_group_code
 
   open mycursor
   fetch next from mycursor into @user_group_code
   ...
   begin
      ...
      begin
         ...
         where aud.user_group_code = @user_group_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_user_group records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_user_group rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_user_group rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_user_group'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_user_group record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_user_group records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_user_group] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_user_group', NULL, NULL
GO
