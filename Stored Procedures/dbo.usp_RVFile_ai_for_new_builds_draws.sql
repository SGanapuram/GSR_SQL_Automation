...
CREATE procedure [dbo].[usp_RVFile_ai_for_new_builds_draws] 
(
...
declare @rows_affected     int,
        ...
        (select alloc_num
         ...
         where not exists (select 1 
						               from dbo.aud_inventory_build_draw aibd2 WITH (NOLOCK)
								           where aibd2.inv_num = aibd.inv_num and
								                 aibd2.inv_b_d_num = aibd.inv_b_d_num AND
				                         aibd2.trans_id <= @prev_min_trans_id) AND							                                                                                     			                                                    
			         trans_id = (select max(aibd2.trans_id) 
			                     from dbo.aud_inventory_build_draw aibd2 WITH (NOLOCK)
						               where aibd2.inv_num = aibd.inv_num and 
						                     aibd2.inv_b_d_num = aibd.inv_b_d_num AND
			     	   					         aibd2.trans_id <= @min_trans_id)) aibd
   where ai.alloc_item_type in ('R', 'I', 'N', 'D', 'C', 'T') AND
  	     ai.alloc_num = aibd.alloc_num 
   select @rows_affected = @@rowcount  
   if @debugon = 1  
   begin  
      update @times   
   ...
   begin
      print ' '
      select @smsg = 'usp_RVFile_ai_for_new_builds_draws: ' + convert(varchar, @rows_affected) + ' rows returned.'
      print @smsg
      print ' '
...
return 0
GO
GRANT EXECUTE ON  [dbo].[usp_RVFile_ai_for_new_builds_draws] TO [next_usr]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'usp_RVFile_ai_for_new_builds_draws', NULL, NULL
GO
