...
CREATE PROC [dbo].[MDS_position_grid]
...
BEGIN                      
	...
	if (isnull(@AsOfDate,'01/01/1900')='01/01/1900')                      
	 select @AsOfDate=max(pl_asof_date) from v_CUBE_cob_date                        
	...
	IF @ProfitCntr IS NOT NULL                                              
	 BEGIN                                              
		...
		INNER JOIN dbo.udf_MET_ListToTable(@ProfitCntr) i ON pt.tag_value=i.vchar_value                       
	...
	(          
	 ...
	 BEGIN                
	  insert into #pos                                   
	   ...
	   trader_init ,                      
		...
		'Historical'                   
	 from dbo.t_CUBE_snapshot_pos_detail pos with(NOLOCK)                    
	 ...
	 BEGIN                
		insert into #pos                                   
		   ...
		   trader_init ,                      
			...
			NULL    ,  NULL,   NULL,  NULL,NULL,              
			'Live'                  
		 from dbo.v_CUBE_position pos  with(NOLOCK)                    
		 where exists (select 1 from #children c1 where c1.port_num=pos.real_port_num)                      
	 END                     
	 ...
	 FROM #pos p                                                  
	 CROSS APPLY dbo.udf_METgetUomConversion (pos_qty_uom_code, 'MT',NULL,NULL,cmdty_code )                                  
	 where QuantityMT is null                                                  
                      
	 UPDATE p                                                  
	 SET QuantityBBL =  primary_pos_qty*uom_factor                                                 
	 FROM #pos p                           
	 CROSS APPLY dbo.udf_METgetUomConversion (pos_qty_uom_code, 'BBL',NULL,NULL,cmdty_code )                                              
	 where QuantityBBL is null                                                  
                      
	 UPDATE p                                        
	 SET QuantityMT =  secondary_pos_qty*uom_factor                                                 
	 FROM #pos p                                                  
	 CROSS APPLY dbo.udf_METgetUomConversion (secondary_qty_uom_code, 'MT',NULL,NULL,cmdty_code )                                              
	 where QuantityMT is null                                                  
                      
	 UPDATE p                                                  
	 SET QuantityBBL =  secondary_pos_qty*uom_factor                                                 
	 FROM #pos p                                                  
	 CROSS APPLY dbo.udf_METgetUomConversion (secondary_qty_uom_code, 'BBL',NULL,NULL,cmdty_code )                                              
	 where QuantityBBL is null                          
	...
	BEGIN  
	  ...
	  null,                      
	  null                 
	  from dbo.v_CUBE_commkt_formula pr   with(NOLOCK)                    
	...
	BEGIN                      
	 ...
	 null,                      
	 null                 
	 from dbo.v_MDS_price_detail pr   with(NOLOCK)                    
...
END                          
GO
GRANT EXECUTE ON  [dbo].[MDS_position_grid] TO [next_usr]
GO
