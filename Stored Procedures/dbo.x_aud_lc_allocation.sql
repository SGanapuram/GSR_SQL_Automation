...
CREATE procedure [dbo].[x_aud_lc_allocation]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @lc_num             int,
   ...
   begin
      ...
      RAISERROR('aud_lc_allocation: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct lc_num, lc_alloc_num
      from dbo.aud_lc_allocation
      order by lc_num, lc_alloc_num
 
   open mycursor
   fetch next from mycursor into @lc_num, @lc_alloc_num
   ...
   begin
      ...
      from dbo.aud_lc_allocation aud
      ...
      begin
         ...
         set @tempstr = cast(@lc_num as varchar) + '/' + cast(@lc_alloc_num as varchar)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_lc_allocation records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_lc_allocation rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_lc_allocation rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_lc_allocation'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_lc_allocation record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_lc_allocation records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_lc_allocation] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_lc_allocation', NULL, NULL
GO
