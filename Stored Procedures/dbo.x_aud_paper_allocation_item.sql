...
CREATE procedure [dbo].[x_aud_paper_allocation_item]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @paper_alloc_num      int,
        @paper_alloc_item_num smallint,
        @trans_id             int,
        @resp_trans_id        int,
        @cutoffdate           datetime,
        @rows_deleted         int,
   ...
   begin
      ...
      RAISERROR('aud_paper_allocation_item: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct paper_alloc_num, paper_alloc_item_num
      from dbo.aud_paper_allocation_item
      order by paper_alloc_num, paper_alloc_item_num
 
   open mycursor
   fetch next from mycursor into @paper_alloc_num, @paper_alloc_item_num
   ...
   begin
      ...
      from dbo.aud_paper_allocation_item aud
      where aud.paper_alloc_num = @paper_alloc_num and
            aud.paper_alloc_item_num = @paper_alloc_item_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@paper_alloc_num as varchar) + '/' + cast(@paper_alloc_item_num as varchar)
      ...
      begin
         ...
         where aud.paper_alloc_num = @paper_alloc_num and
               aud.paper_alloc_item_num = @paper_alloc_item_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_paper_allocation_item records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_paper_allocation_item rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_paper_allocation_item rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_paper_allocation_item'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_paper_allocation_item record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_paper_allocation_item records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_paper_allocation_item] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_paper_allocation_item', NULL, NULL
GO
