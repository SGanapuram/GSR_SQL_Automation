...
CREATE procedure [dbo].[x_aud_formula_body_trigger]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @formula_num        int,
   ...
   begin
      ...
      RAISERROR('aud_formula_body_trigger: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct formula_num, formula_body_num, trigger_num
      from dbo.aud_formula_body_trigger
      order by formula_num, formula_body_num, trigger_num
 
   open mycursor
   fetch next from mycursor into @formula_num, @formula_body_num, @trigger_num
   ...
   begin
      ...
      begin
         ...
         set @tempstr = cast(@formula_num as varchar) + '/' + cast(@formula_body_num as varchar) + '/' + cast(@trigger_num as varchar)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_formula_body_trigger records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_formula_body_trigger rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_formula_body_trigger rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_formula_body_trigger'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_formula_body_trigger record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_formula_body_trigger records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_formula_body_trigger] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_formula_body_trigger', NULL, NULL
GO
