...
CREATE procedure [dbo].[x_aud_credit_alarm]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @credit_limit_num   int,
   ...
   begin
      ...
      RAISERROR('aud_credit_alarm: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct credit_limit_num
      from dbo.aud_credit_alarm
      order by credit_limit_num
 
   open mycursor
   fetch next from mycursor into @credit_limit_num
   ...
   begin
      ...
      begin
         ...
         set @tempstr = cast(@credit_limit_num as varchar)
      ...
      begin
         ...
         where aud.credit_limit_num = @credit_limit_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_credit_alarm records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_credit_alarm rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_credit_alarm rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_credit_alarm'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_credit_alarm record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_credit_alarm records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_credit_alarm] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_credit_alarm', NULL, NULL
GO
