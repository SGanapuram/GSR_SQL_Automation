...
CREATE procedure [dbo].[x_aud_shipment_chain]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @shipment_num       int,
        @next_shipment_num  int,
        @trans_id           int,
        @resp_trans_id      int,
        @cutoffdate         datetime,
        @rows_deleted       int,
   ...
   begin
      ...
      RAISERROR('aud_shipment_chain: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct shipment_num, next_shipment_num
      from dbo.aud_shipment_chain
      order by shipment_num, next_shipment_num
 
   open mycursor
   fetch next from mycursor into @shipment_num, @next_shipment_num
   ...
   begin
      ...
      from dbo.aud_shipment_chain aud
      where aud.shipment_num = @shipment_num and
            aud.next_shipment_num = @next_shipment_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@shipment_num as varchar) + '/' + cast(@next_shipment_num as varchar)
      ...
      begin
         ...
         where aud.shipment_num = @shipment_num and
               aud.next_shipment_num = @next_shipment_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_shipment_chain records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_shipment_chain rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_shipment_chain rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_shipment_chain'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_shipment_chain record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_shipment_chain records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_shipment_chain] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_shipment_chain', NULL, NULL
GO
