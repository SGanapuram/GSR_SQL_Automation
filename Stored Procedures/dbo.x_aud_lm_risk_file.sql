...
CREATE procedure [dbo].[x_aud_lm_risk_file]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @oid                int,
   ...
   begin
      ...
      RAISERROR('aud_lm_risk_file: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct oid
      from dbo.aud_lm_risk_file
      order by oid
   ...
   begin
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_lm_risk_file records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_lm_risk_file rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_lm_risk_file rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_lm_risk_file'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_lm_risk_file record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_lm_risk_file records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_lm_risk_file] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_lm_risk_file', NULL, NULL
GO
