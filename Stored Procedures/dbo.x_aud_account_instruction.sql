...
CREATE procedure [dbo].[x_aud_account_instruction]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @acct_num           int,
   ...
   begin
      ...
      RAISERROR('aud_account_instruction: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct acct_num, acct_instr_num
      from dbo.aud_account_instruction
      order by acct_num, acct_instr_num
 
   open mycursor
   fetch next from mycursor into @acct_num, @acct_instr_num
   ...
   begin
      ...
      from dbo.aud_account_instruction aud
      where aud.acct_num = @acct_num and
            aud.acct_instr_num = @acct_instr_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@acct_num as varchar) + '/' + cast(@acct_instr_num as varchar)
      ...
      begin
         ...
         where aud.acct_num = @acct_num and
               aud.acct_instr_num = @acct_instr_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_account_instruction records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_account_instruction rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_account_instruction rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_account_instruction'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_account_instruction record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_account_instruction records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_account_instruction] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_account_instruction', NULL, NULL
GO
