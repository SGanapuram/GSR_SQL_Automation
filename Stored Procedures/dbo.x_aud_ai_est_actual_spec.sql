...
CREATE procedure [dbo].[x_aud_ai_est_actual_spec]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @alloc_num          int,
   ...
   begin
      ...
      RAISERROR('aud_ai_est_actual_spec: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct alloc_num, alloc_item_num, ai_est_actual_num, spec_code
      from dbo.aud_ai_est_actual_spec
      order by alloc_num, alloc_item_num, ai_est_actual_num, spec_code
 
   open mycursor
   fetch next from mycursor into @alloc_num, @alloc_item_num, @ai_est_actual_num, @spec_code
   ...
   begin
      ...
      begin
         ...
         set @tempstr = cast(@alloc_num as varchar) + '/' + cast(@alloc_item_num as varchar) + '/' + cast(@ai_est_actual_num as varchar) + '/' + @spec_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_ai_est_actual_spec records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_ai_est_actual_spec rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_ai_est_actual_spec rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_ai_est_actual_spec'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_ai_est_actual_spec record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_ai_est_actual_spec records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_ai_est_actual_spec] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_ai_est_actual_spec', NULL, NULL
GO
