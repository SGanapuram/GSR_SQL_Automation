...
CREATE procedure [dbo].[usp_dump_fx_data_for_VAR_portnum]
(
   @debugon          bit = 0
)
as 
...
declare @errcode           int
   ...
   if (select count(*) from #fx_infra_portnums) = 0
   begin
      print '=> The #fx_infra_portnums table does not contain any infra-portfolios!'
      return 0
   end
   ...
   select @my_top_port_num = min(port_num)
   from #fx_infra_portnums
   
   while @my_top_port_num is not null
   begin
	    begin try    
...
return @status
GO
GRANT EXECUTE ON  [dbo].[usp_dump_fx_data_for_VAR_portnum] TO [next_usr]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'usp_dump_fx_data_for_VAR_portnum', NULL, NULL
GO
