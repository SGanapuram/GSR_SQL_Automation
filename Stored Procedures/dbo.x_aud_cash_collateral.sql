...
CREATE procedure [dbo].[x_aud_cash_collateral]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @cash_coll_num      int,
   ...
   begin
      ...
      RAISERROR('aud_cash_collateral: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct cash_coll_num
      from dbo.aud_cash_collateral
      order by cash_coll_num
 
   open mycursor
   fetch next from mycursor into @cash_coll_num
   ...
   begin
      ...
      from dbo.aud_cash_collateral aud
      where aud.cash_coll_num = @cash_coll_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@cash_coll_num as varchar)
      ...
      begin
         ...
         where aud.cash_coll_num = @cash_coll_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_cash_collateral records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_cash_collateral rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_cash_collateral rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_cash_collateral'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_cash_collateral record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_cash_collateral records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_cash_collateral] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_cash_collateral', NULL, NULL
GO
