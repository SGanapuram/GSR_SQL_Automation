...
set nocount on    
  ...
  into #tidpl1    
   from #tidpl     
  group by dist_num, open_pl, closed_pl, pl_curr_code, addl_cost_sum, pl_asof_date     
  having count(*) = 1    
...
-- finish write changed columns.  
  ...
  select     
	 ...
	 diffColList,     
     tid.dist_num,     
     convert(varchar, tid.pl_asof_date, 101) as pl_asof_date,     
      open_pl,    
      closed_pl,    
      addl_cost_sum,    
      pl_curr_code,    
     trans_id1    
   from #tidpl1 tid  left outer join #diffColList difc    
   on tid.dist_num = difc.dist_num and tid.pl_asof_date= difc.pl_asof_date 
  ...
  from (select *    
   ...
   on b.dist_num = difc.dist_num and b.pl_asof_date= difc.pl_asof_date   
  where a.dist_num = b.dist_num and    
        a.pl_asof_date = b.pl_asof_date    
  order by dist_num, pl_asof_date, resp_trans_id         
