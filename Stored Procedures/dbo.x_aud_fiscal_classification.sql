...
CREATE procedure [dbo].[x_aud_fiscal_classification]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @fiscal_class_code  char(2),
        @trans_id           int,
        @resp_trans_id      int,
        @cutoffdate         datetime,
        @rows_deleted       int,
   ...
   begin
      ...
      RAISERROR('aud_fiscal_classification: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct fiscal_class_code
      from dbo.aud_fiscal_classification
      order by fiscal_class_code
 
   open mycursor
   fetch next from mycursor into @fiscal_class_code
   ...
   begin
      ...
      from dbo.aud_fiscal_classification aud
      where aud.fiscal_class_code = @fiscal_class_code and
            exists (select 1
      ...
      begin
         ...
         where aud.fiscal_class_code = @fiscal_class_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_fiscal_classification records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_fiscal_classification rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_fiscal_classification rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_fiscal_classification'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_fiscal_classification record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_fiscal_classification records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_fiscal_classification] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_fiscal_classification', NULL, NULL
GO
