...
CREATE procedure [dbo].[x_aud_broker_fifo_run_rec]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @broker_num         int,
   ...
   begin
      ...
      RAISERROR('aud_broker_fifo_run_rec: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct broker_num, world_port_num
      from dbo.aud_broker_fifo_run_rec
      order by broker_num, world_port_num
 
   open mycursor
   fetch next from mycursor into @broker_num, @world_port_num
   ...
   begin
      ...
      from dbo.aud_broker_fifo_run_rec aud
      where aud.broker_num = @broker_num and
            aud.world_port_num = @world_port_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@broker_num as varchar) + '/' + cast(@world_port_num as varchar)
      ...
      begin
         ...
         where aud.broker_num = @broker_num and
               aud.world_port_num = @world_port_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_broker_fifo_run_rec records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_broker_fifo_run_rec rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_broker_fifo_run_rec rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_broker_fifo_run_rec'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_broker_fifo_run_rec record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_broker_fifo_run_rec records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_broker_fifo_run_rec] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_broker_fifo_run_rec', NULL, NULL
GO
