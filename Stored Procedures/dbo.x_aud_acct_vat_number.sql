...
CREATE procedure [dbo].[x_aud_acct_vat_number]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @acct_vat_num       int,
   ...
   begin
      ...
      RAISERROR('aud_acct_vat_number: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct acct_vat_num
      from dbo.aud_acct_vat_number
      order by acct_vat_num
 
   open mycursor
   fetch next from mycursor into @acct_vat_num
   ...
   begin
      ...
      from dbo.aud_acct_vat_number aud
      where aud.acct_vat_num = @acct_vat_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@acct_vat_num as varchar)
      ...
      begin
         ...
         where aud.acct_vat_num = @acct_vat_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_acct_vat_number records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_acct_vat_number rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_acct_vat_number rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_acct_vat_number'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_acct_vat_number record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_acct_vat_number records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_acct_vat_number] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_acct_vat_number', NULL, NULL
GO
