...
CREATE procedure [dbo].[x_aud_acct_fiscal_rep]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @acct_fiscal_rep_id int,
        @trans_id           int,
        @resp_trans_id      int,
        @cutoffdate         datetime,
        @rows_deleted       int,
   ...
   begin
      ...
      RAISERROR('aud_acct_fiscal_rep: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct acct_fiscal_rep_id
      from dbo.aud_acct_fiscal_rep
      order by acct_fiscal_rep_id
 
   open mycursor
   fetch next from mycursor into @acct_fiscal_rep_id
   ...
   begin
      ...
      from dbo.aud_acct_fiscal_rep aud
      where aud.acct_fiscal_rep_id = @acct_fiscal_rep_id and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@acct_fiscal_rep_id as varchar)
      ...
      begin
         ...
         where aud.acct_fiscal_rep_id = @acct_fiscal_rep_id
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_acct_fiscal_rep records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_acct_fiscal_rep rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_acct_fiscal_rep rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_acct_fiscal_rep'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_acct_fiscal_rep record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_acct_fiscal_rep records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_acct_fiscal_rep] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_acct_fiscal_rep', NULL, NULL
GO
