...
CREATE procedure [dbo].[x_aud_commodity_uom]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @cmdty_code         char(8),
   ...
   begin
      ...
      RAISERROR('aud_commodity_uom: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct cmdty_code, cmdty_uom_for, uom_code
      from dbo.aud_commodity_uom
      order by cmdty_code, cmdty_uom_for, uom_code
 
   open mycursor
   fetch next from mycursor into @cmdty_code, @cmdty_uom_for, @uom_code
   ...
   begin
      ...
      from dbo.aud_commodity_uom aud
      where aud.cmdty_code = @cmdty_code and
            aud.cmdty_uom_for = @cmdty_uom_for and
      ...
      begin
         ...
         set @tempstr = @cmdty_code + '/' + @cmdty_uom_for + '/' + @uom_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_commodity_uom records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_commodity_uom rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_commodity_uom rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_commodity_uom'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_commodity_uom record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_commodity_uom records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_commodity_uom] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_commodity_uom', NULL, NULL
GO
