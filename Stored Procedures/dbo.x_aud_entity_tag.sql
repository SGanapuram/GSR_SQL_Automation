...
CREATE procedure [dbo].[x_aud_entity_tag]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @entity_tag_key     int,
   ...
   begin
      ...
      RAISERROR('aud_entity_tag: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct entity_tag_key
      from dbo.aud_entity_tag
      order by entity_tag_key
 
   open mycursor
   fetch next from mycursor into @entity_tag_key
   ...
   begin
      ...
      from dbo.aud_entity_tag aud
      where aud.entity_tag_key = @entity_tag_key and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@entity_tag_key as varchar)
      ...
      begin
         ...
         where aud.entity_tag_key = @entity_tag_key
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_entity_tag records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_entity_tag rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_entity_tag rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_entity_tag'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_entity_tag record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_entity_tag records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_entity_tag] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_entity_tag', NULL, NULL
GO
