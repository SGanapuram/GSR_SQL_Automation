...
CREATE procedure [dbo].[x_aud_generic_data_name]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @gdn_num            int,
   ...
   begin
      ...
      RAISERROR('aud_generic_data_name: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct gdn_num
      from dbo.aud_generic_data_name
      order by gdn_num
 
   open mycursor
   fetch next from mycursor into @gdn_num
   ...
   begin
      ...
      from dbo.aud_generic_data_name aud
      where aud.gdn_num = @gdn_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@gdn_num as varchar)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_generic_data_name records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_generic_data_name rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_generic_data_name rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_generic_data_name'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_generic_data_name record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_generic_data_name records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_generic_data_name] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_generic_data_name', NULL, NULL
GO
