...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[usp_MDS_get_ags_trade_detail]    
(    
 @portNum int = NULL,    
 @tradeNum int = NULL,    
...
select * into #withPL from v_MDS_get_ags_trade_detail where 0=1    
...
begin        
 ...
 insert into #withPL        
 select * from v_MDS_get_ags_trade_detail         
...
if object_id('tempdb..#withPL', 'U') is not null                  
   exec('drop table #withPL')                  
if object_id('tempdb..#children', 'U') is not null                  
   exec('drop table #children') 
GO
GRANT EXECUTE ON  [dbo].[usp_MDS_get_ags_trade_detail] TO [next_usr]
GO
