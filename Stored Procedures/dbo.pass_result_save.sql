...
declare @resp_trans_id  int,  
 ...
 @pl_history_trans_id int  
   ...
   insert into #tablelist (tablename, audtablename, keycolumn)  
     select 'pl_history', 'aud_pl_history', 'pl_asof_date'  
     union all  
     select 'tid_mark_to_market', 'aud_tid_mark_to_market', 'mtm_pl_asof_date'  
     union all  
     select 'portfolio_profit_loss', 'aud_portfolio_profit_loss', 'pl_asof_date'  
     union all  
     select 'position_mark_to_market', 'aud_position_mark_to_market', 'mtm_asof_date'  
     union all  
     select 'tid_mtm_volatility', 'aud_tid_mtm_volatility', 'mtm_pl_asof_date'  
     union all  
     select 'ti_mark_to_market', 'aud_ti_mark_to_market', 'mtm_pl_asof_date'  
     union all  
     select 'tid_pl', 'aud_tid_pl', 'pl_asof_date'  
     union all  
     select 'trade_item_pl', 'aud_trade_item_pl', 'pl_asof_date'  
     union all  
     select 'cost_ext_info', 'aud_cost_ext_info', 'pl_history_trans_id'  
     union all  
     select 'fx_exposure', 'aud_fx_exposure', 'pl_history_trans_id'  
     union all  
     select 'fx_exposure_dist', 'aud_fx_exposure_dist', 'pl_history_trans_id'  
     union all  
     select 'inventory_history', 'aud_inventory_history', 'asof_date'    
	 union all
	 select 'position_history', 'aud_position_history', 'asof_date'    
	 union all
	 select 'portfolio_eod', 'aud_portfolio_eod', 'snapshot_asof_date'    
	 union all
	 select 'position_group_eod', 'aud_position_group_eod', 'pl_history_trans_id'    
	 union all
	 select 'portfolio_group_eod', 'aud_portfolio_group_eod', 'pl_history_trans_id'    
   ...
   select @pl_history_trans_id = min(trans_id) from dbo.pl_history where pl_asof_date = @asofDate  
   ...
   begin  
      ...
      if @keycolumn = 'pl_history_trans_id'  
      begin         
         ...
         select @sql = @sql + ' where trans_id ' + ' > ' + convert(varchar, @pl_history_trans_id) + ''     
      end  
      else  
      begin  
  select @sql = 'insert into dbo.' + @audtablename + ' (' + @collist + ', resp_trans_id)' +  ' select ' + @collist + ',' + cast(@resp_trans_id as varchar) + ' from dbo.' + @tablename  
         select @sql = @sql + ' where ' + @keycolumn + ' = ''' + convert(varchar, @asofDate, 101) + ''''  
      end  
      if @debugon = 1  
      begin  
...
SET QUOTED_IDENTIFIER OFF   
GO
GRANT EXECUTE ON  [dbo].[pass_result_save] TO [ictspass]
GO
GRANT EXECUTE ON  [dbo].[pass_result_save] TO [next_usr]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'pass_result_save', NULL, NULL
GO
