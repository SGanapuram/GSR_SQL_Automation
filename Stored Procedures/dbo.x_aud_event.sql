...
CREATE procedure [dbo].[x_aud_event]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @event_num          int,
   ...
   begin
      ...
      RAISERROR('aud_event: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
   ...
   open mycursor
   fetch next from mycursor into @event_num
   ...
   begin
      ...
      from dbo.aud_event aud
      where aud.event_num = @event_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@event_num as varchar)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_event records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_event rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_event rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_event'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_event record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_event records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_event] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_event', NULL, NULL
GO
