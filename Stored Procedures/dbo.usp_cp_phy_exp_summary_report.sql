...
CREATE procedure [dbo].[usp_cp_phy_exp_summary_report]
(
	    ...
	    where ct_doc_num in 
	          (select lc_num 
	           ...
	           where (lc_exp_imp_ind = 'E' and 
	                  lc_beneficiary in (select * from dbo.fnToSplit(@my_booking_comp_nums, ',')) and 
	                  lc_applicant = @my_exp_acct_num) or 
	                 (lc_exp_imp_ind = 'I' and 
	                  lc_beneficiary = @my_exp_acct_num and 
	                  lc_applicant in (select * from dbo.fnToSplit(@my_booking_comp_nums, ',')) )) 
		   ...
		   where exp_booking_comp_num in (select * from dbo.fnToSplit(@my_booking_comp_nums, ',')) AND 
...
begin
       ...
       EXEC dbo.usp_cp_phy_exposure_details_update @my_from_date, 
	                                                @my_to_date, 
	                                                @cash_from_date,
	                                                @cash_to_date, 
	                                                @TOI, 
...
declare cp_phy_summary_cursor cursor for
select ExpDate AS 'exp_date',		    			  
		   Amount 'ExposureAmt',
		   cost_pay_rec_ind,
       	           credit_term_code,
		   LC_CoveredAmt,
...
declare @Amt            decimal(20, 8)
declare @LCCovAmt       decimal(20, 8)
declare @costpayrecind  char(1)
declare @credittermcode char(8)
declare @Limitamt	decimal(20, 8)
declare @creditsecind   char(1)

create table #cp_phy_exposure_summary
(
    exp_date		 datetime null,
    LimitAmount          varchar(3000) null,
    Open_exp_rec	 decimal(20, 8) null,
    Open_exp_pay	 decimal(20, 8) null,
    LC_exp_rec		 decimal(20, 8) null,
    LC_exp_pay		 decimal(20, 8) null,
    Net_Receivable	 decimal(20, 8) null
)

open cp_phy_summary_cursor 
fetch NEXT from cp_phy_summary_cursor into @fromdate, 
	                                   @Amt, 
	                                   @costpayrecind, 
	                                   @credittermcode, 										  
					   @LCCovAmt,
...
while @@FETCH_STATUS = 0
begin
	EXEC dbo.usp_cp_phy_exp_summary_total_report @fromdate, 
	                                                  @Amt, 
		                                          @LCCovAmt, 
		                                          @Limitamt,
		                                          @costpayrecind, 
...
close cp_phy_summary_cursor
deallocate cp_phy_summary_cursor

select * from #cp_phy_exposure_summary

drop table #cp_phy_exposure
drop table #cp_phy_exposure_summary

endofsp: 
return 0
GO
GRANT EXECUTE ON  [dbo].[usp_cp_phy_exp_summary_report] TO [next_usr]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'usp_cp_phy_exp_summary_report', NULL, NULL
GO
