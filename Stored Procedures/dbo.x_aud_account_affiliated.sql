...
CREATE procedure [dbo].[x_aud_account_affiliated]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @cntparty_acct_num  int,
        @booking_comp_num   int,
        @affiliate_type     char(15),
        @trans_id           int,
        @resp_trans_id      int,
        @cutoffdate         datetime,
        @rows_deleted       int,
   ...
   begin
      ...
      RAISERROR('aud_account_affiliated: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct cntparty_acct_num, booking_comp_num, affiliate_type
      from dbo.aud_account_affiliated
      order by cntparty_acct_num, booking_comp_num, affiliate_type
 
   open mycursor
   fetch next from mycursor into @cntparty_acct_num, @booking_comp_num, @affiliate_type
   ...
   begin
      ...
      from dbo.aud_account_affiliated aud
      where aud.cntparty_acct_num = @cntparty_acct_num and
            aud.booking_comp_num = @booking_comp_num and
            aud.affiliate_type = @affiliate_type and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@cntparty_acct_num as varchar) + '/' + cast(@booking_comp_num as varchar) + '/' + @affiliate_type
      ...
      begin
         ...
         where aud.cntparty_acct_num = @cntparty_acct_num and
               aud.booking_comp_num = @booking_comp_num and
               aud.affiliate_type = @affiliate_type
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_account_affiliated records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_account_affiliated rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_account_affiliated rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_account_affiliated'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_account_affiliated record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_account_affiliated records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_account_affiliated] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_account_affiliated', NULL, NULL
GO
