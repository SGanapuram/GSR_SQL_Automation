...
CREATE procedure [dbo].[x_aud_portfolio_edpl]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @port_num           int,
   ...
   begin
      ...
      RAISERROR('aud_portfolio_edpl: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
   ...
   begin
      ...
      from dbo.aud_portfolio_edpl aud
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_portfolio_edpl records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_portfolio_edpl rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_portfolio_edpl rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_portfolio_edpl'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_portfolio_edpl record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_portfolio_edpl records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_portfolio_edpl] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_portfolio_edpl', NULL, NULL
GO
