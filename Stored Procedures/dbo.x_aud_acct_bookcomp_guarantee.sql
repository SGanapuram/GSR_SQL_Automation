...
CREATE procedure [dbo].[x_aud_acct_bookcomp_guarantee]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @acct_guarantee_num int,
        @trans_id           int,
        @resp_trans_id      int,
        @cutoffdate         datetime,
        @rows_deleted       int,
   ...
   begin
      ...
      RAISERROR('aud_acct_bookcomp_guarantee: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct acct_guarantee_num
      from dbo.aud_acct_bookcomp_guarantee
      order by acct_guarantee_num
 
   open mycursor
   fetch next from mycursor into @acct_guarantee_num
   ...
   begin
      ...
      from dbo.aud_acct_bookcomp_guarantee aud
      where aud.acct_guarantee_num = @acct_guarantee_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@acct_guarantee_num as varchar)
      ...
      begin
         ...
         where aud.acct_guarantee_num = @acct_guarantee_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_acct_bookcomp_guarantee records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_acct_bookcomp_guarantee rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_acct_bookcomp_guarantee rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_acct_bookcomp_guarantee'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_acct_bookcomp_guarantee record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_acct_bookcomp_guarantee records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_acct_bookcomp_guarantee] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_acct_bookcomp_guarantee', NULL, NULL
GO
