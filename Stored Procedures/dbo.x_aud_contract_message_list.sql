...
CREATE procedure [dbo].[x_aud_contract_message_list]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @contr_num          int,
        @contr_rev_num      int,
        @contr_msg_list_num int,
        @trans_id           int,
        @resp_trans_id      int,
        @cutoffdate         datetime,
        @rows_deleted       int,
   ...
   begin
      ...
      RAISERROR('aud_contract_message_list: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct contr_num, contr_rev_num, contr_msg_list_num
      from dbo.aud_contract_message_list
      order by contr_num, contr_rev_num, contr_msg_list_num
 
   open mycursor
   fetch next from mycursor into @contr_num, @contr_rev_num, @contr_msg_list_num
   ...
   begin
      ...
      from dbo.aud_contract_message_list aud
      where aud.contr_num = @contr_num and
            aud.contr_rev_num = @contr_rev_num and
            aud.contr_msg_list_num = @contr_msg_list_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@contr_num as varchar) + '/' + cast(@contr_rev_num as varchar) + '/' + cast(@contr_msg_list_num as varchar)
      ...
      begin
         ...
         where aud.contr_num = @contr_num and
               aud.contr_rev_num = @contr_rev_num and
               aud.contr_msg_list_num = @contr_msg_list_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_contract_message_list records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_contract_message_list rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_contract_message_list rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_contract_message_list'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_contract_message_list record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_contract_message_list records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_contract_message_list] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_contract_message_list', NULL, NULL
GO
