...
CREATE procedure [dbo].[x_aud_product_usage]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @product_usage_code char(8),
        @trans_id           int,
        @resp_trans_id      int,
        @cutoffdate         datetime,
        @rows_deleted       int,
   ...
   begin
      ...
      RAISERROR('aud_product_usage: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct product_usage_code
      from dbo.aud_product_usage
      order by product_usage_code
 
   open mycursor
   fetch next from mycursor into @product_usage_code
   ...
   begin
      ...
      from dbo.aud_product_usage aud
      where aud.product_usage_code = @product_usage_code and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = @product_usage_code
      ...
      begin
         ...
         where aud.product_usage_code = @product_usage_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_product_usage records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_product_usage rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_product_usage rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_product_usage'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_product_usage record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_product_usage records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_product_usage] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_product_usage', NULL, NULL
GO
