...
CREATE procedure [dbo].[x_aud_vat_declaration]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @vat_declaration_id int,
        @trans_id           int,
        @resp_trans_id      int,
        @cutoffdate         datetime,
        @rows_deleted       int,
   ...
   begin
      ...
      RAISERROR('aud_vat_declaration: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct vat_declaration_id
      from dbo.aud_vat_declaration
      order by vat_declaration_id
 
   open mycursor
   fetch next from mycursor into @vat_declaration_id
   ...
   begin
      ...
      from dbo.aud_vat_declaration aud
      where aud.vat_declaration_id = @vat_declaration_id and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@vat_declaration_id as varchar)
      ...
      begin
         ...
         where aud.vat_declaration_id = @vat_declaration_id
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_vat_declaration records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_vat_declaration rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_vat_declaration rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_vat_declaration'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_vat_declaration record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_vat_declaration records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_vat_declaration] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_vat_declaration', NULL, NULL
GO
