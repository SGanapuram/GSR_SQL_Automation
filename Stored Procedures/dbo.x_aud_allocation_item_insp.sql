...
CREATE procedure [dbo].[x_aud_allocation_item_insp]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @alloc_num          int,
   ...
   begin
      ...
      RAISERROR('aud_allocation_item_insp: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct alloc_num, alloc_item_num, insp_comp_num
      from dbo.aud_allocation_item_insp
      order by alloc_num, alloc_item_num, insp_comp_num
 
   open mycursor
   fetch next from mycursor into @alloc_num, @alloc_item_num, @insp_comp_num
   ...
   begin
      ...
      from dbo.aud_allocation_item_insp aud
      where aud.alloc_num = @alloc_num and
            aud.alloc_item_num = @alloc_item_num and
            aud.insp_comp_num = @insp_comp_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@alloc_num as varchar) + '/' + cast(@alloc_item_num as varchar) + '/' + cast(@insp_comp_num as varchar)
      ...
      begin
         ...
         where aud.alloc_num = @alloc_num and
               aud.alloc_item_num = @alloc_item_num and
               aud.insp_comp_num = @insp_comp_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_allocation_item_insp records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_allocation_item_insp rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_allocation_item_insp rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_allocation_item_insp'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_allocation_item_insp record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_allocation_item_insp records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_allocation_item_insp] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_allocation_item_insp', NULL, NULL
GO
