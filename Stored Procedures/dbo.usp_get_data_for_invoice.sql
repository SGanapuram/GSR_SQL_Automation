...
tidp.wet_qty     as wmtQty,               
tidp.dry_qty     as DryQty,            
tidp.franchise_charge   as Fanchise,            
DNMT =            
CASE             
 WHEN tidp.franchise_charge =0 THEN tidp.dry_qty            
 WHEN tidp.franchise_charge > 0 then tidp.dry_qty -(tidp.dry_qty * tidp.franchise_charge/100)            
             
 end,            
(select spec_typical_val from trade_item_spec tis where tis.trade_num = tidp.trade_num and tis.order_num = tidp.order_num and tis.item_num = tidp.item_num and spec_code = 'MOISTURE') as MOISTURE,          
PrilimPercentage=          
case           
...
inner join voucher_cost vc on v.voucher_num=vc.voucher_num      
inner join cost c on c.cost_num= vc.cost_num      
where v.voucher_type_code ='FINAL' and c.cost_type_code in ('PO')      
...
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'usp_get_data_for_invoice', NULL, NULL
GO
