...
CREATE procedure [dbo].[x_aud_exch_fifo_alloc_item]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @exch_fifo_alloc_num      int,
        @exch_fifo_alloc_item_num smallint,
        @trans_id                 int,
        @resp_trans_id            int,
        @cutoffdate               datetime,
        @rows_deleted             int,
   ...
   begin
      ...
      RAISERROR('aud_exch_fifo_alloc_item: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct exch_fifo_alloc_num, exch_fifo_alloc_item_num
      from dbo.aud_exch_fifo_alloc_item
      order by exch_fifo_alloc_num, exch_fifo_alloc_item_num
 
   open mycursor
   fetch next from mycursor into @exch_fifo_alloc_num, @exch_fifo_alloc_item_num
   ...
   begin
      ...
      from dbo.aud_exch_fifo_alloc_item aud
      where aud.exch_fifo_alloc_num = @exch_fifo_alloc_num and
            aud.exch_fifo_alloc_item_num = @exch_fifo_alloc_item_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@exch_fifo_alloc_num as varchar) + '/' + cast(@exch_fifo_alloc_item_num as varchar)
      ...
      begin
         ...
         where aud.exch_fifo_alloc_num = @exch_fifo_alloc_num and
               aud.exch_fifo_alloc_item_num = @exch_fifo_alloc_item_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_exch_fifo_alloc_item records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_exch_fifo_alloc_item rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_exch_fifo_alloc_item rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_exch_fifo_alloc_item'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_exch_fifo_alloc_item record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_exch_fifo_alloc_item records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_exch_fifo_alloc_item] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_exch_fifo_alloc_item', NULL, NULL
GO
