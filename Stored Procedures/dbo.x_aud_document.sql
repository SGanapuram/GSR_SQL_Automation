...
CREATE procedure [dbo].[x_aud_document]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @doc_num            int,
   ...
   begin
      ...
      RAISERROR('aud_document: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct doc_num, doc_rev_num
      from dbo.aud_document
      order by doc_num, doc_rev_num
 
   open mycursor
   fetch next from mycursor into @doc_num, @doc_rev_num
   ...
   begin
      ...
      begin
         ...
         set @tempstr = cast(@doc_num as varchar) + '/' + cast(@doc_rev_num as varchar)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_document records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_document rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_document rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_document'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_document record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_document records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_document] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_document', NULL, NULL
GO
