...
CREATE procedure [dbo].[x_aud_strategy]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @user_init          char(3),
        @strategy_name      varchar(15),
        @port_num           int,
        @cmdty_code         char(8),
        @mkt_code           char(8),
        @trading_prd        char(8),
   ...
   begin
      ...
      RAISERROR('aud_strategy: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct user_init, strategy_name, port_num, cmdty_code, mkt_code, trading_prd
      from dbo.aud_strategy
      order by user_init, strategy_name, port_num, cmdty_code, mkt_code, trading_prd
 
   open mycursor
   fetch next from mycursor into @user_init, @strategy_name, @port_num, @cmdty_code, @mkt_code, @trading_prd
   ...
   begin
      ...
      from dbo.aud_strategy aud
      where aud.user_init = @user_init and
            aud.strategy_name = @strategy_name and
            aud.port_num = @port_num and
            aud.cmdty_code = @cmdty_code and
            aud.mkt_code = @mkt_code and
      ...
      begin
         ...
         set @tempstr = @user_init + '/' + @strategy_name + '/' + cast(@port_num as varchar) + '/' + @cmdty_code + '/' + @mkt_code + '/' + @trading_prd
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_strategy records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_strategy rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_strategy rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_strategy'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_strategy record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_strategy records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_strategy] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_strategy', NULL, NULL
GO
