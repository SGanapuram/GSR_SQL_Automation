...
CREATE procedure [dbo].[x_aud_contract_transmission]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @contr_num          int,
   ...
   begin
      ...
      RAISERROR('aud_contract_transmission: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct contr_num, contr_rev_num, contr_copy_num
      from dbo.aud_contract_transmission
      order by contr_num, contr_rev_num, contr_copy_num
 
   open mycursor
   fetch next from mycursor into @contr_num, @contr_rev_num, @contr_copy_num
   ...
   begin
      ...
      from dbo.aud_contract_transmission aud
      where aud.contr_num = @contr_num and
            aud.contr_rev_num = @contr_rev_num and
            aud.contr_copy_num = @contr_copy_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@contr_num as varchar) + '/' + cast(@contr_rev_num as varchar) + '/' + cast(@contr_copy_num as varchar)
      ...
      begin
         ...
         where aud.contr_num = @contr_num and
               aud.contr_rev_num = @contr_rev_num and
               aud.contr_copy_num = @contr_copy_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_contract_transmission records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_contract_transmission rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_contract_transmission rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_contract_transmission'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_contract_transmission record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_contract_transmission records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_contract_transmission] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_contract_transmission', NULL, NULL
GO
