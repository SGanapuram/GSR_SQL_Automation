...
CREATE procedure [dbo].[x_aud_idms_board_mapping]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @next_name          varchar(40),
   ...
   begin
      ...
      RAISERROR('aud_idms_board_mapping: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct next_name
      from dbo.aud_idms_board_mapping
      order by next_name
 
   open mycursor
   fetch next from mycursor into @next_name
   ...
   begin
      ...
      from dbo.aud_idms_board_mapping aud
      where aud.next_name = @next_name and
            exists (select 1
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_idms_board_mapping records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_idms_board_mapping rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_idms_board_mapping rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_idms_board_mapping'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_idms_board_mapping record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_idms_board_mapping records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_idms_board_mapping] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_idms_board_mapping', NULL, NULL
GO
