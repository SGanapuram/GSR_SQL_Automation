...
declare @my_top_port_num   int,                                                                  
   ...
   where exists (select 1                                               
                 ...
                 where ppl1.amt = ppl2.amt) */-- ADSO-13375                                
   ...
   --New Change         
   update #pl                                    
   set #pl.trade_num = inv.trade_num,  
       #pl.order_num = inv.order_num,  
    #pl.item_num = inv.sale_item_num   
   from #pl   
          join inventory inv           
             on #pl.pl_record_key = inv.pos_num  and   
       (#pl.pl_owner_code = 'P' or #pl.pl_owner_code = 'IPO')        
    
   --New Change        
   update #pl                                    
   set #pl.trade_key = convert(varchar, #pl.trade_num) + '/' +   
                           convert(varchar, #pl.order_num) + '/' +   
            convert(varchar, #pl.item_num)        
    
   --New Change           
   update #pl   
   set #pl.alloc_num = result.allocation1     
   from #pl   
           join (select min(distinct alItem.alloc_num) allocation1,       
                        alItem.trade_num,      
                  alItem.order_num,       
                        alItem.item_num      
                 from #pl   
             join allocation_item alItem      
                            on #pl.trade_num = alItem.trade_num and   
          #pl.order_num = alItem.order_num and   
          #pl.item_num = alItem.item_num       
                 group by alItem.alloc_num,  
              alItem.trade_num,     
                          alItem.order_num,  
        alItem.item_num) result       
              on #pl.trade_num = result.trade_num and   
        #pl.order_num = result.order_num and   
     #pl.item_num = result.item_num   
   where #pl.pl_owner_code='P'    
    
   update #pl   
   set #pl.alloc_item_num = alItem.alloc_item_num   
   from #pl   
           join allocation_item alItem       
              on #pl.alloc_num = alItem.alloc_num and   
        alItem.trade_num = #pl.trade_num and       
                 alItem.order_num = #pl.order_num and   
     alItem.item_num = #pl.item_num   
   where #pl.pl_owner_code='P'       
    
   --New Change          
   update #pl                                   
   set #pl.ship_id = shipment.oid   
   from #pl   
           join shipment   
              on #pl.alloc_num = shipment.alloc_num        
    
    --New Change          
    update #pl                                   
    set #pl.parcel_num = parcel.oid   
 from #pl   
         join parcel   
      on parcel.alloc_num = #pl.alloc_num and   
         parcel.alloc_item_num = #pl.alloc_item_num        
    
    --New Change          
    update #pl                                    
    set #pl.inv_trans_price = al.transfer_price   
 from #pl   
         join allocation al   
            on #pl.alloc_num = al.alloc_num       
          
    update #pl                                   
    set #pl.inv_trans_price = at.transfer_price   
 from #pl   
         join allocation_transfer at   
      on at.source_alloc_num = #pl.alloc_num             
    where #pl.pl_owner_code in ('IPO', 'CPB', 'CPO') and   
       #pl.inv_trans_price is NULL       
     
    /* here trade number and order number are inventory number and inventory build draw number in temp table ( #p1).*/    
    
   --update #pl                                       
   --  set #pl.inv_trans_price = at.transfer_price from #pl join allocation_transfer at on      
   --  (at.source_inv_num =  #pl.trade_num and at.source_invbd_num = #pl.order_num or at.target_inv_num = #pl.trade_num and at.target_invbd_num = #pl.order_num )    
   --    where #pl.pl_owner_code = 'IPO' and #pl.inv_trans_price is NULL       
       
   update #pl                                     
   set #pl.inv_trans_price = at.transfer_price   
   from #pl   
           join allocation_transfer at   
        on (at.source_inv_num =  #pl.pl_inv_num and   
         at.source_invbd_num = #pl.pl_inv_bd_num or   
      at.target_inv_num = #pl.pl_inv_num and   
      at.target_invbd_num = #pl.pl_inv_bd_num)    
   where #pl.pl_owner_code = 'IPO' and   
       #pl.inv_trans_price is NULL       
