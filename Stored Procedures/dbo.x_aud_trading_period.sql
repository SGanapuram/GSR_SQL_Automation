...
CREATE procedure [dbo].[x_aud_trading_period]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @commkt_key         int,
   ...
   begin
      ...
      RAISERROR('aud_trading_period: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct commkt_key, trading_prd
      from dbo.aud_trading_period
      order by commkt_key, trading_prd
 
   open mycursor
   fetch next from mycursor into @commkt_key, @trading_prd
   ...
   begin
      ...
      from dbo.aud_trading_period aud
      ...
      begin
         ...
         set @tempstr = cast(@commkt_key as varchar) + '/' + @trading_prd
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_trading_period records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_trading_period rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_trading_period rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_trading_period'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_trading_period record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_trading_period records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_trading_period] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_trading_period', NULL, NULL
GO
