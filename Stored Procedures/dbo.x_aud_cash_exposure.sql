...
CREATE procedure [dbo].[x_aud_cash_exposure]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @exposure_num       int,
        @cash_exp_num       smallint,
        @trans_id           int,
        @resp_trans_id      int,
        @cutoffdate         datetime,
        @rows_deleted       int,
   ...
   begin
      ...
      RAISERROR('aud_cash_exposure: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct exposure_num, cash_exp_num
      from dbo.aud_cash_exposure
      order by exposure_num, cash_exp_num
 
   open mycursor
   fetch next from mycursor into @exposure_num, @cash_exp_num
   ...
   begin
      ...
      from dbo.aud_cash_exposure aud
      where aud.exposure_num = @exposure_num and
            aud.cash_exp_num = @cash_exp_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@exposure_num as varchar) + '/' + cast(@cash_exp_num as varchar)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_cash_exposure records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_cash_exposure rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_cash_exposure rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_cash_exposure'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_cash_exposure record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_cash_exposure records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_cash_exposure] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_cash_exposure', NULL, NULL
GO
