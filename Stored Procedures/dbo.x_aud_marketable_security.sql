...
CREATE procedure [dbo].[x_aud_marketable_security]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @mkt_security_num   int,
   ...
   begin
      ...
      RAISERROR('aud_marketable_security: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct mkt_security_num
      from dbo.aud_marketable_security
      order by mkt_security_num
 
   open mycursor
   fetch next from mycursor into @mkt_security_num
   ...
   begin
      ...
      from dbo.aud_marketable_security aud
      where aud.mkt_security_num = @mkt_security_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@mkt_security_num as varchar)
      ...
      begin
         ...
         where aud.mkt_security_num = @mkt_security_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_marketable_security records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_marketable_security rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_marketable_security rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_marketable_security'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_marketable_security record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_marketable_security records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_marketable_security] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_marketable_security', NULL, NULL
GO
