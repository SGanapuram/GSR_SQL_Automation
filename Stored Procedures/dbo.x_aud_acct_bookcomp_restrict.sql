...
CREATE procedure [dbo].[x_aud_acct_bookcomp_restrict]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @acct_restriction_num int,
        @trans_id             int,
        @resp_trans_id        int,
        @cutoffdate           datetime,
        @rows_deleted         int,
   ...
   begin
      ...
      RAISERROR('aud_acct_bookcomp_restrict: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct acct_restriction_num
      from dbo.aud_acct_bookcomp_restrict
      order by acct_restriction_num
 
   open mycursor
   fetch next from mycursor into @acct_restriction_num
   ...
   begin
      ...
      from dbo.aud_acct_bookcomp_restrict aud
      where aud.acct_restriction_num = @acct_restriction_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@acct_restriction_num as varchar)
      ...
      begin
         ...
         where aud.acct_restriction_num = @acct_restriction_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_acct_bookcomp_restrict records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_acct_bookcomp_restrict rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_acct_bookcomp_restrict rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_acct_bookcomp_restrict'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_acct_bookcomp_restrict record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_acct_bookcomp_restrict records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_acct_bookcomp_restrict] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_acct_bookcomp_restrict', NULL, NULL
GO
