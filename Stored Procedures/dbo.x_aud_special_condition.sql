...
CREATE procedure [dbo].[x_aud_special_condition]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @special_cond_num   int,
   ...
   begin
      ...
      RAISERROR('aud_special_condition: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct special_cond_num
      from dbo.aud_special_condition
      order by special_cond_num
 
   open mycursor
   fetch next from mycursor into @special_cond_num
   ...
   begin
      ...
      from dbo.aud_special_condition aud
      where aud.special_cond_num = @special_cond_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@special_cond_num as varchar)
      ...
      begin
         ...
         where aud.special_cond_num = @special_cond_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_special_condition records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_special_condition rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_special_condition rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_special_condition'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_special_condition record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_special_condition records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_special_condition] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_special_condition', NULL, NULL
GO
