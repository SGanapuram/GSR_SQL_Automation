...
CREATE procedure [dbo].[x_aud_exch_fifo_alloc]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @exch_fifo_alloc_num int,
   ...
   begin
      ...
      RAISERROR('aud_exch_fifo_alloc: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct exch_fifo_alloc_num
      from dbo.aud_exch_fifo_alloc
      order by exch_fifo_alloc_num
 
   open mycursor
   fetch next from mycursor into @exch_fifo_alloc_num
   ...
   begin
      ...
      from dbo.aud_exch_fifo_alloc aud
      ...
      begin
         ...
         set @tempstr = cast(@exch_fifo_alloc_num as varchar)
      ...
      begin
         ...
         where aud.exch_fifo_alloc_num = @exch_fifo_alloc_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_exch_fifo_alloc records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_exch_fifo_alloc rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_exch_fifo_alloc rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_exch_fifo_alloc'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_exch_fifo_alloc record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_exch_fifo_alloc records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_exch_fifo_alloc] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_exch_fifo_alloc', NULL, NULL
GO
