...
CREATE procedure [dbo].[usp_get_unpaid_voucher_details_dashboard]        
as        
set nocount on        
...
-- of code is using XML Paths to concatenate the multiple       
 ...
 --Added 01/26/2012 - By Subu to Tag Interco Ind.    
 update #unpaidvouchers set InterCoInd='Y' where cp_acct_short_name in (select acct_short_name from account a, account_alias aa where a.acct_num=aa.acct_num and alias_source_code='INTERCOS')    
...
errexit:           
 ...
 isnull(InterCoInd,'N') 'InterCo Ind'  ,  
      voucher_book_date 'SAP Book Date'  
   from #unpaidvouchers        
...
endofsp:        
   ...
   return 0        
GO
GRANT EXECUTE ON  [dbo].[usp_get_unpaid_voucher_details_dashboard] TO [next_usr]
GO
