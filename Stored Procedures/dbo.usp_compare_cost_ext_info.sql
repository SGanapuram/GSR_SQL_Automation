...
set nocount on      
 ...
 lc_num,    
  ...
  into #costei1      
   from #costei      
  group by cost_num,pr_cost_num,prepayment_ind,voyage_code,qty_adj_rule_num,qty_adj_factor,orig_voucher_num,    
 pay_term_override_ind,vat_rate,discount_rate,cost_pl_contribution_ind,material_code,related_cost_num,    
 fx_exp_num,creation_fx_rate,creation_rate_m_d_ind,fx_link_oid,fx_locking_status,fx_compute_ind,    
 fx_real_port_num,reserve_cost_amt,pl_contrib_mod_transid,manual_input_pl_contrib_ind,cost_desc,    
 ...
 str(lc_num) as lc_num,    
     trans_id1      
   from #costei1 co left outer join #diffColList difc    
 on co.cost_num = difc.cost_num
   where co.resp_trans_id = @resp_trans_id_NEW      
   union            
  select       
     'OLD' as PASS,      
     b.resp_trans_id,
