...
CREATE procedure [dbo].[x_aud_interface_exec_params]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @exec_num           int,
   ...
   begin
      ...
      RAISERROR('aud_interface_exec_params: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct exec_num
      from dbo.aud_interface_exec_params
      order by exec_num
 
   open mycursor
   fetch next from mycursor into @exec_num
   ...
   begin
      ...
      from dbo.aud_interface_exec_params aud
      where aud.exec_num = @exec_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@exec_num as varchar)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_interface_exec_params records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_interface_exec_params rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_interface_exec_params rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_interface_exec_params'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_interface_exec_params record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_interface_exec_params records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_interface_exec_params] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_interface_exec_params', NULL, NULL
GO
