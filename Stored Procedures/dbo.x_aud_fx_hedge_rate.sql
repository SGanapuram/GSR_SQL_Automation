...
CREATE procedure [dbo].[x_aud_fx_hedge_rate]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @fx_hedge_rate_num  int,
   ...
   begin
      ...
      RAISERROR('aud_fx_hedge_rate: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct fx_hedge_rate_num
      from dbo.aud_fx_hedge_rate
      order by fx_hedge_rate_num
 
   open mycursor
   fetch next from mycursor into @fx_hedge_rate_num
   ...
   begin
      ...
      from dbo.aud_fx_hedge_rate aud
      where aud.fx_hedge_rate_num = @fx_hedge_rate_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@fx_hedge_rate_num as varchar)
      ...
      begin
         ...
         where aud.fx_hedge_rate_num = @fx_hedge_rate_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_fx_hedge_rate records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_fx_hedge_rate rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_fx_hedge_rate rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_fx_hedge_rate'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_fx_hedge_rate record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_fx_hedge_rate records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_fx_hedge_rate] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_fx_hedge_rate', NULL, NULL
GO
