...
CREATE procedure [dbo].[x_aud_qty_adj_rule]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @qty_adj_rule_num   int,
   ...
   begin
      ...
      RAISERROR('aud_qty_adj_rule: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct qty_adj_rule_num
      from dbo.aud_qty_adj_rule
      order by qty_adj_rule_num
 
   open mycursor
   fetch next from mycursor into @qty_adj_rule_num
   ...
   begin
      ...
      from dbo.aud_qty_adj_rule aud
      where aud.qty_adj_rule_num = @qty_adj_rule_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@qty_adj_rule_num as varchar)
      ...
      begin
         ...
         where aud.qty_adj_rule_num = @qty_adj_rule_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_qty_adj_rule records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_qty_adj_rule rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_qty_adj_rule rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_qty_adj_rule'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_qty_adj_rule record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_qty_adj_rule records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_qty_adj_rule] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_qty_adj_rule', NULL, NULL
GO
