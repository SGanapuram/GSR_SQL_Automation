...
CREATE procedure [dbo].[x_aud_commodity_market_source]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @commkt_key         int,
   ...
   begin
      ...
      RAISERROR('aud_commodity_market_source: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct commkt_key, price_source_code
      from dbo.aud_commodity_market_source
      order by commkt_key, price_source_code
 
   open mycursor
   fetch next from mycursor into @commkt_key, @price_source_code
   ...
   begin
      ...
      from dbo.aud_commodity_market_source aud
      ...
      begin
         ...
         set @tempstr = cast(@commkt_key as varchar) + '/' + @price_source_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_commodity_market_source records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_commodity_market_source rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_commodity_market_source rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_commodity_market_source'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_commodity_market_source record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_commodity_market_source records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_commodity_market_source] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_commodity_market_source', NULL, NULL
GO
