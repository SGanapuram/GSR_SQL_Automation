...
CREATE procedure [dbo].[x_aud_commkt_source_roll_date]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @commkt_key         int,
   ...
   begin
      ...
      RAISERROR('aud_commkt_source_roll_date: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct commkt_key, price_source_code, roll_date_rule_num
      from dbo.aud_commkt_source_roll_date
      order by commkt_key, price_source_code, roll_date_rule_num
 
   open mycursor
   fetch next from mycursor into @commkt_key, @price_source_code, @roll_date_rule_num
   ...
   begin
      ...
      from dbo.aud_commkt_source_roll_date aud
      where aud.commkt_key = @commkt_key and
            aud.price_source_code = @price_source_code and
            aud.roll_date_rule_num = @roll_date_rule_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@commkt_key as varchar) + '/' + @price_source_code + '/' + cast(@roll_date_rule_num as varchar)
      ...
      begin
         ...
         where aud.commkt_key = @commkt_key and
               aud.price_source_code = @price_source_code and
               aud.roll_date_rule_num = @roll_date_rule_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_commkt_source_roll_date records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_commkt_source_roll_date rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_commkt_source_roll_date rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_commkt_source_roll_date'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_commkt_source_roll_date record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_commkt_source_roll_date records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_commkt_source_roll_date] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_commkt_source_roll_date', NULL, NULL
GO
