...
CREATE procedure [dbo].[x_aud_spread_composition]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @spread_cmdty_code  char(8),
        @comp_cmdty_code    char(8),
        @trading_prd_offset int,
        @trans_id           int,
        @resp_trans_id      int,
        @cutoffdate         datetime,
        @rows_deleted       int,
   ...
   begin
      ...
      RAISERROR('aud_spread_composition: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct spread_cmdty_code, comp_cmdty_code, trading_prd_offset
      from dbo.aud_spread_composition
      order by spread_cmdty_code, comp_cmdty_code, trading_prd_offset
 
   open mycursor
   fetch next from mycursor into @spread_cmdty_code, @comp_cmdty_code, @trading_prd_offset
   ...
   begin
      ...
      from dbo.aud_spread_composition aud
      where aud.spread_cmdty_code = @spread_cmdty_code and
            aud.comp_cmdty_code = @comp_cmdty_code and
            aud.trading_prd_offset = @trading_prd_offset and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = @spread_cmdty_code + '/' + @comp_cmdty_code + '/' + cast(@trading_prd_offset as varchar)
      ...
      begin
         ...
         where aud.spread_cmdty_code = @spread_cmdty_code and
               aud.comp_cmdty_code = @comp_cmdty_code and
               aud.trading_prd_offset = @trading_prd_offset
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_spread_composition records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_spread_composition rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_spread_composition rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_spread_composition'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_spread_composition record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_spread_composition records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_spread_composition] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_spread_composition', NULL, NULL
GO
