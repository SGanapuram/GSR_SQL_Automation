...
CREATE procedure [dbo].[x_aud_market_pricing_term]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @mpt_num            int,
   ...
   begin
      ...
      RAISERROR('aud_market_pricing_term: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct mpt_num
      from dbo.aud_market_pricing_term
      order by mpt_num
 
   open mycursor
   fetch next from mycursor into @mpt_num
   ...
   begin
      ...
      from dbo.aud_market_pricing_term aud
      where aud.mpt_num = @mpt_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@mpt_num as varchar)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_market_pricing_term records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_market_pricing_term rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_market_pricing_term rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_market_pricing_term'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_market_pricing_term record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_market_pricing_term records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_market_pricing_term] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_market_pricing_term', NULL, NULL
GO
