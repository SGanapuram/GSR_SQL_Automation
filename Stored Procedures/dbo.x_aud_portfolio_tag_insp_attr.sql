...
CREATE procedure [dbo].[x_aud_portfolio_tag_insp_attr]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @tag_name           char(16),
        @ref_insp_attr_name varchar(30),
        @trans_id           int,
        @resp_trans_id      int,
        @cutoffdate         datetime,
        @rows_deleted       int,
   ...
   begin
      ...
      RAISERROR('aud_portfolio_tag_insp_attr: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct tag_name, ref_insp_attr_name
      from dbo.aud_portfolio_tag_insp_attr
      order by tag_name, ref_insp_attr_name
 
   open mycursor
   fetch next from mycursor into @tag_name, @ref_insp_attr_name
   ...
   begin
      ...
      from dbo.aud_portfolio_tag_insp_attr aud
      where aud.tag_name = @tag_name and
            aud.ref_insp_attr_name = @ref_insp_attr_name and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = @tag_name + '/' + @ref_insp_attr_name
      ...
      begin
         ...
         where aud.tag_name = @tag_name and
               aud.ref_insp_attr_name = @ref_insp_attr_name
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_portfolio_tag_insp_attr records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_portfolio_tag_insp_attr rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_portfolio_tag_insp_attr rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_portfolio_tag_insp_attr'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_portfolio_tag_insp_attr record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_portfolio_tag_insp_attr records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_portfolio_tag_insp_attr] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_portfolio_tag_insp_attr', NULL, NULL
GO
