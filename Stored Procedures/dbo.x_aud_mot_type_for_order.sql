...
CREATE procedure [dbo].[x_aud_mot_type_for_order]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @mot_type_code      char(8),
   ...
   begin
      ...
      RAISERROR('aud_mot_type_for_order: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct mot_type_code, order_type_code
      from dbo.aud_mot_type_for_order
      order by mot_type_code, order_type_code
 
   open mycursor
   fetch next from mycursor into @mot_type_code, @order_type_code
   ...
   begin
      ...
      from dbo.aud_mot_type_for_order aud
      where aud.mot_type_code = @mot_type_code and
            aud.order_type_code = @order_type_code and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = @mot_type_code + '/' + @order_type_code
      ...
      begin
         ...
         where aud.mot_type_code = @mot_type_code and
               aud.order_type_code = @order_type_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_mot_type_for_order records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_mot_type_for_order rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_mot_type_for_order rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_mot_type_for_order'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_mot_type_for_order record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_mot_type_for_order records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_mot_type_for_order] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_mot_type_for_order', NULL, NULL
GO
