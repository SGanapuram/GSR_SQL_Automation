...
CREATE procedure [dbo].[x_aud_rc_assign_trade]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @assign_num         int,
   ...
   begin
      ...
      RAISERROR('aud_rc_assign_trade: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct assign_num
      from dbo.aud_rc_assign_trade
      order by assign_num
 
   open mycursor
   fetch next from mycursor into @assign_num
   ...
   begin
      ...
      from dbo.aud_rc_assign_trade aud
      where aud.assign_num = @assign_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@assign_num as varchar)
      ...
      begin
         ...
         where aud.assign_num = @assign_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_rc_assign_trade records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_rc_assign_trade rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_rc_assign_trade rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_rc_assign_trade'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_rc_assign_trade record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_rc_assign_trade records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_rc_assign_trade] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_rc_assign_trade', NULL, NULL
GO
