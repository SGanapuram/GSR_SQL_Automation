...
CREATE procedure [dbo].[x_aud_bank_exposure]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @bank_exp_num       int,
   ...
   begin
      ...
      RAISERROR('aud_bank_exposure: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct bank_exp_num
      from dbo.aud_bank_exposure
      order by bank_exp_num
 
   open mycursor
   fetch next from mycursor into @bank_exp_num
   ...
   begin
      ...
      from dbo.aud_bank_exposure aud
      where aud.bank_exp_num = @bank_exp_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@bank_exp_num as varchar)
      ...
      begin
         ...
         where aud.bank_exp_num = @bank_exp_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_bank_exposure records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_bank_exposure rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_bank_exposure rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_bank_exposure'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_bank_exposure record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_bank_exposure records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_bank_exposure] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_bank_exposure', NULL, NULL
GO
