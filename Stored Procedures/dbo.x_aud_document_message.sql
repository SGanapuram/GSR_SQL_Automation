...
CREATE procedure [dbo].[x_aud_document_message]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set xact_abort on
declare @doc_num            int,
        @doc_rev_num        smallint,
        @doc_msg_num        smallint,
        @trans_id           int,
        @resp_trans_id      int,
        @cutoffdate         datetime,
        @rows_deleted       int,
   ...
   begin
      ...
      RAISERROR('aud_document_message: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct doc_num, doc_rev_num, doc_msg_num
      from dbo.aud_document_message
      order by doc_num, doc_rev_num, doc_msg_num
 
   open mycursor
   fetch next from mycursor into @doc_num, @doc_rev_num, @doc_msg_num
   ...
   begin
      ...
      from dbo.aud_document_message aud
      where aud.doc_num = @doc_num and
            aud.doc_rev_num = @doc_rev_num and
            aud.doc_msg_num = @doc_msg_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@doc_num as varchar) + '/' + cast(@doc_rev_num as varchar) + '/' + cast(@doc_msg_num as varchar)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_document_message records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_document_message rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_document_message rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_document_message'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_document_message record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_document_message records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_document_message] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_document_message', NULL, NULL
GO
