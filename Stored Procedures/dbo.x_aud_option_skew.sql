...
CREATE procedure [dbo].[x_aud_option_skew]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @commkt_key         int,
        ...
        @trading_prd        char(8),
        @opt_strike_skew    numeric(20, 8),
        @put_call_ind       char(1),
        @opt_vol_quote_date datetime,
        @trans_id           int,
        @resp_trans_id      int,
        @cutoffdate         datetime,
        @rows_deleted       int,
   ...
   begin
      ...
      RAISERROR('aud_option_skew: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct commkt_key, price_source_code, trading_prd, opt_strike_skew, put_call_ind, opt_vol_quote_date
      from dbo.aud_option_skew
      order by commkt_key, price_source_code, trading_prd, opt_strike_skew, put_call_ind, opt_vol_quote_date
 
   open mycursor
   fetch next from mycursor into @commkt_key, @price_source_code, @trading_prd, @opt_strike_skew, @put_call_ind, @opt_vol_quote_date
   ...
   begin
      ...
      begin
         ...
         set @tempstr = cast(@commkt_key as varchar) + '/' + @price_source_code + '/' + @trading_prd + '/' + rtrim(ltrim(str(@opt_strike_skew, 20, 8))) + '/' + @put_call_ind + '/' + convert(varchar, @opt_vol_quote_date, 101)
      ...
      begin
         ...
         where aud.commkt_key = @commkt_key and
               ...
               aud.opt_strike_skew = @opt_strike_skew and
               aud.put_call_ind = @put_call_ind and
               aud.opt_vol_quote_date = @opt_vol_quote_date
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_option_skew records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_option_skew rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_option_skew rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_option_skew'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_option_skew record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_option_skew records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_option_skew] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_option_skew', NULL, NULL
GO
