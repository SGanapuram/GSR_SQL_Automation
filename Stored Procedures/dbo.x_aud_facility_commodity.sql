...
CREATE procedure [dbo].[x_aud_facility_commodity]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @facility_code      char(8),
   ...
   begin
      ...
      RAISERROR('aud_facility_commodity: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct facility_code, cmdty_code
      from dbo.aud_facility_commodity
      order by facility_code, cmdty_code
 
   open mycursor
   fetch next from mycursor into @facility_code, @cmdty_code
   ...
   begin
      ...
      from dbo.aud_facility_commodity aud
      ...
      begin
         ...
         set @tempstr = @facility_code + '/' + @cmdty_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_facility_commodity records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_facility_commodity rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_facility_commodity rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_facility_commodity'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_facility_commodity record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_facility_commodity records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_facility_commodity] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_facility_commodity', NULL, NULL
GO
