...
CREATE procedure [dbo].[x_aud_uic_status]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @entity_id          int,
   ...
   begin
      ...
      RAISERROR('aud_uic_status: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct entity_id
      from dbo.aud_uic_status
      order by entity_id
 
   open mycursor
   fetch next from mycursor into @entity_id
   ...
   begin
      ...
      from dbo.aud_uic_status aud
      where aud.entity_id = @entity_id and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@entity_id as varchar)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_uic_status records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_uic_status rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_uic_status rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_uic_status'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_uic_status record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_uic_status records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_uic_status] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_uic_status', NULL, NULL
GO
