...
CREATE procedure [dbo].[x_aud_company_code_list]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @company_code       char(4),
   ...
   begin
      ...
      RAISERROR('aud_company_code_list: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct company_code
      from dbo.aud_company_code_list
      order by company_code
 
   open mycursor
   fetch next from mycursor into @company_code
   ...
   begin
      ...
      from dbo.aud_company_code_list aud
      where aud.company_code = @company_code and
            exists (select 1
      ...
      begin
         ...
         where aud.company_code = @company_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_company_code_list records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_company_code_list rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_company_code_list rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_company_code_list'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_company_code_list record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_company_code_list records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_company_code_list] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_company_code_list', NULL, NULL
GO
