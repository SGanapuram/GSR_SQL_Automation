...
CREATE procedure [dbo].[MDS_get_fx_data]                                                          
(                                                                              
   @port_num    int=NULL,                                                            
...
----- END- Added per lionel/JM requirement to remove SPOT FX Risk shown on Premium for a USD Pricing deal.                                
 ...
 begin catch                                                                              
  ...
  --Added by Subu on Jul 30th 2012 to reflect the correct expected payment date incase Due Date is different from expected pay date. -- Applies only for Physicals/secondary                                                                  
    ...
    select @pl_asof_date=(select max(pl_asof_date) from v_CUBE_cob_date)                                                                 
...
from #fx_dump fd                                                                     
 join v_CUBE_portfolio pt on fd.real_port_num=pt.port_num                                                      
...
return @status 
GO
GRANT EXECUTE ON  [dbo].[MDS_get_fx_data] TO [next_usr]
GO
