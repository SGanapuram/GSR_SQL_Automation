...
CREATE procedure [dbo].[x_aud_otc_option]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @otc_opt_code       char(8),
   ...
   begin
      ...
      RAISERROR('aud_otc_option: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct otc_opt_code
      from dbo.aud_otc_option
      order by otc_opt_code
 
   open mycursor
   fetch next from mycursor into @otc_opt_code
   ...
   begin
      ...
      begin
         ...
         where aud.otc_opt_code = @otc_opt_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_otc_option records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_otc_option rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_otc_option rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_otc_option'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_otc_option record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_otc_option records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_otc_option] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_otc_option', NULL, NULL
GO
