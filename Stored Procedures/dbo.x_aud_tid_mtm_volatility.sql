...
CREATE procedure [dbo].[x_aud_tid_mtm_volatility]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @dist_num           int,
   ...
   begin
      ...
      RAISERROR('aud_tid_mtm_volatility: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct dist_num, mtm_pl_asof_date, vol_num
      from dbo.aud_tid_mtm_volatility
      order by dist_num, mtm_pl_asof_date, vol_num
 
   open mycursor
   fetch next from mycursor into @dist_num, @mtm_pl_asof_date, @vol_num
   ...
   begin
      ...
      from dbo.aud_tid_mtm_volatility aud
      where aud.dist_num = @dist_num and
            aud.mtm_pl_asof_date = @mtm_pl_asof_date and
            aud.vol_num = @vol_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@dist_num as varchar) + '/' + convert(varchar, @mtm_pl_asof_date, 101) + '/' + cast(@vol_num as varchar)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_tid_mtm_volatility records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_tid_mtm_volatility rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_tid_mtm_volatility rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_tid_mtm_volatility'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_tid_mtm_volatility record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_tid_mtm_volatility records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_tid_mtm_volatility] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_tid_mtm_volatility', NULL, NULL
GO
