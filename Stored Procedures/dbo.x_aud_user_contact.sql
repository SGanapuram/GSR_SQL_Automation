...
CREATE procedure [dbo].[x_aud_user_contact]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @user_init          char(3),
   ...
   begin
      ...
      RAISERROR('aud_user_contact: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct user_init, acct_num, acct_cont_num
      from dbo.aud_user_contact
      order by user_init, acct_num, acct_cont_num
 
   open mycursor
   fetch next from mycursor into @user_init, @acct_num, @acct_cont_num
   ...
   begin
      ...
      from dbo.aud_user_contact aud
      where aud.user_init = @user_init and
            aud.acct_num = @acct_num and
            aud.acct_cont_num = @acct_cont_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = @user_init + '/' + cast(@acct_num as varchar) + '/' + cast(@acct_cont_num as varchar)
      ...
      begin
         ...
         where aud.user_init = @user_init and
               aud.acct_num = @acct_num and
               aud.acct_cont_num = @acct_cont_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_user_contact records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_user_contact rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_user_contact rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_user_contact'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_user_contact record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_user_contact records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_user_contact] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_user_contact', NULL, NULL
GO
