...
set nocount on      
   ...
   select *    
       into #plHist_EF_sum     
   from #plhist1 a    
   where pl_owner_code in ('EF', 'ES') and 
   ...
   order by real_port_num, pl_record_key, pl_owner_code,
            pl_asof_date, pl_type, resp_trans_id      
      
   drop table #plhist1   
GO
GRANT EXECUTE ON  [dbo].[usp_compare_pl_history_nonExpFutSumSwap] TO [next_usr]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'usp_compare_pl_history_nonExpFutSumSwap', NULL, NULL
GO
