...
CREATE procedure [dbo].[x_aud_path_segment]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @path_oid           int,
   ...
   begin
      ...
      RAISERROR('aud_path_segment: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct path_oid, segment_oid
      from dbo.aud_path_segment
      order by path_oid, segment_oid
 
   open mycursor
   fetch next from mycursor into @path_oid, @segment_oid
   ...
   begin
      ...
      from dbo.aud_path_segment aud
      where aud.path_oid = @path_oid and
            aud.segment_oid = @segment_oid and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@path_oid as varchar) + '/' + cast(@segment_oid as varchar)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_path_segment records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_path_segment rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_path_segment rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_path_segment'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_path_segment record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_path_segment records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_path_segment] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_path_segment', NULL, NULL
GO
