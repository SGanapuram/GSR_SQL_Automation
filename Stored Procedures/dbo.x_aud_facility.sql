...
CREATE procedure [dbo].[x_aud_facility]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @facility_code      char(8),
   ...
   begin
      ...
      RAISERROR('aud_facility: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct facility_code
      from dbo.aud_facility
      order by facility_code
 
   open mycursor
   fetch next from mycursor into @facility_code
   ...
   begin
      ...
      begin
         ...
         where aud.facility_code = @facility_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_facility records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_facility rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_facility rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_facility'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_facility record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_facility records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_facility] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_facility', NULL, NULL
GO
