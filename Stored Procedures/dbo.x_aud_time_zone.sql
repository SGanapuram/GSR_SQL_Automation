...
CREATE procedure [dbo].[x_aud_time_zone]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @time_zone_code     char(8),
   ...
   begin
      ...
      RAISERROR('aud_time_zone: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct time_zone_code
      from dbo.aud_time_zone
      order by time_zone_code
 
   open mycursor
   fetch next from mycursor into @time_zone_code
   ...
   begin
      ...
      from dbo.aud_time_zone aud
      where aud.time_zone_code = @time_zone_code and
            exists (select 1
      ...
      begin
         ...
         where aud.time_zone_code = @time_zone_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_time_zone records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_time_zone rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_time_zone rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_time_zone'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_time_zone record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_time_zone records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_time_zone] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_time_zone', NULL, NULL
GO
