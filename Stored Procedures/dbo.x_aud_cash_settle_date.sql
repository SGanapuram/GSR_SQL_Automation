...
CREATE procedure [dbo].[x_aud_cash_settle_date]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @trade_num          int,
   ...
   begin
      ...
      RAISERROR('aud_cash_settle_date: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct trade_num, order_num, cash_settle_num
      from dbo.aud_cash_settle_date
      order by trade_num, order_num, cash_settle_num
 
   open mycursor
   fetch next from mycursor into @trade_num, @order_num, @cash_settle_num
   ...
   begin
      ...
      from dbo.aud_cash_settle_date aud
      where aud.trade_num = @trade_num and
            aud.order_num = @order_num and
            aud.cash_settle_num = @cash_settle_num and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@trade_num as varchar) + '/' + cast(@order_num as varchar) + '/' + cast(@cash_settle_num as varchar)
      ...
      begin
         ...
         where aud.trade_num = @trade_num and
               aud.order_num = @order_num and
               aud.cash_settle_num = @cash_settle_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_cash_settle_date records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_cash_settle_date rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_cash_settle_date rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_cash_settle_date'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_cash_settle_date record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_cash_settle_date records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_cash_settle_date] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_cash_settle_date', NULL, NULL
GO
