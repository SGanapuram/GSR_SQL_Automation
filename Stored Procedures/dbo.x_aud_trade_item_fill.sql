...
CREATE procedure [dbo].[x_aud_trade_item_fill]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @trade_num          int,
   ...
   begin
      ...
      RAISERROR('aud_trade_item_fill: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct trade_num, order_num, item_num, item_fill_num
      from dbo.aud_trade_item_fill
      order by trade_num, order_num, item_num, item_fill_num
 
   open mycursor
   fetch next from mycursor into @trade_num, @order_num, @item_num, @item_fill_num
   ...
   begin
      ...
      begin
         ...
         set @tempstr = cast(@trade_num as varchar) + '/' + cast(@order_num as varchar) + '/' + cast(@item_num as varchar) + '/' + cast(@item_fill_num as varchar)
      ...
      begin
         ...
         where aud.trade_num = @trade_num and
               aud.order_num = @order_num and
               aud.item_num = @item_num and
               aud.item_fill_num = @item_fill_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_trade_item_fill records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_trade_item_fill rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_trade_item_fill rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_trade_item_fill'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_trade_item_fill record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_trade_item_fill records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_trade_item_fill] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_trade_item_fill', NULL, NULL
GO
