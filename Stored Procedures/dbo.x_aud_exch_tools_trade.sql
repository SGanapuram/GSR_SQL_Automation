...
CREATE procedure [dbo].[x_aud_exch_tools_trade]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
set nocount on
set xact_abort on
declare @external_trade_oid int,
        @trans_id           int,
        @resp_trans_id      int,
        @cutoffdate         datetime,
        @rows_deleted       int,
   ...
   begin
      ...
      RAISERROR('aud_exch_tools_trade: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct external_trade_oid
      from dbo.aud_exch_tools_trade
      order by external_trade_oid
 
   open mycursor
   fetch next from mycursor into @external_trade_oid
   ...
   begin
      ...
      from dbo.aud_exch_tools_trade aud
      where aud.external_trade_oid = @external_trade_oid and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@external_trade_oid as varchar)
      ...
      begin
         ...
         where aud.external_trade_oid = @external_trade_oid
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_exch_tools_trade records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_exch_tools_trade rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_exch_tools_trade rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_exch_tools_trade'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_exch_tools_trade record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_exch_tools_trade records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_exch_tools_trade] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_exch_tools_trade', NULL, NULL
GO
