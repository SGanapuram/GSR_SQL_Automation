...
CREATE procedure [dbo].[x_aud_option_strike_alias]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @commkt_key         int,
   ...
   begin
      ...
      RAISERROR('aud_option_strike_alias: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct commkt_key, trading_prd, opt_strike_price, put_call_ind, price_source_code, alias_source_code
      from dbo.aud_option_strike_alias
      order by commkt_key, trading_prd, opt_strike_price, put_call_ind, price_source_code, alias_source_code
 
   open mycursor
   fetch next from mycursor into @commkt_key, @trading_prd, @opt_strike_price, @put_call_ind, @price_source_code, @alias_source_code
   ...
   begin
      ...
      from dbo.aud_option_strike_alias aud
      where aud.commkt_key = @commkt_key and
            aud.trading_prd = @trading_prd and
            aud.opt_strike_price = @opt_strike_price and
            aud.put_call_ind = @put_call_ind and
      ...
      begin
         ...
         set @tempstr = cast(@commkt_key as varchar) + '/' + @trading_prd + '/' + rtrim(ltrim(str(@opt_strike_price, 20, 8))) + '/' + @put_call_ind + '/' + @price_source_code + '/' + @alias_source_code
      ...
      begin
         ...
         where aud.commkt_key = @commkt_key and
               ...
               aud.put_call_ind = @put_call_ind and
               aud.price_source_code = @price_source_code and
               aud.alias_source_code = @alias_source_code
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_option_strike_alias records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_option_strike_alias rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_option_strike_alias rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_option_strike_alias'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_option_strike_alias record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_option_strike_alias records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_option_strike_alias] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_option_strike_alias', NULL, NULL
GO
