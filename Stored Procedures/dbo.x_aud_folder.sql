...
CREATE procedure [dbo].[x_aud_folder]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @folder_num         int,
   ...
   begin
      ...
      RAISERROR('aud_folder: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct folder_num
      from dbo.aud_folder
      order by folder_num
 
   open mycursor
   fetch next from mycursor into @folder_num
   ...
   begin
      ...
      begin
         ...
         set @tempstr = cast(@folder_num as varchar)
      ...
      begin
         ...
         where aud.folder_num = @folder_num
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_folder records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_folder rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_folder rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_folder'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_folder record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_folder records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_folder] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_folder', NULL, NULL
GO
