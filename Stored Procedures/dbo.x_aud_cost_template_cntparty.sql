...
CREATE procedure [dbo].[x_aud_cost_template_cntparty]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @cost_template_oid  int,
   ...
   begin
      ...
      RAISERROR('aud_cost_template_cntparty: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct cost_template_oid, acct_num
      from dbo.aud_cost_template_cntparty
      order by cost_template_oid, acct_num
 
   open mycursor
   fetch next from mycursor into @cost_template_oid, @acct_num
   ...
   begin
      ...
      from dbo.aud_cost_template_cntparty aud
      where aud.cost_template_oid = @cost_template_oid and
      ...
      begin
         ...
         set @tempstr = cast(@cost_template_oid as varchar) + '/' + cast(@acct_num as varchar)
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_cost_template_cntparty records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_cost_template_cntparty rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_cost_template_cntparty rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_cost_template_cntparty'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_cost_template_cntparty record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_cost_template_cntparty records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_cost_template_cntparty] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_cost_template_cntparty', NULL, NULL
GO
