...
CREATE procedure [dbo].[x_aud_acct_bookcomp_crinfo]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @acct_bookcomp_key  int,
   ...
   begin
      ...
      RAISERROR('aud_acct_bookcomp_crinfo: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct acct_bookcomp_key
      from dbo.aud_acct_bookcomp_crinfo
      order by acct_bookcomp_key
 
   open mycursor
   fetch next from mycursor into @acct_bookcomp_key
   ...
   begin
      ...
      from dbo.aud_acct_bookcomp_crinfo aud
      where aud.acct_bookcomp_key = @acct_bookcomp_key and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = cast(@acct_bookcomp_key as varchar)
      ...
      begin
         ...
         where aud.acct_bookcomp_key = @acct_bookcomp_key
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_acct_bookcomp_crinfo records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_acct_bookcomp_crinfo rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_acct_bookcomp_crinfo rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_acct_bookcomp_crinfo'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_acct_bookcomp_crinfo record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_acct_bookcomp_crinfo records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_acct_bookcomp_crinfo] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_acct_bookcomp_crinfo', NULL, NULL
GO
