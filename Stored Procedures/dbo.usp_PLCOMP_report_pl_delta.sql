...
declare @status                  int,            
   ...
   select port_num, pl_calc_date, trans_id            
   ...
   begin try         
     ...
     TradeType,                                          
        PlTypeDesc,                                          
      --adso-5326 
      isnull(Allocation,ai.alloc_num) ,
      case when Allocation is null and ai.alloc_num is null then null else shipmnt.oid end,                                            
        AllocationKey,                                        
        AvgPrice,                                          
        MarketPrice,                                          
        PriceUom,                                       
  ...
  a.transfer_price,      
  --ADSO-3127-1,ADSO-3127-2,ADSO-3127-4,ADSO-3127-5,ADSO-3127-6,ADSO-3127-9,ADSO-3127-12,ADSO-3127-14,ADSO-3127-5      
  posHist.strike_price      
  ,posHist.put_call_ind       
  ,posM2M.otc_opt_code      
  ,case when tieo.opt_type is not null then tieo.opt_type      
  ...
  ,trd.cargo_id_number      
  ,trdItm.formula_ind      
  ,posHist.settlement_type      
  ,trdItm.p_s_ind      
  ,tioo.desired_opt_eval_method      
  ,CostNum  --BTGO-1044    
        
     from (select                   
              case when EndCOB.pl_record_key is null             
                      then 'DELETED'             
                   when StartCOB.pl_record_key is null             
                      then 'NEW'             
                   else 'CHANGE'             
              end 'ChangeType',                                          
              case when StartCOB.pl_record_key is null             
                      then EndCOB.pl_record_key             
                   else             
                      StartCOB.pl_record_key             
              end 'PLRecordKey',                                          
              ...
              case when StartCOB.pl_amt is null             
                      then EndCOB.pl_owner             
                   else             
                      StartCOB.pl_owner             
              end 'Owner',                                          
              isnull(EndCOB.trade_key,StartCOB.trade_key) 'TradeKey',                                          
              case when EndCOB.pl_amt is null             
                      then StartCOB.trade_cost_type             
                   else             
                      EndCOB.trade_cost_type             
              end 'TradeCostType',                                          
              EndCOB.pl_type_code 'PLType',                                          
              case when EndCOB.pl_amt is null             
                      then StartCOB.pl_type_desc             
                   else             
                      EndCOB.pl_type_desc             
              end 'PlTypeDesc',                                          
     ...
     StartCOB.open_qty 'Open_Qty1',                                          
              ...
              case when EndCOB.cmdty_short_name is null             
                      then StartCOB.cmdty_short_name             
                   else EndCOB.cmdty_short_name             
              end 'Commodity',                                          
              case when EndCOB.mkt_short_name is null             
                      then StartCOB.mkt_short_name             
                   else             
                      EndCOB.mkt_short_name             
              end 'Market',                                          
              case when EndCOB.pl_amt is null             
                      then StartCOB.trading_prd_desc             
                   else EndCOB.trading_prd_desc             
              end 'TradingPrd',                     
              case when EndCOB.price_curr_code is null and             
                        EndCOB.fx_rate in (1,-1)             
                      then 'USD'             
                   else             
                      EndCOB.price_curr_code             
              end  'Currency',                                          
              case when EndCOB.cost_creation_date is null and             
                        StartCOB.trade_cost_type = 'ADDITIONAL COSTS'             
                      then StartCOB.cost_creation_date                                           
                   when EndCOB.trade_cost_type = 'ADDITIONAL COSTS'             
                      then EndCOB.cost_creation_date             
                   else             
                      null             
              end 'CostCreation',                                              
              case when EndCOB.creator_init is null             
                      then StartCOB.creator_init             
                   else             
                      EndCOB.creator_init             
              end 'Creator',                                          
              EndCOB.fx_rate 'FxRate',                                          
              EndCOB.inhouse_ind 'InhouseInd',                                          
              case when EndCOB.pl_realization_date is null             
                      then StartCOB.pl_realization_date             
                   else             
                      EndCOB.pl_realization_date             
              end 'PLRealizationDate',                                          
              case when EndCOB.contr_date is null             
                      then StartCOB.contr_date             
                   else             
                      EndCOB.contr_date              
              end 'ContractDate',                                          
              ...
              --ADSO-3127          
              case when EndCOB.alloc_num is not null and EndCOB.alloc_item_num is not NULL then convert(varchar, EndCOB.alloc_num) + '/' +  convert(varchar, EndCOB.alloc_item_num)          
     ...
     NULL          
              ...
              case when EndCOB.pl_amt is null             
                      then StartCOB.trading_prd_date             
                   else             
                      EndCOB.trading_prd_date             
              end 'TradingPrdDate',                                          
              ...
              case when (isnull(EndCOB.fx_rate, 1) in (1, 0) OR             
                         isnull(StartCOB.fx_rate, 1) in (1,0) )                               
                      then 0                               
              ...
              EndCOB.trader_name,                                 
              EndCOB.trade_num 'TradeNum',              
              EndCOB.order_num 'OrderNum',              
              EndCOB.item_num 'ItemNum'         
              --ADSO-3127      
              --,      
              --StartCOB.inv_trans_price ,          
     -- StartCOB.cross_port_trans_price      
      ...
      --ADSO-3127      
                 ...
                 from #pl a,             
                      #portpl2 ppl                                       
                 where a.pl_asof_date = @cob_date2 and             
                        ppl.port_num = a.real_port_num) EndCOB                                          
                    FULL OUTER JOIN             
       ...
       a.sch_qty,        
          ...
          --a.cross_port_trans_price     
                        ...
                        from #pl a,             
                             #portpl1 ppl                                            
                        where a.pl_asof_date = @cob_date1 and                                           
                              ppl.port_num = a.real_port_num) StartCOB                                          
                     ON EndCOB.real_port_num = StartCOB.real_port_num and             
                        EndCOB.pl_record_key = StartCOB.pl_record_key and             
                        EndCOB.pl_type_code = StartCOB.pl_type_code and             
                        EndCOB.pl_owner_code = StartCOB.pl_owner_code) res                                         
             LEFT OUTER JOIN dbo.portfolio port WITH (NOLOCK)            
                ON port.port_num = res.PortNum        
                      
             --ADSO-3127-1,ADSO-3127-2,ADSO-3127-4,ADSO-3127-5,ADSO-3127-6,ADSO-3127-9,ADSO-3127-12,ADSO-3127-14      
    ...
    LEFT OUTER JOIN dbo.trade_item trdItm WITH (NOLOCK)            
                ON trdItm.trade_num = res.TradeNum and               
                   trdItm.order_num = res.OrderNum and               
                   trdItm.item_num = res.ItemNum      
             left outer join dbo.trade_item_exch_opt tieo       
    on tieo.trade_num = res.TradeNum and               
                   tieo.order_num = res.OrderNum and               
                   tieo.item_num = res.ItemNum      
    left outer join trade_item_otc_opt tioo       
       on tioo.trade_num = res.TradeNum and               
                   tioo.order_num = res.OrderNum and               
                   tioo.item_num = res.ItemNum        
                   --BTGO-1044    
    --         left OUTER JOIN dbo.cost c      
    --on c.cost_owner_key6 = res.TradeNum and       
    --   c.cost_owner_key7 = res.OrderNum and       
    --   c.cost_owner_key8 = res.ItemNum and c.cost_status = 'OPEN' and c.cost_type_code = 'WPP'       
    left outer join dbo.allocation_item ai on             
       ai.trade_num = res.TradeNum and               
                   ai.order_num = res.OrderNum and               
                   ai.item_num =  res.ItemNum                                                                    
             left outer join dbo.allocation a on             
     ai.alloc_num = a.alloc_num        
     --adso-5326   
     left outer join dbo.shipment shipmnt on
          shipmnt.alloc_num = ai.alloc_num 
   ...
   begin try                 
     ...
     set Commodity = pl.Commodity,                                
      ...
      from #PlDelta pd            
              JOIN #PlDelta pl                                
                 ON pd.TradeKey = pl.TradeKey and            
                    pd.TradeCostType = pl.TradeCostType            
      where pd.TradeType is null and             
            pl.TradeType is not null                                
   end try            
   begin catch            
...
next1:                 
   ...
   select @portfolio_tag_column_list = tag_column_list            
   from #tag_column_info            
   where entity_name = 'Portfolio'            
                      
   select @trade_item_tag_column_list = tag_column_list            
   from #tag_column_info            
   ...
   select @portfolio_show_tag_column_list = entity_tags_show_list              
   from #entities              
   where entity_name = 'Portfolio'              
                        
   select @trade_item_show_tag_column_list = entity_tags_show_list              
   from #entities              
   where entity_name = 'TradeItem'             
             
   if @portfolio_show_tag_column_list is not null and @portfolio_show_tag_column_list <> ''          
   begin          
    --declare @portfolio_show_tag_column_list    varchar(MAX)          
    --SET @portfolio_show_tag_column_list = 'BOOKCOMP,BOOKLOC'          
    SET @portfolio_show_tag_column_list = '[' + @portfolio_show_tag_column_list + ']' --[BOOKCOMP,BOOKLOC]          
    SET @portfolio_show_tag_column_list = REPLACE(@portfolio_show_tag_column_list, ',', '(PortfolioTAG)], [') -- [BOOKCOMP(PortfolioTAG)], [BOOKLOC]          
    set @portfolio_show_tag_column_list = @portfolio_show_tag_column_list + '(PortfolioTAG)]' -- [BOOKCOMP(PortfolioTAG)], [BOOKLOC](PortfolioTAG)]          
    SET @portfolio_show_tag_column_list = REPLACE(@portfolio_show_tag_column_list, '](', '(')          
    --select @portfolio_show_tag_column_list          
   end           
               
   if @trade_item_show_tag_column_list is not null and @trade_item_show_tag_column_list <> ''          
   begin          
    --declare @trade_item_show_tag_column_list    varchar(MAX)          
    --SET @trade_item_show_tag_column_list = 'BOOKCOMP,BOOKLOC'          
    SET @trade_item_show_tag_column_list = '[' + @trade_item_show_tag_column_list + ']' --[BOOKCOMP,BOOKLOC]          
    SET @trade_item_show_tag_column_list = REPLACE(@trade_item_show_tag_column_list, ',', '(TradeItemTAG)], [') -- [BOOKCOMP(PortfolioTAG)], [BOOKLOC]          
    set @trade_item_show_tag_column_list = @trade_item_show_tag_column_list + '(TradeItemTAG)]' -- [BOOKCOMP(PortfolioTAG)], [BOOKLOC](PortfolioTAG)]          
    SET @trade_item_show_tag_column_list = REPLACE(@trade_item_show_tag_column_list, '](', '(')          
    --select @trade_item_show_tag_column_list          
  end          
            
   -----------------------------------------------------------------------         
            
   print 'DEBUG: @portfolio_tag_column_list = ''' + @portfolio_tag_column_list + ''''            
   print 'DEBUG: @trade_item_tag_column_list = ''' + @trade_item_tag_column_list + ''''            
         
   --ADSO-3127-1,ADSO-3127-2,ADSO-3127-4,ADSO-3127-5,ADSO-3127-6,ADSO-3127-9,ADSO-3127-12,ADSO-3127-14,ADSO-3127-5      
   set @sql = '            
   select  distinct case when PLRealizationDate > cobDate2 then ''No''             
       ...
       NewContract,                                
          ...
          begin          
           set @sql = @sql +   'case when InvTransferPrice is NULL then CrossPortTransferPrice          
	                           else InvTransferPrice         
          ...
          Contr_Qty1,            
          Contr_Qty as Contr_Qty2,             
          (isnull(Contr_Qty, 0) - isnull(Contr_Qty1, 0)) as Contr_QtyChgs,         
    Sch_Qty1,            
          Sch_Qty as Sch_Qty2,             
          ...
          QtyUom,                                
          case when InhouseInd = ''I'' then ''Internal''            
          ...
          cob2Calc as PlCalcDate2,                                
					cobDate1 as COBDate1,
					cobDate2 as COBDate2, 	          
          CostCreation as CostCreationDate,                                
          FxRelatedInd,                                
          TradeModDate as TradeModDate,                                
          TradingEntity,                             
          PositionNumber as PositionNumber,               
          case when TradeType in (''FUTURE'', ''SWAP'', ''PHYSICAL'') then ''Total Trade Value''           
    ...
    case when SettlementType = ''C'' then ''Cash''      
      when SettlementType = ''P'' then ''Physical''      
      else      
      NULL      
      end as SettlementType,      
    case when PSInd = ''P'' then ''Buy''      
      when PSInd = ''S'' then ''Sell''      
      else      
      NULL      
      end as BuyOrSell,      
    case when DesiredOptEvalMethod is not null and DesiredOptEvalMethod = ''C'' then ''Calculate''      
     when DesiredOptEvalMethod is not null and DesiredOptEvalMethod = ''L'' then ''Look-Up''      
     else      
     NULL      
     end as MarketEvalMethod,       
    CostNum as CostNumber,       
          case when Counterparty in (''NYMEX'', ''THE ICE'', ''LCH CLEARNET'', ''SGX'', ''NOS CLEARING'') OR             
                    QtyUom = ''LOTS'' OR             
                    TradeType = ''FUTURE''             
                  then ''CLEARED''                    
               when InhouseInd in (''Y'', ''I'')             
                  then ''INTERNAL''                
        else ''OTC''             
          end ''InstrumentType'''           
                    
          if (@portfolio_show_tag_column_list is null or @portfolio_show_tag_column_list = '') and (@trade_item_show_tag_column_list is null or @trade_item_show_tag_column_list = '')          
          begin          
   set @sql = @sql + ' from #PlDelta pl'          
          end          
          else if (@portfolio_show_tag_column_list is not null or @portfolio_show_tag_column_list <> '') and (@trade_item_show_tag_column_list is null or @trade_item_show_tag_column_list = '')          
         begin          
   set @sql = @sql + ' ,'  + @portfolio_show_tag_column_list + ' from #PlDelta pl LEFT OUTER JOIN #xx101_porttags pt with (nolock) on pl.PortNum = pt.port_num '            
          end          
          else if (@portfolio_show_tag_column_list is null or @portfolio_show_tag_column_list  = '') and (@trade_item_show_tag_column_list is not null or @trade_item_show_tag_column_list <> '')          
          begin          
   set @sql = @sql + ' ,'  + @trade_item_show_tag_column_list  + ' from #PlDelta pl  '            
   set @sql = @sql + 'LEFT OUTER JOIN #xx101_titags ti with (nolock) on pl.TradeNum = ti.trade_num and '            
   set @sql = @sql + 'pl.OrderNum = ti.order_num and pl.ItemNum = ti.item_num'            
          end          
          else          
          begin          
             set @sql = @sql + ' , ' +  @portfolio_show_tag_column_list + ', ' + @trade_item_show_tag_column_list + ' '            
   ...
   if @debugon = 1            
   begin            
      set @smsg = '=> ' + cast(@rows_affected as varchar) + ' records in result set'            
      RAISERROR (@smsg, 0, 1) WITH NOWAIT            
      set @time_finished = (select convert(varchar, getdate(), 109))            
      set @smsg = '==> Started : ' + @time_started            
      RAISERROR (@smsg, 0, 1) WITH NOWAIT            
