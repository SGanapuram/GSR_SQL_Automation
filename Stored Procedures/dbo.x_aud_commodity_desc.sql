...
CREATE procedure [dbo].[x_aud_commodity_desc]
(
   @daysold   smallint = 30,
   @debugon   char(1) = 'N'
)
...
declare @cmdty_code         char(8),
        @cmdty_desc_lang    varchar(15),
        @cmdty_desc_for     varchar(8),
        @trans_id           int,
        @resp_trans_id      int,
        @cutoffdate         datetime,
        @rows_deleted       int,
   ...
   begin
      ...
      RAISERROR('aud_commodity_desc: Purging records before ''%s''', 0, 1, @smsg) with nowait
   end
 
   declare mycursor CURSOR FORWARD_ONLY READ_ONLY for
      select distinct cmdty_code, cmdty_desc_lang, cmdty_desc_for
      from dbo.aud_commodity_desc
      order by cmdty_code, cmdty_desc_lang, cmdty_desc_for
 
   open mycursor
   fetch next from mycursor into @cmdty_code, @cmdty_desc_lang, @cmdty_desc_for
   ...
   begin
      ...
      from dbo.aud_commodity_desc aud
      where aud.cmdty_code = @cmdty_code and
            aud.cmdty_desc_lang = @cmdty_desc_lang and
            aud.cmdty_desc_for = @cmdty_desc_for and
            exists (select 1
      ...
      begin
         ...
         set @tempstr = @cmdty_code + '/' + @cmdty_desc_lang + '/' + @cmdty_desc_for
      ...
      begin
         ...
         where aud.cmdty_code = @cmdty_code and
               aud.cmdty_desc_lang = @cmdty_desc_lang and
               aud.cmdty_desc_for = @cmdty_desc_for
      ...
      begin try
       ...
       begin catch
         ...
         RAISERROR('=> Failed to delete aud_commodity_desc records due to the error:', 0, 1) with nowait
      ...
      begin
         if @rows_deleted > 0
            RAISERROR('    => %d aud_commodity_desc rows were deleted!', 0, 1, @rows_deleted) with nowait
         else
            RAISERROR('    => No aud_commodity_desc rows were deleted!', 0, 1) with nowait
      end
   ...
   if @total_rows_deleted = 0
      RAISERROR('No rows were deleted from the ''aud_commodity_desc'' table!', 0, 1) with nowait
   else
   begin
      if @total_rows_deleted = 1
         RAISERROR('1 aud_commodity_desc record was deleted!', 0, 1) with nowait
      else
         RAISERROR('%d aud_commodity_desc records were deleted!', 0, 1, @total_rows_deleted) with nowait
   end
   return 0
GO
GRANT EXECUTE ON  [dbo].[x_aud_commodity_desc] TO [ictspurge]
GO
EXEC sp_addextendedproperty N'SymphonyProduct', N'OIL', 'SCHEMA', N'dbo', 'PROCEDURE', N'x_aud_commodity_desc', NULL, NULL
GO
